...
c
      subroutine slxcd3(nx,ny,nz,phi,cof,tx,sum,nxa,nyc,nze)
       !dir$ attributes code_align : 32 :: slxcd3
       !dir$ optimize : 3
       !dir$ attributes optimization_parameter: "TARGET_ARCH=skylake_avx512" :: slxcd3
       use omp_lib
c
...
c
        !dir$ assume_aligned phi:64,cof:64,tx:64
!$OMP PARALLEL DO SCHEDULE(STTAIC,8) PRIVATE(i,ib,j,k) SHARED(phi,cof,tx,nx,ny,nz)
	do k=1,nz,2
	  do j=1,ny,2
           !dir$ ivdep
           !dir$ vector aligned
...
c
        !dir$ assume_aligned sum:64,phi:64,cof:64,tx:64
...
!$OMP PARALLEL DO SCHEDULE(STATIC,8) PRIVATE(i,ib,j,k) SHARED(sum,phi,cof,tx,nx,ny,nz)
	...
	return
      ...
      subroutine slycd3(nx,ny,nz,phi,cof,ty,sum,nxa,nyc,nze)
       !dir$ attributes code_align : 32 :: slycd3
       !dir$ optimize : 3
       !dir$ attributes optimization_parameter: "TARGET_ARCH=skylake_avx512" :: slycd3
       use omp_lib
c
...
c
        !dir$ assume_aligned phi:64,cof:64,ty:64
!$OMP PARALLEL DO SCHEDULE(STATIC,8) PRIVATE(i,jb,j,k) SHARED(phi,cof,ty,nx,ny,nz)
	do k=1,nz,2
	  do i=1,nx,2
           !dir$ ivdep
           !dir$ vector aligned
...
!$OMP PARALLEL DO PRIVATE(i,jb,j,k), SHARED(sum,phi,cof,ty,nx,ny,nz)
	do k=1,nz,2
	  do i=1,nx,2
	    do j=1,ny-1
	      phi(i,j,k) = cof(i,j,k,8) - (
...
c
           ...
           !dir$ unroll(8)
           !dir$ fma
           !$omp simd reduction(/:phi) reduction(-:phi)
           do i=1,nx,2
...
c     forward even-even
c
         !dir$ assume_aligned sum:64,cof:64,phi:64,ty:64
!$OMP PARALLEL DO SCHEDULE(STATIC,8) PRIVATE(i,jb,j,k) SHARED(sum,phi,cof,ty,nx,ny,nz)
	do k=2,nz,2
	  do i=2,nx,2
	    do j=1,ny-1
	      phi(i,j,k) = cof(i,j,k,8) - (
...
!$OMP PARALLEL DO SCHEDULE(STATIC,8) PRIVATE(i,jb,j,k) SHARED(sum,phi,cof,ty,nx,ny,nz)
	...
	return
      ...
      subroutine slzcd3(nx,ny,nz,phi,cof,tz,sum,nxa,nyc,nze)
       !dir$ attributes code_align : 32 :: slzcd3
       !dir$ optimize : 3
       !dir$ attributes optimization_parameter: "TARGET_ARCH=skylake_avx512" :: slzcd3
       use omp_lib
c
