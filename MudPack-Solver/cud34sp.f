...
c ... purpose
c
c     cud34sp attempts to improve the second order approximation generated
c     by cud3sp to a fourth order approximation using difference corrections
...
c
c
      subroutine cud34sp(work,phi,ierror)
        !dir$ attributes code_align : 32 :: cud34sp
        !dir$ optimize : 3
      implicit none
      complex phi(*),work(*)
      integer ierror
     ...
     +kcycle,iprer,ipost,intpol
      !dir$ attributes align : 64 :: /icud3sp/
      integer nx,ny,nz
      ierror = 0
      nx = nfx
      ny = nfy
      ...
      call cd34sp(nx,ny,nz,phi,work)
      end

      subroutine cd34sp(nx,ny,nz,phi,wk)
       !dir$ attributes forceinline :: cd34sp
       !dir$ attributes code_align : 32 :: cd34sp
       !dir$ optimize : 3
      implicit none
      integer nx,ny,nz
      complex phi(nx,ny,nz),wk(*)
      integer intl,nxa,nxb,nyc,nyd,nze,nzf,ixp,jyq,kzr,iex,jey,kez,
     +nfx,nfy,nfz,iguess,maxcy,method,nwork,lwork,itero,
     +kcycle,iprer,ipost,intpol
      common/icud3sp/intl,nxa,nxb,nyc,nyd,nze,nzf,ixp,jyq,kzr,iex,jey,
     ...
     +kczbgn(50),nxk(50),nyk(50),nzk(50),ngrid,klevel,kcur,kps
      !dir$ attributes align : 64 :: /cud3spc/
...
c
      call ctr3sp(nx,ny,nz,phi,wk(irf),wk(icx),wk(icy),wk(icz))
c
...
c
	call cres3(nx,ny,nz,wk(irf),ncx,ncy,ncz,wk(irc),nxa,nxb,
     +             nyc,nyd,nze,nzf)
      end do
c
...
c
	...
	ncy = nyk(k)
	ncz = nzk(k)
	call cprolon3(ncx,ncy,ncz,wk(ipc),nx,ny,nz,wk(ipf),nxa,nxb,nyc,
     +                nyd,nze,nzf,intpol)
      end do
      kcur = ngrid
      call kcycd3sp(wk)
...
c
	...
	do j=1,ny
      ...
      subroutine ctr3sp(nx,ny,nz,u,r,cofx,cofy,cofz)
       !dir$ attributes forceinline :: ctr3sp
       !dir$ attributes code_align : 32 :: ctr3sp
       !dir$ optimize : 3
       !dir$ attributes optimization_parameter: "TARGET_ARCH=skylake_avx512" :: ctr3sp
       use omp_lib
c
...
c
     ...
     +kcycle,iprer,ipost,intpol
      ...
      common/fcud3sp/xa,xb,yc,yd,ze,zf,tolmax,relmax
        !dir$ attributes align : 64 :: /fcud3sp/
     ...
     +               dlx4,dly4,dlz4
      ...
      complex tx,ty,tz,cxx,cyy,czz,ux3,ux4,uy3,uy4,uz3,uz4,cx,cy,cz
      dlx = (xb-xa)/(nx-1)
      dly = (yd-yc)/(ny-1)
      dlz = (zf-ze)/(nz-1)
      dlxx = dlx*dlx
...
C$OMP PARALLEL DO SCHEDULE(STATIC,8) PRIVATE(i,j,k,ux3,ux4,uy3,uy4,uz3,uz4) &
C$OMP PRIVATE(cxx,cx,cyy,cy,czz,cz,tx,ty,tz) &
C$OMP SHARED (r,u,cofx,cofy,cofz,nx,ny,nz,dlx,dly,dlz) &
C$OMP SHARED (dlxx,dlyy,dlzz,dlx4,dly4,dlz4,tdlx3,tdly3,tdlz3)
      do k=3,nz-2
	do j=3,ny-2
          !dir$ assume_aligned r:64,u:64
          !dir$ ivdep
          !dir$ vector aligned
          !dir$ vector always
	  do i=3,nx-2
...
!$omp shared(r,jfinal,ifinal)
	...
	do i=istart,ifinal
	  do j=jstart,2
	    ...
	    czz = 0.5*dlzz*(cofz(k,1)+cofz(k,2))
	    cz = dlz*(cofz(k,2)-cofz(k,1))
	    call cpde3(nx,ny,nz,u,i,j,k,ux3,ux4,uy3,uy4,uz3,uz4,
     +                 nxa,nyc,nze)
	    tx = (cxx*ux4*0.5+cx*ux3)/6.0
	    ty = (cyy*uy4*0.5+cy*uy3)/6.0
	    tz = (czz*uz4*0.5+cz*uz3)/6.0
