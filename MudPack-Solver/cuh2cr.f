...
c
!$OMP PARALLEL DO SCHEDULE(STATIC,8) SHARED(sum,cof,phi,tx,nx,ny) PRIVATE(i,ib,j)
	do j=1,ny,2
	  do i=1,nx-1
	    phi(i,j) = cof(i,j,10)-(cof(i,j,2)*phi(i+1,j+1)+
     +                              cof(i,j,3)*phi(i,j+1)+
...
c
!$OMP PARALLEL DO SCHEDULE(STATIC,8) SHARED(cof,phi,ty,nx,ny) PRIVATE(i,jb,j)
	do i=1,nx,2
	  do j=1,ny
...
c
!$OMP PARALLEL DO SCHEDULE(STATIC,8) SHARED(sum,cof,phi,ty,nx,ny) PRIVATE(i,jb,j)
	do i=1,nx,2
	  do j=1,ny-1
	    phi(i,j) = cof(i,j,10)-(cof(i,j,1)*phi(i+1,j)+
     +                              cof(i,j,2)*phi(i+1,j+1)+
