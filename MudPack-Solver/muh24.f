...
c ... purpose
c
c     muh24 attempts to improve the second order approximation generated
c     by muh2 to a fourth order approximation using difference corrections
...
c     muh2.f, mudcom.f
c
      subroutine muh24(wk,iwk,phi,ierror)
       !dir$ attributes code_align : 32 :: muh24
       !dir$ optimize : 3
      implicit none
      integer iwk(*)
      real phi(*),wk(*)
	...
	return
      ...
      call mh24(nx,ny,phi,wk,iwk)
      end

      subroutine mh24(nx,ny,phi,wk,iwk)
       !dir$ attributes forceinline :: mh24
       !dir$ attributes code_align : 32 :: mh24
       !dir$ optimize : 3
       !dir$ attributes optimization_parameter: "TARGET_ARCH=skylake_avx512" :: mh24
       use omp_lib
      implicit none
      integer nx,ny,iwk(*)
      integer intl,nxa,nxb,nyc,nyd,ixp,jyq,iex,jey,nfx,nfy,iguess,
...
c
      nx = nxk(ngrid)
      ny = nyk(ngrid)
      !dir$ assume_aligned phi:64,wk:64
!$omp parallel do default(none) schedule(static,8) &
!$omp private(j,i,ij) shared(ny,nx,jnx,phi,wk)
      do j=1,ny
	jnx = j*(nx+2)
...
!$omp end parallel do
      ...
      subroutine trh2(nx,ny,u,cof)
       !dir$ attributes forceinline :: trh2
       !dir$ attributes code_align : 32 :: trh2
       !dir$ optimize : 3
       !dir$ attributes optimization_parameter: "TARGET_ARCH=skylake_avx512" :: trh2
      use omp_lib
c
...
!$omp private(i,j,cxx,cx,cyy,ux3,ux4,uy3,uy4,tx,ty) &
!$omp shared(istart,ifinal,dlxx,u,cof,dlyy,nx,ny,nxa) &
!$omp shared(nyc,dlx,dly)
      do i=istart,ifinal
        !dir$ assume_aligned cof:64,u:64
	do j=jstart,2
	  cxx = 0.5*dlxx*(cof(i,j,1)+cof(i,j,2))
	  cx = dlx*(cof(i,j,2)-cof(i,j,1))
	  cyy = 0.5*dlyy*(cof(i,j,3)+cof(i,j,4))
...
!$omp end parallel do
!$omp parallel do default(none) schedule(static,8)  &
!$omp private(j,i,cxx,cx,cyy,cy,ux3,ux4,uy3,uy4)    &
!$omp private(tx,ty)                                &
!$omp shared(ny,istart,dlxx,cof,u,dlx,dlyy,nx,ny)   &
!$omp shared(nxa,nyc,dly,ifinal)
      do j=3,ny-2
        !dir$ assume_aligned cof:64,u:64
	do i=istart,2
	  cxx = 0.5*dlxx*(cof(i,j,1)+cof(i,j,2))
