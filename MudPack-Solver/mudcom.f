...
c
      subroutine swk2(nfx,nfy,phif,rhsf,phi,rhs)
       !dir$ attributes forceinline :: swk2
       !dir$ attributes code_align : 32 :: swk2
       !dir$ optimize : 3
       !dir$ attributes optimization_parameter: "TARGET_ARCH=skylake_avx512" :: swk2
c
...
c
	...
	phi(i,nfy+1) = 0.0
      ...
      subroutine trsfc2(nx,ny,phi,rhs,ncx,ncy,phic,rhsc)
        !dir$ attributes forceinline :: trsfc2
       !dir$ attributes code_align : 32 :: trsfc2
       !dir$ optimize : 3
       !dir$ attributes optimization_parameter: "TARGET_ARCH=skylake_avx512" :: trsfc2
c
...
c
        !dir$ assume_aligned phic:64,phi:64,rhsc:64,rhs:64
	do jc=1,ncy
	  j = jc+jc-1
           !dir$ ivdep
           !dir$ vector aligned
...
c
	...
	do jc=1,ncy
      ...
      subroutine res2(nx,ny,resf,ncx,ncy,rhsc,nxa,nxb,nyc,nyd)
       !dir$ attributes code_align : 32 :: res2
       !dir$ optimize : 3
       !dir$ attributes optimization_parameter: "TARGET_ARCH=skylake_avx512" :: res2
       use omp_lib
      implicit none
...
!$omp private(jc,j,jm1,jp1,ic,i,im1,ip1)             &
!$omp shared(ncy,jy,nyc,ny,ncx,ix,nx,nxa)            &
!$omp shared(rhsc,resf)
...
c
        !dir$ assume_aligned rhsc:64,resf:64
	do ic=1,ncx,ncx-1
	  i = ic+ix*(ic-1)
	  im1 = max0(i-1,2)
	  ip1 = min0(i+1,nx-1)
...
c
      subroutine prolon2(ncx,ncy,p,nx,ny,q,nxa,nxb,nyc,nyd,intpol)
         !dir$ attributes code_align : 32 :: prolon2
         !dir$ optimize : 3
      implicit none
...
c
      subroutine prolon1(ncx,p,nx,q,nxa,nxb,intpol)
        !dir$ attributes code_align : 32 :: prolon1
       !dir$ optimize : 3
      implicit none
...
c
     ...
     +                phcor)
       !dir$ attributes forceinline :: cor2
       !dir$ attributes code_align : 32 :: cor2
       !dir$ optimize : 3
       !dir$ attributes optimization_parameter: "TARGET_ARCH=skylake_avx512" :: cor2
       use omp_lib
c
...
c
      ...
      !dir$ assume_aligned phcor:64
      do j=0,ny+1
        !dir$ vector aligned
        !dir$ vector always
        !dir$ vector nontemporal(phcor)
        !dir$ unroll(16)
	do i=0,nx+1
	  phcor(i,j) = 0.0
	end do
...
c
	...
	do j=jst,jfn
      ...
      subroutine pde2(nx,ny,u,i,j,ux3,ux4,uy3,uy4,nxa,nyc)
       !dir$ attributes forceinline :: pde2
       !dir$ attributes code_align : 32 :: pde2
       !dir$ optimize : 3
      use omp_lib
      implicit none
      ...
      !dir$ attributes align : 64 :: /pde2com/
      real ux3,ux4,uy3,uy4
!$omp threadprivate(pde2com)
c
	...
	if (j.gt.2 .and. j.lt.ny-1) then
     ...
     +          /dly4
      ...
      subroutine swk3(nfx,nfy,nfz,phif,rhsf,phi,rhs)
       !dir$ attributes forceinline :: swk3
       !dir$ attributes code_align : 32 :: swk3
       !dir$ optimize : 3
       !dir$ attributes optimization_parameter: "TARGET_ARCH=skylake_avx512" :: swk3
c
c     set phif,rhsf input in arrays which include
c     virtual boundaries for phi (for all 2-d real codes)
c
      ...
      !dir$ assume_aligned phi:64,phif:64,rhs:64,rhsf:64
      do k=1,nfz
	do j=1,nfy
          !dir$ vector aligned
          !dir$ vector always
...
c
	...
	do i=0,nfx+1
      ...
      subroutine trsfc3(nx,ny,nz,phi,rhs,ncx,ncy,ncz,phic,rhsc)
       !dir$ attributes forceinline :: trsfc3
       !dir$ attributes code_align : 32 :: trsfc3
       !dir$ optimize : 3
       !dir$ attributes optimization_parameter: "TARGET_ARCH=skylake_avx512" :: trsfc3
c
c     transfer fine grid to coarse grid
c
      implicit none
