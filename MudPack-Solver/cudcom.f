...
c ... purpose
c
c     cudcom.f is a file containing subroutines called by some or all 
c     of the COMPLEX two- and three-dimensional mudpack solvers.  
c     cudcom.f must be loaded with any complex mudpack solver.
c
      subroutine cswk2(nfx,nfy,phif,rhsf,phi,rhs)
c
c     set phif,rhsf input in arrays which include
c     virtual boundaries for phi (for all 2-d complex codes)
c
      implicit none
      integer nfx,nfy,i,j
      complex phif(nfx,nfy),rhsf(nfx,nfy)
      complex phi(0:nfx+1,0:nfy+1),rhs(nfx,nfy)
      do j=1,nfy
	do i=1,nfx
	  phi(i,j) = phif(i,j)
	  rhs(i,j) = rhsf(i,j)
...
c
	...
	phi(i,nfy+1) = (0.0,0.0)
      ...
      subroutine ctrsfc2(nx,ny,phi,rhs,ncx,ncy,phic,rhsc)
c
...
c
      ...
      complex phic(0:ncx+1,0:ncy+1),rhsc(ncx,ncy)
c
c     set virtual boundaries in phic to zero
c
      do jc=0,ncy+1
	phic(0,jc) = (0.0,0.0)
	phic(ncx+1,jc) = (0.0,0.0)
      end do
      do ic=0,ncx+1
	phic(ic,0) = (0.0,0.0)
	phic(ic,ncy+1) = (0.0,0.0)
      end do
...
c
	do jc=1,ncy
      ...
      subroutine cres2(nx,ny,resf,ncx,ncy,rhsc,nxa,nxb,nyc,nyd)
      implicit none
...
c     using full weighting for all complex 2d codes
c
      complex resf(nx,ny),rhsc(ncx,ncy)
c
...
c
      subroutine cprolon2(ncx,ncy,p,nx,ny,q,nxa,nxb,nyc,nyd,intpol)
      implicit none
      integer ncx,ncy,nx,ny,intpol,nxa,nxb,nyc,nyd
      complex p(0:ncx+1,0:ncy+1),q(0:nx+1,0:ny+1)
...
c
	  do j=joddst,joddfn,2
	    jc = j/2+1
	    call cprolon1(ncx,p(0,jc),nx,q(0,j),nxa,nxb,intpol)
	  end do
	  do j=2,jfn,2
	    do i=ist,ifn
...
c
      subroutine cprolon1(ncx,p,nx,q,nxa,nxb,intpol)
      implicit none
      integer intpol,nxa,nxb,ncx,nx,i,ic,ist,ifn,ioddst,ioddfn
      complex p(0:ncx+1),q(0:nx+1)
      ist = 1
      ioddst = 1
      ifn = nx
      ioddfn = nx
...
c
      ...
      subroutine ccor2(nx,ny,phif,ncx,ncy,phic,nxa,nxb,nyc,nyd,intpol,
     +                 phcor)
c
...
c
      ...
      complex phif(0:nx+1,0:ny+1),phic(0:ncx+1,0:ncy+1)
      complex phcor(0:nx+1,0:ny+1)
      do j=0,ny+1
	do i=0,nx+1
	  phcor(i,j) = (0.0,0.0)
	end do
...
c
      call cprolon2(ncx,ncy,phic,nx,ny,phcor,nxa,nxb,nyc,nyd,intpol)
c
...
c
	...
	do j=jst,jfn
      ...
      subroutine cpde2(nx,ny,u,i,j,ux3,ux4,uy3,uy4,nxa,nyc)
      implicit none
      ...
      complex u(nx,ny),ux3,ux4,uy3,uy4
c
...
c
	if (j.gt.2 .and. j.lt.ny-1) then
     ...
     +          /dly4
      ...
      subroutine cswk3(nfx,nfy,nfz,phif,rhsf,phi,rhs)
c
c     set phif,rhsf input in arrays which include
c     virtual boundaries for phi (for all 2-d complex real codes)
c
      implicit none
      integer nfx,nfy,nfz,i,j,k
      complex phif(nfx,nfy,nfz),rhsf(nfx,nfy,nfz)
      complex phi(0:nfx+1,0:nfy+1,0:nfz+1),rhs(nfx,nfy,nfz)
      complex zero
      zero = (0.0,0.0)
      do k=1,nfz
	do j=1,nfy
...
c
	...
	do i=0,nfx+1
      ...
      subroutine ctrsfc3(nx,ny,nz,phi,rhs,ncx,ncy,ncz,phic,rhsc)
c
c     transfer fine grid to coarse grid
c
      implicit none
      ...
      complex phic(0:ncx+1,0:ncy+1,0:ncz+1),rhsc(ncx,ncy,ncz)
      complex zero
      zero = (0.0,0.0)
c
c     set virtual boundaries in phic to zero
c
	...
	do jc=0,ncy+1
	  phic(0,jc,kc) = zero
	  phic(ncx+1,jc,kc) = zero
	end do
      end do
      do kc=0,ncz+1
	do ic=0,ncx+1
	  phic(ic,0,kc) = zero
	  phic(ic,ncy+1,kc) = zero
	end do
      end do
      do jc=0,ncy+1
	do ic=0,ncx+1
	  phic(ic,jc,0) = zero
	  phic(ic,jc,ncz+1) = zero
	end do
      end do
...
c
	...
	do kc=1,ncz
      ...
      subroutine cres3(nx,ny,nz,resf,ncx,ncy,ncz,rhsc,
...
c
      ...
      complex rm,rk,rp
      complex resf(nx,ny,nz),rhsc(ncx,ncy,ncz)
      complex zero
      zero = (0.0,0.0)
c
...
c
C$OMP PARALLEL DO PRIVATE(i,j,k,ic,jc,kc,rm,rk,rp)
C$OMP+SHARED(resf,rhsc,ncx,ncy,ncz)
      do kc=2,ncz-1
	k = kc+kc-1
	do jc=2,ncy-1
	  j = jc+jc-1
...
c
C$OMP PARALLEL DO PRIVATE(i,j,k,ic,jc,kc,rm,rk,rp)
C$OMP+SHARED(ix,jy,kz,resf,rhsc,ncx,ncy,ncz)
      do kc=2,ncz-1
	k = kc+kz*(kc-1)
	do jc=2,ncy-1
	  j = jc+jy*(jc-1)
...
c    (y,z) interior
c
C$OMP PARALLEL DO PRIVATE(j,k,jc,kc,rm,rk,rp)
C$OMP+SHARED(kz,jy,ic,im1,i,ip1,resf,rhsc,ncy,ncz)
	do kc=2,ncz-1
	  k = kc+kz*(kc-1)
	  do jc=2,ncy-1
	    j = jc+jy*(jc-1)
...
c     (x,z) interior
c
C$OMP PARALLEL DO PRIVATE(i,k,ic,kc,rm,rk,rp)
C$OMP+SHARED(ix,kz,jc,jm1,j,jp1,resf,rhsc,ncx,ncz)
	do kc=2,ncz-1
	  k = kc+kz*(kc-1)
	  do ic=2,ncx-1
	    i = ic+ix*(ic-1)
...
c     (x,y) interior
c
C$OMP PARALLEL DO PRIVATE(i,j,ic,jc,rm,rk,rp)
C$OMP+SHARED(ix,jy,kc,km1,k,kp1,resf,rhsc,ncx,ncz)
	do jc=2,ncy-1
	  j = jc+jy*(jc-1)
	  do ic=2,ncx-1
	    i = ic+ix*(ic-1)
...
c
      subroutine cprolon3(ncx,ncy,ncz,p,nx,ny,nz,q,nxa,nxb,nyc,nyd,
     +                    nze,nzf,intpol)
      implicit none
      integer ncx,ncy,ncz,nx,ny,nz,intpol,nxa,nxb,nyc,nyd,nze,nzf
      complex p(0:ncx+1,0:ncy+1,0:ncz+1),q(0:nx+1,0:ny+1,0:nz+1)
...
c
	  do k=koddst,koddfn,2
	    kc = k/2+1
	    call cprolon2(ncx,ncy,p(0,0,kc),nx,ny,q(0,0,k),nxa,nxb,nyc,
     +                   nyd,intpol)
	  end do
	  do k=2,kfn,2
	    do j=jst,jfn
...
c
      ...
      subroutine ccor3(nx,ny,nz,phif,ncx,ncy,ncz,phic,nxa,nxb,nyc,nyd,
...
c     in phif using linear or cubic interpolation
c
      complex phif(0:nx+1,0:ny+1,0:nz+1),phic(0:ncx+1,0:ncy+1,0:ncz+1)
      complex phcor(0:nx+1,0:ny+1,0:nz+1)
      do k=0,nz+1
	do j=0,ny+1
	  do i=0,nx+1
	    phcor(i,j,k) = (0.0,0.0)
...
c     lift correction in phic to fine grid in phcor
c
      call cprolon3(ncx,ncy,ncz,phic,nx,ny,nz,phcor,nxa,nxb,nyc,nyd,
     +             nze,nzf,intpol)
c
c     add correction in phcor to phif on nonspecified boundaries
c
...
c
	...
	do k=kst,kfn
      ...
      subroutine cper3vb(nx,ny,nz,phi,nxa,nyc,nze)
c
...
c     x=xb,y=yd
     ...
     + / dxxyy
      ...
      subroutine cpde3(nx,ny,nz,u,i,j,k,ux3,ux4,uy3,uy4,uz3,uz4,
     +                nxa,nyc,nze)
c
...
c
     ...
     +               dlx4,dly4,dlz4
      complex ux3,ux4,uy3,uy4,uz3,uz4
c
c     x,y partial derivatives
c
      call cp3de2(nx,ny,u(1,1,k),i,j,ux3,ux4,uy3,uy4,nxa,nyc)
c
...
c
     ...
     +       u(i,j,3))/dlz4
      ...
      subroutine cp3de2(nx,ny,u,i,j,ux3,ux4,uy3,uy4,nxa,nyc)
c
...
c
      ...
      common/pde3com/dlx,dly,dlz,dlxx,dlyy,dlzz,tdlx3,tdly3,tdlz3,
     +               dlx4,dly4,dlz4
      complex ux3,ux4,uy3,uy4
      l=ny
c
c     x partial derivatives
c
      call cp3de1(nx,u(1,j),i,ux3,ux4,nxa)
c
c     y partial derivatives
c
      if (nyc.ne.0) then
...
c
     ...
     +        dly4
      ...
      subroutine cp3de1(nx,u,i,ux3,ux4,nxa)
c
...
c
c
c     cfactri and cfactrip are:
c     subroutines called by any complex mudpack solver which uses line
...
c
	...
	do j=1,m
      ...
      subroutine cfactrp(m,n,a,b,c,d,e,sum)
c
c     factor the m simultaneous "tridiagonal" systems of order n
...
c
      implicit none
      integer m,n,i,j
      complex a(n,m),b(n,m),c(n,m),d(n,m),e(n,m),sum(m)
      do j=1,m
	d(1,j) = a(1,j)
      end do
      do i=2,n-2
...
c
	...
	b(n-1,j) = b(n-1,j)-sum(j)
      ...
      subroutine ctransp(n,amat)
c
c     transpose n by n complex matrix
c
      implicit none
      integer n,i,j
      complex amat(n,n),temp
      do i=1,n-1
	do j=i+1,n
	  temp = amat(i,j)
	  amat(i,j) = amat(j,i)
...
c
c     factor na by na complex matrix a
c
      subroutine csgfa (a,lda,n,ipvt,info)
      integer lda,n,ipvt(1),info                                                
      complex a(lda,1),t
      integer cisfmax,j,k,kp1,l,nm1
      info = 0                                                                  
      nm1 = n - 1                                                               
      if (nm1 .lt. 1) go to 70                                                  
      do 60 k = 1, nm1                                                          
         kp1 = k + 1                                                            
	 l = cisfmax(n-k+1,a(k,k),1) + k - 1
         ipvt(k) = l                                                            
	 if (cabs(a(l,k)) .eq. 0.0e0) go to 40
            if (l .eq. k) go to 10                                              
               t = a(l,k)                                                       
               a(l,k) = a(k,k)                                                  
               a(k,k) = t                                                       
	    ...
	    call csscl(n-k,t,a(k+1,k),1)
            do 30 j = kp1, n                                                    
               ...
               if (l .eq. k) go to 20                                           
                  ...
                  a(k,j) = t                                                    
   20          continue                                                         
	       call csxpy(n-k,t,a(k+1,k),1,a(k+1,j),1)
   30       continue                                                            
         go to 50                                                               
   40    continue                                                               
            info = k                                                            
...
c
c     solve na by na complex matrix equation a*x=b
c     solution overwrite rhs in b
c
      subroutine csgsl (a,lda,n,ipvt,b,job)
      ...
      if (job .ne. 0) go to 50                                                  
         ...
         do 20 k = 1, nm1                                                       
            ...
            if (l .eq. k) go to 10                                              
               ...
               b(k) = t                                                         
   10       continue                                                            
	    call csxpy(n-k,t,a(k+1,k),1,b(k+1),1)
   20    continue                                                               
   30    continue                                                               
         do 40 kb = 1, n                                                        
            k = n + 1 - kb                                                      
            b(k) = b(k)/a(k,k)                                                  
            t = -b(k)                                                           
	    call csxpy(k-1,t,a(1,k),1,b(1),1)
   40    continue                                                               
      go to 100                                                                 
   50 continue                                                                  
         do 60 k = 1, n                                                         
	    t = csdt(k-1,a(1,k),1,b(1),1)
            b(k) = (b(k) - t)/a(k,k)                                            
   60    continue                                                               
         if (nm1 .lt. 1) go to 90                                               
         do 80 kb = 1, nm1                                                      
            k = n - kb                                                          
	    b(k) = b(k) + csdt(n-k,a(k+1,k),1,b(k+1),1)
            l = ipvt(k)                                                         
            if (l .eq. k) go to 70                                              
               t = b(l)                                                         
               b(l) = b(k)                                                      
  ...
  100 continue                                                                  
      ...
      complex function csdt(n,sx,incx,sy,incy)
      complex sx(1),sy(1),stemp
   ...
   60 csdt = stemp
      ...
      integer function cisfmax(n,sx,incx)
      complex sx(1)
      real smax
      integer i,incx,ix,n                                                       
      cisfmax = 0
   ...
   30 continue                                                                  
      ...
      subroutine csxpy(n,sa,sx,incx,sy,incy)
      complex sx(1),sy(1),sa
      integer i,incx,incy,ix,iy,m,mp1,n                                         
      if(n.le.0)return                                                          
      if (cabs(sa) .eq. 0.0) return
      if(incx.eq.1.and.incy.eq.1)go to 20                                       
      ix = 1                                                                    
      iy = 1                                                                    
      if(incx.lt.0)ix = (-n+1)*incx + 1                                         
