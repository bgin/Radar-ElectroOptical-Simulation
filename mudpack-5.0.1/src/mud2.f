...
c ... purpose
c
c     mud2 attempts to produce a second order finite difference
...
c
c
      subroutine mud2(iparm,fparm,work,coef,bndyc,rhs,phi,mgopt,ierror)
      implicit none
...
c         ip = kpbgn(k)
	  ...
	  ity = ktybgn(k)
	  klevel = k
	  call dismd2(nx,ny,work(ic),work(itx),work(ity),
	...
	return
      ...
      nx = nfx
      ny = nfy
      call mud21(nx,ny,rhs,phi,coef,bndyc,work)
      iparm(17) = itero
	...
	if (relmax.gt.tolmax) ierror = -1   ! flag convergenc failure
      ...
      subroutine mud21(nx,ny,rhsf,phif,coef,bndyc,wk)
      implicit none
      integer nx,ny
...
c
	do k=1,ngrid
	  ...
	  ip = kpbgn(k)
	  ic = kcbgn(k)
	  call adjmd2(nx,ny,wk(ip),wk(ic),bndyc,coef)
	end do
c
...
c
      do l=1,iprer
	call relmd2(nx,ny,wk(ip),wk(ic),wk(itx),wk(ity),wk(kps))
      end do
...
c
      ...
      call resmd2(nx,ny,wk(ip),ncx,ncy,wk(ipc),wk(irc),wk(ic),wk(kps))
c
...
c
	  ...
	  ncy = nyk(klevel-1)
	  irc = kcbgn(klevel-1)+5*ncx*ncy
	  call resmd2(nx,ny,wk(ip),ncx,ncy,wk(ipc),wk(irc),wk(ic),
     +                wk(kps))
c
...
c
	...
	call relmd2(nx,ny,wk(ip),wk(ic),wk(itx),wk(ity),wk(kps))
      ...
      subroutine dismd2(nx,ny,cof,tx,ty,bndyc,coef,wk,ier)
c
...
c
     ...
     +                  ty(1,1,5),wk(kps))
      ...
      subroutine adjmd2(nx,ny,phi,cof,bndyc,coef)
c
...
c
	...
	do i=1,nx
      ...
      subroutine resmd2(nx,ny,phi,ncx,ncy,phic,rhsc,cof,resf)
c
...
c
      ...
      subroutine relmd2(nx,ny,phi,cof,tx,ty,sum)
c
...
c
	...
	call relmd2p(nx,ny,phi,cof)
      else if (method.eq.1) then           ! line x relaxation
	call slxmd2(nx,ny,phi,cof,tx,sum)
      else if (method.eq.2) then           ! line y relaxation
	call slymd2(nx,ny,phi,cof,ty,sum)
	...
	call slymd2(nx,ny,phi,cof,ty,sum)
      ...
      subroutine relmd2p(nx,ny,phi,cof)
c
...
c
	...
	do i=1,nx
      ...
      subroutine slxmd2(nx,ny,phi,cof,tx,sum)
c
...
c
	...
	do i=1,nx
      ...
      subroutine slymd2(nx,ny,phi,cof,ty,sum)
      implicit none
