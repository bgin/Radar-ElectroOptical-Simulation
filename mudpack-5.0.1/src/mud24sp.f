...
c ... purpose
c
c     mud24sp attempts to improve the second order approximation generated
c     by mud2sp to a fourth order approximation using difference corrections
...
c
c
      subroutine mud24sp(work,phi,ierror)
      implicit none
      real phi(*),work(*)
	...
	return
      ...
      call mu24sp(nx,ny,phi,work)
      end

      subroutine mu24sp(nx,ny,phi,wk)
      implicit none
      integer nx,ny
      integer intl,nxa,nxb,nyc,nyd,ixp,jyq,iex,jey,nfx,nfy,iguess,
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
...
c
      call tr2sp(nx,ny,phi,wk(irf),wk(icx),wk(icy))
c
...
c
	...
	do i=1,nx
      ...
      subroutine tr2sp(nx,ny,u,rhs,cofx,cofy)
c
c     estimate truncation error using second order approximation in u
c
      implicit none
      integer nx,ny
      real rhs(nx,ny),u(nx,ny),cofx(nx,3),cofy(ny,3)
      integer intl,nxa,nxb,nyc,nyd,ixp,jyq,iex,jey,nfx,nfy,iguess,
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
      real xa,xb,yc,yd,tolmax,relmax
...
!$OMP SHARED (rhs,u,cofx,cofy,nx,ny)
