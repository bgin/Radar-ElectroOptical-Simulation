...
c ... purpose
c
c     (cuh3 is a complex hybrid multigrid/direct method solver)
c     cuh3 attempts to produce a second order finite difference
...
c
c     Here cxx,cyy,czz,cx,cy,cz,ce,r,p are all complex valued
c     functions of the real variables x,y,z.
c
...
c
c
      subroutine cuh3(iparm,fparm,wk,iwk,coef,bndyc,rhs,phi,mopt,ierror)
      implicit none
     ...
     +kcycle,iprer,ipost,intpol
      common/icud3/intl,nxa,nxb,nyc,nyd,nze,nzf,ixp,jyq,kzr,iex,jey,kez,
     +nfx,nfy,nfz,iguess,maxcy,method,meth2,nwork,lwork,itero,
     +kcycle,iprer,ipost,intpol
      real xa,xb,yc,yd,ze,zf,tolmax,relmax
      common/fcud3/xa,xb,yc,yd,ze,zf,tolmax,relmax
      integer kpbgn,kcbgn,ktxbgn,ktybgn,ktzbgn,nxk,nyk,nzk,ngrid,
     +        klevel,kcur,kps
      common/cud3c/kpbgn(50),kcbgn(50),ktxbgn(50),ktybgn(50),
     +ktzbgn(50),nxk(50),nyk(50),nzk(50),ngrid,klevel,kcur,kps
      integer ibeta,ialfa,izmat,idmat
      common/cuh3c/ibeta,ialfa,izmat,idmat
...
c
	do kb=1,ngrid
	  ...
	  itz = ktzbgn(k)
	  klevel = k
	  call disch3(nx,ny,nz,wk(ic),wk(itx),
...
c
	...
	return
      ...
      ny = nfy
      nz = nfz
      call cuh31(nx,ny,nz,rhs,phi,coef,bndyc,wk,iwk)
c
...
c
      ...
      subroutine cuh31(nx,ny,nz,rhsf,phif,coef,bndyc,wk,iwk)
      implicit none
      integer iwk(*)
      integer intl,nxa,nxb,nyc,nyd,nze,nzf,ixp,jyq,kzr,iex,jey,kez,
     +nfx,nfy,nfz,iguess,maxcy,method,meth2,nwork,lwork,itero,
...
c
      call cswk3(nx,ny,nz,phif,rhsf,wk(ip),wk(ir))
      if (iguess.eq.0) then
c
...
c
	do kb=1,ngrid
	  ...
	  ip = kpbgn(k)
	  ic = kcbgn(k)
	  call adjch3(nx,ny,nz,wk(ip),wk(ic),bndyc,coef)
c
...
c
c     execute maxcy more cultigrid k cycles from finest level
c
      kcur = ngrid
      do iter=1,maxcy
	itero = iter
...
c
	  ...
	  do k=1,nfz
	    ...
	    do j=1,nfy
	      ...
	      do i=1,nfx
		ijk = jk+i+1
		phmax = amax1(phmax,cabs(wk(ijk)))
		relmax = amax1(relmax,cabs(wk(ijk)-phif(i,j,k)))
		phif(i,j,k) = wk(ijk)
	      end do
	    end do
	  end do
...
c
	if (nze .ne. 0) then
	  call cdir3(nxny,nx,ny,nz,wk(ip),wk(ic),wk(ibeta),wk(ialfa),
     +               wk(kps),iwk)
	  return
	else
	  call cdir3p(nxny,nx,ny,nz,wk(ip),wk(ic),wk(ibeta),wk(ialfa),
...
c
      call resch3(nx,ny,nz,wk(ip),wk(ic),ncx,ncy,ncz,wk(ipc),wk(irc),
     +            wk(kps))
c
...
c
	if (klevel .gt. 1) then
	  ...
	  irc = kcbgn(klevel-1)+7*ncx*ncy*ncz
	  ipc = kpbgn(klevel-1)
	  call resch3(nx,ny,nz,wk(ip),wk(ic),ncx,ncy,ncz,wk(ipc),
     +              wk(irc),wk(kps))
c
...
c
	...
	call relch3(wk)
      ...
      subroutine resch3(nx,ny,nz,phi,cof,ncx,ncy,ncz,phic,rhsc,resf)
c
...
c
     ...
     +kcycle,iprer,ipost,intpol
      ...
      complex rhsc(ncx,ncy,ncz),resf(nx,ny,nz),cof(nx,ny,nz,8)
c
...
c
!$OMP PARALLEL DO SCHEDULE(STATIC,8) PRIVATE(i,j,k) SHARED(resf,cof,phi,nx,ny,nz)
      do k=1,nz
	do j=1,ny
	  do i=1,nx
...
c
      ...
      subroutine disch3(nx,ny,nz,cof,tx,ty,tz,bndyc,coef,wk,iwk,ier)
c
c     discretize the 3-d elliptic pde
c
      implicit none
      integer nx,ny,nz,ier
      complex cof(nx,ny,nz,8)
     ...
     +ktzbgn(50),nxk(50),nyk(50),nzk(50),ngrid,klevel,kcur,kps
      integer ibeta,ialfa,izmat,idmat
      common/cuh3c/ibeta,ialfa,izmat,idmat
      real x,y,z,dlx,dly,dlz,dlx2,dly2,dlz2,dlxx,dlyy,dlzz
      real cmin,cemax,alfmax,cxxr,cxxi,cyyr,cyyi,czzr,czzi
      complex cxx,cyy,czz,cx,cy,cz,ce,alfa,gbdy,c1,c2,c3,c4,c5,c6
...
c
	  call clud3(nxny,nx,ny,nz,cof,wk(ibeta),wk(ialfa),iwk,nxa,nyc)
	  return
	else
c
...
c
	  do k=1,nz-1
	    call csetbet3(nxny,nx,ny,nz,cof,wk(ibeta),k,nxa,nyc)
	    call csetalf3(nxny,nx,ny,nz,cof,wk(ialfa),k)
	  end do
	  call clud3p(nxny,nx,ny,nz,cof,wk(ibeta),wk(ialfa),wk(izmat),
...
c
     ...
     +                   tz(1,1,1,4),tz(1,1,1,5),wk(kps))
      ...
      subroutine adjch3(nx,ny,nz,phi,cof,bndyc,coef)
c
...
c
      implicit none
      integer nx,ny,nz
      complex phi(0:nx+1,0:ny+1,0:nz+1),cof(nx,ny,nz,8)
      integer intl,nxa,nxb,nyc,nyd,nze,nzf,ixp,jyq,kzr,iex,jey,kez,
     +nfx,nfy,nfz,iguess,maxcy,method,meth2,nwork,lwork,itero,
     +kcycle,iprer,ipost,intpol
      real xa,xb,yc,yd,ze,zf,tolmax,relmax
...
c
	call relch3p(nx,ny,nz,wk(ip),wk(ic))
	return
      end if
      itx = ktxbgn(klevel)
      ity = ktybgn(klevel)
...
c
	...
	itx = ktxbgn(klevel)
	call slxcd3(nx,ny,nz,wk(ip),wk(ic),wk(itx),wk(kps),nxa,nyc,nze)
	else
c
...
c
	...
	ity = ktybgn(klevel)
	call slycd3(nx,ny,nz,wk(ip),wk(ic),wk(ity),wk(kps),nxa,nyc,nze)
	else
c
...
c
	...
	itz = ktzbgn(klevel)
	call slzcd3(nx,ny,nz,wk(ip),wk(ic),wk(itz),wk(kps),nxa,nyc,nze)
	else
c
...
c
	call cplanyz(wk)
      ...
      subroutine relch3p(nx,ny,nz,phi,cof)
c
...
c
!$OMP PARALLEL DO SCHEDULE(STATIC,8) PRIVATE(i,j,k) SHARED(phi,cof,nx,ny,nz)
      do k=1,nz,2
c
...
c
      ...
      subroutine clud3(nxny,nx,ny,nz,cof,beta,alfa,index,nxa,nyc)
c
...
c
      ...
      complex cof(nx,ny,nz,8),beta(nxny,nxny,*),alfa(nxny,nxny,*)
...
c
      kcur = 1
      call csetbet3(nxny,nx,ny,nz,cof,beta,kcur,nxa,nyc)
      call csgfa(beta,nxny,nxny,index,iz)
      do kcur=2,nz
	call csetalf3(nxny,nx,ny,nz,cof,alfa,kcur)
	call ctransp(nxny,alfa(1,1,kcur))
	km1 = kcur-1
	do ll=1,nxny
	  call csgsl(beta(1,1,km1),nxny,nxny,index(1,km1),
     +               alfa(1,ll,kcur),i1)
	end do
	call ctransp(nxny,alfa(1,1,kcur))
c
...
c
	iz = 0
	call csgfa(beta(1,1,kcur),nxny,nxny,index(1,kcur),iz)
      end do
c
...
c
      ...
      subroutine cdir3(nxny,nx,ny,nz,phi,cof,beta,alfa,phxy,index)
c
...
c
      ...
      complex phi(0:nx+1,0:ny+1,0:nz+1),cof(nx,ny,nz,8)
      complex phxy(nxny)
      complex beta(nxny,nxny,*),alfa(nxny,nxny,*)
c     forward sweep
      call cfor3(nxny,nx,ny,nz,phi,cof(1,1,1,8),alfa)
c     backward sweep
      call cbkw3(nxny,nx,ny,nz,phi,cof,beta,phxy,index)
      return
      end

      subroutine cfor3(nxny,nx,ny,nz,phi,frhs,alfa)
c
c     forward sweep
c
      implicit none
      integer nxny,nx,ny,nz,i,j,k,ii,ll,il,jl
      complex phi(0:nx+1,0:ny+1,0:nz+1)
      complex frhs(nx,ny,nz),alfa(nxny,nxny,*)
      complex sum
      do k=1,nz
	do j=1,ny
	  do i=1,nx
...
c
	do ii=1,nxny
      ...
      subroutine cbkw3(nxny,nx,ny,nz,phi,cof,beta,phxy,index)
C                                                                               
C     backward sweep
c
      implicit none
      ...
      complex phi(0:nx+1,0:ny+1,0:nz+1),cof(nx,ny,nz,8)
      complex beta(nxny,nxny,*),phxy(nxny)
...
C
	...
	do i=1,nx
      ...
      call csgsl(beta(1,1,nz),nxny,nxny,index(1,nz),phxy,iz)
      do j=1,ny
	do i=1,nx
	  ij = (j-1)*nx+i
	  phi(i,j,nz) = phxy(ij)
...
C
	...
	call csgsl(beta(1,1,k),nxny,nxny,index(1,k),phxy,iz)
	do j=1,ny
	  do i=1,nx
	    ij = (j-1)*nx+i
	    phi(i,j,k) = phxy(ij)
      ...
      subroutine clud3p(nxny,nx,ny,nz,cof,beta,alfa,zmat,dmat,index,
     +                  nxa,nyc)
C                                                                               
...
c
      ...
      complex cof(nx,ny,nz,8),alfa(nxny,nxny,*),beta(nxny,nxny,*)
      complex dmat(nxny,nxny,*),zmat(nxny,nxny,*),sum
...
c
      call csetbet3(nxny,nx,ny,nz,cof,beta,kcur,nxa,nyc)
      iz = 0
      call csgfa(beta(1,1,1),nxny,nxny,index(1,1),iz)
      do kcur=2,nz-2
c
...
c
	...
	i = ii-(j-1)*nx
	do jj=1,nxny
	  zmat(jj,ii,1) = (0.0,0.0)
	end do
	zmat(ii,ii,1) = cof(i,j,nz-1,6)
      end do
      do ll=1,nxny
	call csgsl(beta(1,1,1),nxny,nxny,index(1,1),zmat(1,ll,1),i1)
      end do
      call ctransp(nxny,zmat(1,1,1))
c
...
c
	...
	do ll=1,nxny
	  call csgsl(beta(1,1,kcur),nxny,nxny,index(1,kcur),
     +               zmat(1,ll,kcur),i1)
	end do
	call ctransp(nxny,zmat(1,1,kcur))
      end do
c
c     solve transpose of zmat(nz-2)umat(nz-2)=alfa(nz-1)-zmat(nz-3)gama(nz-3)
c
      call csetalf3(nxny,nx,ny,nz,cof,alfa,nz-1)
      do ii=1,nxny
	do jj=1,nxny
	  j = ((jj-1)/nx)+1
	  i = jj-(j-1)*nx
     ...
     +                   cof(i,j,nz-3,6)
      ...
      do ll=1,nxny
	call csgsl(beta(1,1,nz-2),nxny,nxny,index(1,nz-2),
     +             zmat(1,ll,nz-2),i1)
      end do
      call ctransp(nxny,zmat(1,1,nz-2))
c
c     set umat(nz-1) = beta(nz-1)-(zmat(1)*dmat(1)+...+zmat(nz-2)*dmat(nz-2))
c     in beta(nz-1)
c
      call csetbet3(nxny,nx,ny,nz,cof,beta,nz-1,nxa,nyc)
      do ii=1,nxny
	do jj=1,nxny
	  sum = (0.0,0.0)
	  do kcur=1,nz-2
...
c
      call csgfa(beta(1,1,nz-1),nxny,nxny,index(1,nz-1),iz)
c
...
c
      ...
      subroutine cdir3p(nxny,nx,ny,nz,phi,cof,beta,alfa,zmat,dmat,
     +                  phxy,index)
c
...
c
      ...
      complex phi(0:nx+1,0:ny+1,0:nz+1),cof(nx,ny,nz,8)
      complex beta(nxny,nxny,*),alfa(nxny,nxny,*)
      complex zmat(nxny,nxny,*), dmat(nxny,nxny,*)
      complex phxy(nxny)
c     forward sweep for periodic
      call cfor3p(nxny,nx,ny,nz,phi,cof(1,1,1,8),alfa,zmat)
c     backward sweep for periodic
      call cbkw3p(nxny,nx,ny,nz,phi,cof,beta,dmat,phxy,index)
      return
      end

      subroutine cfor3p(nxny,nx,ny,nz,phi,frhs,alfa,zmat)
C                                                                               
...
C                                                                               
      ...
      complex phi(0:nx+1,0:ny+1,0:nz+1)
      complex frhs(nx,ny,nz)
      complex alfa(nxny,nxny,*),zmat(nxny,nxny,*),sum
...
c
	...
	phi(i,j,nz-1) = phi(i,j,nz-1) - sum
      ...
      subroutine cbkw3p(nxny,nx,ny,nz,phi,cof,beta,dmat,phxy,index)
C                                                                               
C     DO BACKWARD SWEEP
c
      implicit none
      integer nxny,nx,ny,nz,index(nxny,nz)
      complex phi(0:nx+1,0:ny+1,0:nz+1),cof(nx,ny,nz,8)
      complex beta(nxny,nxny,nz),dmat(nxny,nxny,nz)
      complex phxy(nxny),sum
...
C
	...
	do i=1,nx
      ...
      call csgsl(beta(1,1,nz-1),nxny,nxny,index(1,nz-1),phxy,iz)
      do j=1,ny
	do i=1,nx
	  ij = (j-1)*nx+i
...
c
	...
	do i=1,nx
      ...
      call csgsl(beta(1,1,nz-2),nxny,nxny,index(1,nz-2),phxy,iz)
      do j=1,ny
	do i=1,nx
	  ij = (j-1)*nx+i
...
c
	...
	do i=1,nx
      ...
      subroutine csetbet3(nxny,nx,ny,nz,cof,beta,kcur,nxa,nyc)
c
c     set diagonal matrix on block for k = kcur
c
      implicit none
      integer nxny,nx,ny,nz,nxa,nyc
      complex cof(nx,ny,nz,8),beta(nxny,nxny,*)
      integer k,kcur,ii,ll,ij,jj,i,j
      k = kcur
      do ii = 1,nxny
	do ll=1,nxny
	  beta(ii,ll,k)=(0.0,0.0)
	end do
      end do
      do j=1,ny
	do i=1,nx
...
c
	...
	do i=1,nx
      ...
      subroutine csetalf3(nxny,nx,ny,nz,cof,alfa,kcur)
C                                                                               
C     set block subdiagonal matrix for k = kcur
C                                                                               
      implicit none
      integer nxny,nx,ny,nz,kcur
      complex cof(nx,ny,nz,8),alfa(nxny,nxny,*)
      integer k,j,i,ij,ll
      k = kcur
      do j=1,ny
	do i=1,nx
