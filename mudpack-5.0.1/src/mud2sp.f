...
c
!$OMP PARALLEL DO SCHEDULE(STATIC,8) SHARED(rhs,cofx,cofy,phi,ist,ifn,jst,jfn) PRIVATE(i,j)
	do i=ist,ifn,2
	  do j=jst,jfn,2
	    phi(i,j) = (rhs(i,j) - (
...
!$OMP PARALLEL DO SCHEDULE(STATIC,8) SHARED(rhs,cofx,cofy,phi,ifn,jfn) PRIVATE(i,j)
	do i=2,ifn,2
	  do j=2,jfn,2
	    phi(i,j) = (rhs(i,j) -
     +                 (cofx(i,1)*phi(i-1,j)+cofx(i,2)*phi(i+1,j) +
...
c
!$OMP PARALLEL DO SCHEDULE(STATIC,8) SHARED(rhs,cofx,cofy,phi,ist,ifn,jfn) PRIVATE(i,j)
	do i=ist,ifn,2
	  do j=2,jfn,2
	    phi(i,j) = (rhs(i,j) -
     +                 (cofx(i,1)*phi(i-1,j)+cofx(i,2)*phi(i+1,j) +
...
!$OMP PARALLEL DO SCHEDULE(STATIC,8) SHARED(rhs,cofx,cofy,phi,ifn,jst,jfn) PRIVATE(i,j)
	do i=2,ifn,2
	  do j=jst,jfn,2
	    phi(i,j) = (rhs(i,j) -
     +                 (cofx(i,1)*phi(i-1,j)+cofx(i,2)*phi(i+1,j) +
