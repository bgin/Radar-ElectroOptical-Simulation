...
c ... purpose
c
c     cud3sp attempts to produce a second order finite difference
c     approximation to the 3-d complex SEPARABLE elliptic
...
c
c     cxx,cx,cex,cyy,cy,cey,czz,cz,cez,r,p are all complex
c     valued functions of the real independent variables x,y.
c
...
c
c     Nonseparable complex elliptic PDEs can be approximated with cud3
c
...
c     cudcom.f
c
      subroutine cud3sp(iparm,fparm,work,cfx,cfy,cfz,bndyc,rhs,phi,
     +                  mgopt,ierror)
      implicit none
     ...
     +kcycle,iprer,ipost,intpol
      common/icud3sp/intl,nxa,nxb,nyc,nyd,nze,nzf,ixp,jyq,kzr,iex,jey,
     +kez,nfx,nfy,nfz,iguess,maxcy,method,nwork,lwork,itero,
     +kcycle,iprer,ipost,intpol
      real xa,xb,yc,yd,ze,zf,tolmax,relmax
      common/fcud3sp/xa,xb,yc,yd,ze,zf,tolmax,relmax
      integer kpbgn,krbgn,kcxbgn,kcybgn,kczbgn,nxk,nyk,nzk,ngrid,
     +        klevel,kcur,kps
      common/cud3spc/kpbgn(50),krbgn(50),kcxbgn(50),kcybgn(50),
...
c
	do kb=1,ngrid
	  ...
	  icy = kcybgn(k)
	  icz = kczbgn(k)
	  call discd3sp(nx,ny,nz,work(icx),work(icy),work(icz),
	...
	return
      ...
      ny = nfy
      nz = nfz
      call cud3sp1(nx,ny,nz,rhs,phi,cfx,cfy,cfz,bndyc,work)
      iparm(22) = itero
      if (ierror .le. 0) then
...
c
      ...
      subroutine cud3sp1(nx,ny,nz,rhsf,phif,cfx,cfy,cfz,bndyc,wk)
      implicit none
      integer nx,ny,nz
      complex rhsf(nx,ny,nz),phif(nx,ny,nz),wk(*)
      common/icud3sp/intl,nxa,nxb,nyc,nyd,nze,nzf,ixp,jyq,kzr,iex,jey,
     +kez,nfx,nfy,nfz,iguess,maxcy,method,nwork,lwork,itero,
     +kcycle,iprer,ipost,intpol
      integer intl,nxa,nxb,nyc,nyd,nze,nzf,ixp,jyq,kzr,iex,jey,kez,
...
c
	do kb=1,ngrid
	  ...
	  ip = kpbgn(k)
	  ir = krbgn(k)
	  call adjcd3sp(nx,ny,nz,wk(ip),wk(ir),bndyc,cfx,cfy,cfz)
	end do
c
...
c
      call rescd3sp(nx,ny,nz,wk(ip),wk(ir),wk(icx),wk(icy),wk(icz),
...
c
	...
	call relcd3sp(wk)
      ...
      subroutine rescd3sp(nx,ny,nz,phi,rhs,cofx,cofy,cofz,ncx,ncy,ncz,
     +                    phic,rhsc,resf)
c
...
c
     ...
     +kcycle,iprer,ipost,intpol
      ...
      complex rhs(nx,ny,nz),rhsc(ncx,ncy,ncz),resf(nx,ny,nz)
...
c
!$OMP PARALLEL DO SCHEDULE(STATIC,8) PRIVATE(i,j,k), SHARED(phi,nx,ny,nz) &
...
c
      ...
      subroutine discd3sp(nx,ny,nz,cofx,cofy,cofz,bndyc,cfx,cfy,cfz,ier)
c
...
c
     ...
     +kcycle,iprer,ipost,intpol
      common/fcud3sp/xa,xb,yc,yd,ze,zf,tolmax,relmax
      real dlx,dly,dlz,dlx2,dly2,dlz2,dlxx,dlyy,dlzz,cemax,alfmax
      real x,y,z
      complex cxx,cyy,czz,cx,cy,cz,cex,cey,cez,alfa,gbdy,c1,c2
...
c     first order terms.  set flag at finest level if necessary
c
	if (cabs(cx)*dlx .gt. cabs(cxx+cxx)) then
	...
	if (cabs(cy)*dly .gt. cabs(cyy+cyy)) then
	  if (klevel.eq.ngrid) ier = -4
	...
	cemax = amax1(cabs(cez),cemax)
	if (cabs(cz)*dlz .gt. cabs(czz+czz)) then
	  if (klevel.eq.ngrid) ier = -4
...
c
	...
	if (nxa.eq.0.or.(nxa.eq.2.and.nxb.eq.2)) then
      ...
      subroutine adjcd3sp(nx,ny,nz,phi,rhs,bndyc,cfx,cfy,cfz)
c
...
c
      call relcd3spp(nx,ny,nz,wk(ip),wk(ir),wk(icx),wk(icy),wk(icz))
      return
      end

      subroutine relcd3spp(nx,ny,nz,phi,rhs,cofx,cofy,cofz)
c
...
!$OMP PARALLEL DO SCHEDULE(STATIC,8) SHARED(ist,ifn,jst,jfn,kst,kfn,phi,rhs,cofx,cofy,cofz) &
!$OMP PRIVATE(i,j,k)
      do k=kst,kfn,2
	do i=ist,ifn,2
	  do j=jst,jfn,2
...
c
!$OMP PARALLEL DO SCHEDULE(STATIC,8) SHARED(ist,ifn,jst,jfn,kfn,phi,rhs,cofx,cofy,cofz) &
!$OMP PRIVATE(i,j,k)
      do k=2,kfn,2
	do i=ist,ifn,2
	  do j=2,jfn,2
...
c
!$OMP PARALLEL DO SCHEDULE(STATIC,8) SHARED(ist,ifn,jfn,kst,kfn,phi,rhs,cofx,cofy,cofz) &
!$OMP PRIVATE(i,j,k)
      do k=kst,kfn,2
	do i=ist,ifn,2
	  do j=2,jfn,2
