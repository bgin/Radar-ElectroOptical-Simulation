...
c ... purpose
c
c     cud3 attempts to produce a second order finite difference
...
c
      subroutine cud3(iparm,fparm,work,coef,bndyc,rhs,phi,mgopt,ierror)
      implicit none
...
c
	do kb=1,ngrid
	  ...
	  ity = ktybgn(k)
	  itz = ktzbgn(k)
	  call discd3(nx,ny,nz,work(ic),work(itx),
...
c
	    do kkb=2,k
	      ...
	      if (method.eq.9) ny = nyk(k)
	      if (method.eq.10) nx = nxk(k)
	      call discd3(nx,ny,nz,work(ic),work(itx),work(ity),
	...
	return
      ...
      ny = nfy
      nz = nfz
      call cud31(nx,ny,nz,rhs,phi,coef,bndyc,work)
      iparm(23) = itero
      if (ierror .le. 0) then
...
c
      ...
      subroutine cud31(nx,ny,nz,rhsf,phif,coef,bndyc,wk)
      implicit none
      integer intl,nxa,nxb,nyc,nyd,nze,nzf,ixp,jyq,kzr,iex,jey,kez,
     +nfx,nfy,nfz,iguess,maxcy,method,meth2,nwork,lwork,itero,
     +kcycle,iprer,ipost,intpol
...
c
	do kb=1,ngrid
	  ...
	  ip = kpbgn(k)
	  ic = kcbgn(k)
	  call adjcd3(nx,ny,nz,wk(ip),wk(ic),bndyc,coef)
c
...
c
      call rescd3(nx,ny,nz,wk(ip),wk(ic),ncx,ncy,ncz,wk(ipc),wk(irc),
     +            wk(kps))
c
...
c
	if (klevel .gt. 1) then
	  ...
	  irc = kcbgn(klevel-1)+7*ncx*ncy*ncz
	  ipc = kpbgn(klevel-1)
	  call rescd3(nx,ny,nz,wk(ip),wk(ic),ncx,ncy,ncz,wk(ipc),
     +              wk(irc),wk(kps))
c
...
c
	...
	call relcd3(wk)
      ...
      subroutine rescd3(nx,ny,nz,phi,cof,ncx,ncy,ncz,phic,rhsc,resf)
c
...
c
      ...
      subroutine discd3(nx,ny,nz,cof,tx,ty,tz,bndyc,coef,wk,ier)
c
...
c
     ...
     +ktzbgn(50),nxk(50),nyk(50),nzk(50),ngrid,klevel,kcur,kps
      ...
      complex cxx,cyy,czz,cx,cy,cz,ce,alfa,gbdy,x,y,z,c1,c2,c3,c4,c5,c6
...
c
     ...
     +                  tz(1,1,1,4),tz(1,1,1,5),wk(kps))
      ...
      subroutine adjcd3(nx,ny,nz,phi,cof,bndyc,coef)
c
...
c
	call relcd3p(nx,ny,nz,wk(ip),wk(ic))
	return
      end if
      itx = ktxbgn(klevel)
      ity = ktybgn(klevel)
...
c
	call cplanyz(wk)
      ...
      subroutine relcd3p(nx,ny,nz,phi,cof)
c
