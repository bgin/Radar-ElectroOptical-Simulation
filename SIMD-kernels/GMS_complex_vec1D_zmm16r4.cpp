...
CV1DZMM16r4(const int32_t nsize) {
	using namespace gms::common;
        m_nsize = nsize;
	m_Re = (float*)gms_mm_malloc(static_cast<size_t>(m_nsize),64ULL);
	m_Im = (float*)gms_mm_malloc(static_cast<size_t>(m_nsize),64ULL);
}
...
gms::math::operator+(const CV1DZMM16r4 &x,
	...
	CV1DZMM16r4 ret_vec{x.m_nsize};
	int32_t i;
       
	for (i = 0; i != ROUND_TO_SIXTEEN(ret_vec.m_nsize, 16); i += 32)  {
...
#if (USE_AVX512COMPLEX_NT_STORES) == 1
		_mm512_stream_ps(&ret_vec.m_Im[i + 16], _mm512_add_ps(zmm6, zmm7));
#else
		_mm512_store_ps(&ret_vec.m_Im[i + 16], _mm512_add_ps(zmm6, zmm7));
#endif
		}	
...
gms::math::operator+(const CV1DZMM16r4 &x,
	...
	for (i = 0; i != ROUND_TO_SIXTEEN(ret_vec.m_nsize,16); i += 32) {
		...
		_mm512_stream_ps(&ret_vec.m_Re[i + 0], _mm512_add_ps(zmm0, zmm1));
#else
		_mm512_storeu_ps(&ret_vec.m_Re[i + 0], _mm512_add_ps(zmm0, zmm1));
#endif
		const __m512 zmm2 = _mm512_load_ps(&x.m_Re[i + 16]);
...
gms::math::operator-(const CV1DZMM16r4 &x,
	...
	for (i = 0; i != ROUND_TO_SIXTEEN(ret_vec.m_nsize,16); i += 32) {
		...
		const __m512 zmm7 = _mm512_load_ps(&y.m_Im[i + 16]);
#if (USE_AVX512COMPLEX_NT_STORES) == 1
		_mm512_stream_ps(&ret_vec.m_Im[i + 16], _mm512_sub_ps(zmm6, zmm7));
#else
		_mm512_store_ps(&ret_vec.m_Im[i + 16], _mm512_sub_ps(zmm6, zmm7));
#endif
	}
	for (; i != ret_vec.m_nsize; ++i) {
		ret_vec.m_Re[i] = x.m_Re[i] - y.m_Re[i];
...
gms::math::operator*(const CV1DZMM16r4 &x,
	...
	for (i = 0; i != ROUND_TO_SIXTEEN(ret_vec.m_nsize,16); i += 32) {
		...
		const __m512 zmm0 = _mm512_load_ps(&x.m_Re[i+0]);
		const __m512 zmm1 = _mm512_load_ps(&y.m_Re[i+0]);
		const __m512 zmm2 = _mm512_load_ps(&x.m_Im[i+0]);
		const __m512 zmm3 = _mm512_load_ps(&y.m_Im[i+0]);
#if (USE_AVX512COMPLEX_NT_STORES) == 1
...
#endif
		const __m512 zmm4 = _mm512_load_ps(&x.m_Re[i+16]);
		const __m512 zmm5 = _mm512_load_ps(&y.m_Re[i+16]);
		const __m512 zmm6 = _mm512_load_ps(&x.m_Im[i+16]);
		const __m512 zmm7 = _mm512_load_ps(&y.m_Im[i+16]);
#if (USE_AVX512COMPLEX_NT_STORES) == 1
...
gms::math::operator*(const CV1DZMM16r4 &x,
	...
	for (i = 0; i != ROUND_TO_SIXTEEN(ret_vec.m_nsize,16); i += 32) {
		...
		__m512 zmm0 = _mm512_load_ps(&x.m_Re[i + 0]);
...
#endif
		__m512 zmm2 = _mm512_load_ps(&x.m_Re[i + 16]);
...
gms::math::operator/(const CV1DZMM16r4 &x,
		     const CV1DZMM16r4 &y) {
	if (x.m_nsize != y.m_nsize) { return (CV1DZMM16r4{}); }
	CV1DZMM16r4 ret_vec{x.m_nsize}; 
	int32_t i;
	for (i = 0; i != ROUND_TO_SIXTEEN(ret_vec.m_nsize,16); i += 16) {
		...
		const __m512 zmm0 = _mm512_load_ps(&x.m_Re[i + 0]);
		const __m512 zmm1 = _mm512_load_ps(&y.m_Im[i + 0]);
		const __m512 zmm2 = _mm512_load_ps(&x.m_Im[i + 0]);
		...
		const __m512 zmm3 = _mm512_load_ps(&y.m_Re[i + 0]);
		const __m512 den_term = _mm512_add_ps(
			_mm512_mul_ps(zmm3, zmm3), _mm512_mul_ps(zmm1, zmm1));
#if (USE_AVX512COMPLEX_NT_STORES) == 1
		_mm512_stream_ps(&ret_vec.m_Re[i + 0], _mm512_div_ps(re_term1, den_term));
#else
		_mm512_store_ps(&ret_vec.m_Re[i + 0], _mm512_div_ps(re_term1, den_term));
#endif
#if (USE_AVX512COMPLEX_NT_STORES) == 1
		_mm512_stream_ps(&ret_vec.m_Im[i + 0], _mm512_div_ps(re_term2, den_term));
#else
		_mm512_store_ps(&ret_vec.m_Im[i + 0], _mm512_div_ps(re_term2, den_term));
#endif
