...
CV1DYMM4r8(const int32_t nsize){
	using namespace gms::common;
        data.m_nsize = nsize;
	data.m_Re = (double*)gms_mm_malloc(static_cast<size_t>(data.m_nsize),32ULL);
	data.m_Im = (double*)gms_mm_malloc(static_cast<size_t>(data.m_nsize),32ULL);
}	
...
gms::math::operator+(const CV1DYMM4r8 &x,
	...
	CV1DYMM4r8 ret_vec{x.data.m_nsize};
	int32_t i;
...
gms::math::operator-(const CV1DYMM4r8 &x,
		     const CV1DYMM4r8 &y) {
	if (x.data.m_nsize != y.data.m_nsize) { return (CV1DYMM4r8{}); }
	CV1DYMM4r8 ret_vec{x.data.m_nsize};
	int32_t i;
	for (i = 0; i != ROUND_TO_FOUR(ret_vec.data.m_nsize, 4); i += 8) {
		const __m256d ymm0(_mm256_load_pd(&x.data.m_Re[i+0]));
