...
#include <immintrin.h>
#include "GMS_q4x16_to_rmat9x16_looped_avx512.h"
#include "GMS_rotations_avx512_helpers.hpp"
                      ...
                      void
                      gms::math::q4x16_rm9x16_looped_u_nonunroll_zmm16r4(float * __restrict __ATTR_ALIGN__(64) row1,
...
#pragma code_align(32)
				   ...
				   _mm_prefetch((const char*)&q_w[i+16],_MM_HINT_T0);
				    const __m512 vqx = _mm512_loadu_ps(&q_x[i]);
				    const __m512 t0  = _mm512_mul_ps(vqx,vqx);
				    const __m512 vqy = _mm512_loadu_ps(&q_y[i]);
				    const __m512 vqw = _mm512_loadu_ps(&q_w[i]);
				   _mm_prefetch((const char*)&q_z[i+16],_MM_HINT_T0);
				    const __m512 vqz = _mm512_loadu_ps(&q_z[i]);
				    const __m512 t1  = _mm512_add_ps(_mm512_add_ps(vqx,vqy),vqw);
				    const __m512 t2  = _mm512_mul_ps(t1,t1);
				    const __m512 t3  = _mm512_sub_ps(t0,t2);
				   ...
				   _mm512_storeu_ps(&row9[i],_mm512_fmadd_ps(v16_2,_mm512_mul_ps(vqw,vqw),t3));
				    const __m512 t5  = _mm512_mul_ps(vqy,vqz);
				    const __m512 t6  = _mm512_mul_ps(vqz,vqw);
				    const __m512 t7  = _mm512_mul_ps(vqw,vqy);
				    const __m512 t8  = _mm512_mul_ps(vqx,vqw);
				   ...
				   _mm512_storeu_ps(&row7[i],_mm512_mul_ps(v16_2,_mm512_sub_ps(t7,t8)));
				    const __m512 t9  = _mm512_mul_ps(vqx,vqy);
				   ...
				   _mm512_storeu_ps(&row8[i],_mm512_mul_ps(v16_2,_mm512_add_ps(t6,t9)));
				    const __m512 t10 = _mm512_mul_ps(vqx,vqz);
				   _mm512_storeu_ps(&row3[i],_mm512_mul_ps(v16_2,_mm512_add_ps(t7,t10)));
				    const __m512 t4  = _mm512_mul_ps(vqy,vqy);
...
#pragma novector
					...
					row3[i] = 2.0F*(qy*qw+qx*qz);
                      ...
                      void
                      gms::math::q4x16_rm9x16_looped_a_nonunroll_zmm16r4(float * __restrict __ATTR_ALIGN__(64) row1,
		                                              float * __restrict __ATTR_ALIGN__(64) row2,
							      float * __restrict __ATTR_ALIGN__(64) row3,
							      float * __restrict __ATTR_ALIGN__(64) row4,
							      float * __restrict __ATTR_ALIGN__(64) row5,
...
#pragma code_align(32)
				   ...
				   _mm_prefetch((const char*)&q_y[i+16],_MM_HINT_T0);
				   _mm_prefetch((const char*)&q_w[i+16],_MM_HINT_T0);
				    const __m512 vqx = _mm512_load_ps(&q_x[i]);
				    const __m512 t0  = _mm512_mul_ps(vqx,vqx);
				    const __m512 vqy = _mm512_load_ps(&q_y[i]);
				    const __m512 vqw = _mm512_load_ps(&q_w[i]);
				   _mm_prefetch((const char*)&q_z[i+16],_MM_HINT_T0);
				    const __m512 vqz = _mm512_load_ps(&q_z[i]);
				    const __m512 t1  = _mm512_add_ps(_mm512_add_ps(vqx,vqy),vqw);
				    const __m512 t2  = _mm512_mul_ps(t1,t1);
				    const __m512 t3  = _mm512_sub_ps(t0,t2);
...
#pragma novector
					...
					row3[i] = 2.0F*(qy*qw+qx*qz);
                      ...
                      void
                      gms::math::q4x16_rm9x16_looped_u_unroll4x_zmm16r4(float * __restrict __ATTR_ALIGN__(64) row1,
		                                              float * __restrict __ATTR_ALIGN__(64) row2,
							      float * __restrict __ATTR_ALIGN__(64) row3,
							      float * __restrict __ATTR_ALIGN__(64) row4,
							      float * __restrict __ATTR_ALIGN__(64) row5,
...
#pragma code_align(32)
			      ...
			      for(i = 0; (i+63) < n; i += 64) {
				   ...
				   _mm_prefetch((const char*)&q_w[i+64],_MM_HINT_T0);
				    const __m512 vqx  = _mm512_loadu_ps(&q_x[i+0]);
				    const __m512 vqx2 = _mm512_loadu_ps(&q_x[i+16]);
				    const __m512 vqx3 = _mm512_loadu_ps(&q_x[i+32]);
				    const __m512 vqx4 = _mm512_loadu_ps(&q_x[i+48]);
				    const __m512 t0   = _mm512_mul_ps(vqx,vqx);
				    const __m512 t02  = _mm512_mul_ps(vqx2,vqx2);
				    const __m512 t03  = _mm512_mul_ps(vqx3,vqx3);
				    const __m512 t04  = _mm512_mul_ps(vqx4,vqx4);
				    const __m512 vqy  = _mm512_loadu_ps(&q_y[i+0]);
				    const __m512 vqy2 = _mm512_loadu_ps(&q_y[i+16]);
				    const __m512 vqy3 = _mm512_loadu_ps(&q_y[i+32]);
				    const __m512 vqy4 = _mm512_loadu_ps(&q_y[i+48]);
				    const __m512 vqw  = _mm512_loadu_ps(&q_w[i+0]);
				    const __m512 vqw2 = _mm512_loadu_ps(&q_w[i+16]);
				    const __m512 vqw3 = _mm512_loadu_ps(&q_w[i+32]);
				    const __m512 vqw4 = _mm512_loadu_ps(&q_w[i+48]);
				   _mm_prefetch((const char*)&q_z[i+32],_MM_HINT_T0);
				   _mm_prefetch((const char*)&q_z[i+64],_MM_HINT_T0);
				    const __m512 vqz  = _mm512_loadu_ps(&q_z[i+0]);
				    const __m512 vqz2 = _mm512_loadu_ps(&q_z[i+16]);
				    const __m512 vqz3 = _mm512_loadu_ps(&q_z[i+32]);
				    const __m512 vqz4 = _mm512_loadu_ps(&q_z[i+48]);
				    const __m512 t1   = _mm512_add_ps(_mm512_add_ps(vqx,vqy),vqw);
				    const __m512 t12  = _mm512_add_ps(_mm512_add_ps(vqx2,vqy2),vqw2);
				    const __m512 t13  = _mm512_add_ps(_mm512_add_ps(vqx3,vqy3),vqw3);
				    const __m512 t14  = _mm512_add_ps(_mm512_add_ps(vqx4,vqy4),vqw4);
				    const __m512 t2  = _mm512_mul_ps(t1,t1);
				    const __m512 t22  = _mm512_mul_ps(t12,t12);
				    const __m512 t23  = _mm512_mul_ps(t13,t13);
				    const __m512 t24  = _mm512_mul_ps(t14,t14);
				    const __m512 t3  = _mm512_sub_ps(t0,t2);
				    const __m512 t32  = _mm512_sub_ps(t02,t22);
				    const __m512 t33  = _mm512_sub_ps(t03,t23);
				    const __m512 t34  = _mm512_sub_ps(t04,t24);
				   ...
				   _mm512_storeu_ps(&row9[i+48],_mm512_fmadd_ps(v16_2,_mm512_mul_ps(vqw4,vqw4),t34));
				    const __m512 t5  = _mm512_mul_ps(vqy,vqz);
				    const __m512 t52 = _mm512_mul_ps(vqy2,vqz2);
				    const __m512 t53 = _mm512_mul_ps(vqy3,vqz3);
				    const __m512 t54 = _mm512_mul_ps(vqy4,vqz4);
				    const __m512 t6  = _mm512_mul_ps(vqz,vqw);
				    const __m512 t62  = _mm512_mul_ps(vqz2,vqw2);
				    const __m512 t63  = _mm512_mul_ps(vqz3,vqw3);
				    const __m512 t64  = _mm512_mul_ps(vqz4,vqw4);
				    const __m512 t7  = _mm512_mul_ps(vqw,vqy);
				    const __m512 t72  = _mm512_mul_ps(vqw2,vqy2);
				    const __m512 t73  = _mm512_mul_ps(vqw3,vqy3);
				    const __m512 t74  = _mm512_mul_ps(vqw4,vqy4);
				    const __m512 t8  = _mm512_mul_ps(vqx,vqw);
				    const __m512 t82  = _mm512_mul_ps(vqx2,vqw2);
				    const __m512 t83  = _mm512_mul_ps(vqx3,vqw3);
				    const __m512 t84  = _mm512_mul_ps(vqx4,vqw4);
				   ...
				   _mm512_storeu_ps(&row7[i+48],_mm512_mul_ps(v16_2,_mm512_sub_ps(t74,t84)));
				    const __m512 t9   = _mm512_mul_ps(vqx,vqy);
				    const __m512 t92  = _mm512_mul_ps(vqx2,vqy2);
				    const __m512 t93  = _mm512_mul_ps(vqx3,vqy3);
				    const __m512 t94  = _mm512_mul_ps(vqx4,vqy4);
				   ...
				   _mm512_storeu_ps(&row8[i+48],_mm512_mul_ps(v16_2,_mm512_add_ps(t64,t94)));
				    const __m512 t10  = _mm512_mul_ps(vqx,vqz);
				    const __m512 t102 = _mm512_mul_ps(vqx2,vqz2);
				    const __m512 t103 = _mm512_mul_ps(vqx3,vqz3);
				    const __m512 t104 = _mm512_mul_ps(vqx4,vqz4);
				   ...
				   _mm512_storeu_ps(&row3[i+48],_mm512_mul_ps(v16_2,_mm512_add_ps(t74,t104)));
				    const __m512 t4  = _mm512_mul_ps(vqy,vqy);
				    const __m512 t42 = _mm512_mul_ps(vqy2,vqy2);
				    const __m512 t43 = _mm512_mul_ps(vqy3,vqy3);
				    const __m512 t44 = _mm512_mul_ps(vqy4,vqy4);
				   ...
				   _mm_prefetch((const char*)&q_w[i+64],_MM_HINT_T0);
				    ...
				    const __m512 vqw4 = _mm512_loadu_ps(&q_w[i+48]);
				   _mm_prefetch((const char*)&q_z[i+64],_MM_HINT_T0);
				    const __m512 vqz4 = _mm512_loadu_ps(&q_z[i]);
				    const __m512 t14  = _mm512_add_ps(_mm512_add_ps(vqx4,vqy4),vqw4);
				    const __m512 t24  = _mm512_mul_ps(t14,t14);
				    const __m512 t34  = _mm512_sub_ps(t04,t24);
				   _mm512_storeu_ps(&row5[i+48],_mm512_fmadd_ps(v16_2,_mm512_mul_ps(vqz4,vqz4),t34));
...
#pragma novector
					...
					row3[i] = 2.0F*(qy*qw+qx*qz);
                      ...
                      void
                      gms::math::q4x16_rm9x16_looped_a_unroll4x_zmm16r4( float * __restrict __ATTR_ALIGN__(64) row1,
		                                              float * __restrict __ATTR_ALIGN__(64) row2,
							      float * __restrict __ATTR_ALIGN__(64) row3,
							      float * __restrict __ATTR_ALIGN__(64) row4,
							      float * __restrict __ATTR_ALIGN__(64) row5,
...
#pragma code_align(32)
			      ...
			      for(i = 0; (i+63) < n; i += 64) {
				   ...
				   _mm_prefetch((const char*)&q_y[i+64],_MM_HINT_T0);
				   _mm_prefetch((const char*)&q_w[i+64],_MM_HINT_T0);
				    const __m512 vqx  = _mm512_load_ps(&q_x[i+0]);
				    const __m512 vqx2 = _mm512_load_ps(&q_x[i+16]);
				    const __m512 vqx3 = _mm512_load_ps(&q_x[i+32]);
				    const __m512 vqx4 = _mm512_load_ps(&q_x[i+48]);
				    const __m512 t0   = _mm512_mul_ps(vqx,vqx);
				    const __m512 t02  = _mm512_mul_ps(vqx2,vqx2);
				    const __m512 t03  = _mm512_mul_ps(vqx3,vqx3);
				    const __m512 t04  = _mm512_mul_ps(vqx4,vqx4);
				    const __m512 vqy  = _mm512_load_ps(&q_y[i+0]);
				    const __m512 vqy2 = _mm512_load_ps(&q_y[i+16]);
				    const __m512 vqy3 = _mm512_load_ps(&q_y[i+32]);
				    const __m512 vqy4 = _mm512_load_ps(&q_y[i+48]);
				    const __m512 vqw  = _mm512_load_ps(&q_w[i+0]);
				    const __m512 vqw2 = _mm512_load_ps(&q_w[i+16]);
				    const __m512 vqw3 = _mm512_load_ps(&q_w[i+32]);
				    const __m512 vqw4 = _mm512_load_ps(&q_w[i+48]);
				   _mm_prefetch((const char*)&q_z[i+32],_MM_HINT_T0);
				   _mm_prefetch((const char*)&q_z[i+64],_MM_HINT_T0);
				    const __m512 vqz  = _mm512_load_ps(&q_z[i+0]);
				    const __m512 vqz2 = _mm512_load_ps(&q_z[i+16]);
				    const __m512 vqz3 = _mm512_load_ps(&q_z[i+32]);
				    const __m512 vqz4 = _mm512_load_ps(&q_z[i+48]);
				    const __m512 t1   = _mm512_add_ps(_mm512_add_ps(vqx,vqy),vqw);
				    const __m512 t12  = _mm512_add_ps(_mm512_add_ps(vqx2,vqy2),vqw2);
				    const __m512 t13  = _mm512_add_ps(_mm512_add_ps(vqx3,vqy3),vqw3);
				    const __m512 t14  = _mm512_add_ps(_mm512_add_ps(vqx4,vqy4),vqw4);
				   ...
				   _mm_prefetch((const char*)&q_w[i+64],_MM_HINT_T0);
				    ...
				    const __m512 vqw4 = _mm512_load_ps(&q_w[i+48]);
				   _mm_prefetch((const char*)&q_z[i+64],_MM_HINT_T0);
				    const __m512 vqz4 = _mm512_load_ps(&q_z[i]);
				    const __m512 t14  = _mm512_add_ps(_mm512_add_ps(vqx4,vqy4),vqw4);
				    const __m512 t24  = _mm512_mul_ps(t14,t14);
				    const __m512 t34  = _mm512_sub_ps(t04,t24);
				   _mm512_storeu_ps(&row5[i+48],_mm512_fmadd_ps(v16_2,_mm512_mul_ps(vqz4,vqz4),t34));
