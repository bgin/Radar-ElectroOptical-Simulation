...
#ifndef __GMS_TREE_SCATTERER_H__
#define __GMS_TREE_SCATTERER_H__


namespace file_info {

      const unsigned int gGMS_TREE_SCATTERER_MAJOR = 1;
      const unsigned int gGMS_TREE_SCATTERER_MINOR = 0;
      const unsigned int gGMS_TREE_SCATTERER_MICRO = 0;
      const unsigned int gGMS_TREE_SCATTERER_FULLVER = 1000U*gGMS_TREE_SCATTERER_MAJOR+100U*gGMS_TREE_SCATTERER_MINOR+
                                                       10U*gGMS_TREE_SCATTERER_MICRO;
      const char * const pgGMS_TREE_SCATTERER_CREATION_DATE = "24-12-2019 13:52 +00200 (TUE 24 DEC 2019 GMT+2)";
      const char * const pgGMS_TREE_SCATTERER_BUILD_DATE    = __DATE__ " " __TIME__;
      const char * const pgGMS_TREE_SCATTERER_AUTHOR        = " Programmer: Bernard Gingold, contact: beniekg@gmail.com";
      const char * const pgGMS_TREE_SCATTERER_SYNOPSYS      = " Model of single tree scatterer suitable for computation of radar backscatter.";
...
namespace  gms {
     ...
     namespace  math {
	     ...
	     // Low temporal access and spatial locality (cold)  data structure (POD) type
			 ...
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) trunk_xparam;
			 //   ! PAOS type size of array -- npoints/8
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) trunk_yparam;
			 //    ! PAOS type size of array -- npoints/8
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) trunk_zparam;
			 //    ! Leaves thicknes (micron) per leaf
                         //    ! PAOS type size of array nleaves/8
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) leaves_thick;
			 //    ! Leaves density (g/cm^3) per leaf
                         //    ! PAOS type size of array nleaves/8
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) leaves_dens;
			 //    ! Leaves surface angle to inpinging Radar waveform (rad)
                         //    ! PAOS type size of array nleaves/8
			 //     (theta,phi angles)
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) leaves_incang;
			 ...
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) leaves_xparam;
			 //   ! Parameter y, (b*sin(t))
                         //   ! PAOS type size of arrays is -- npoints/8 1st dim (evaluation of y) ,
                         //   ! nleaves/8 2nd dim (number of leaves)
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) leaves_yparam;
			 //   Branchess thickness
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) branches_thick;
			 //   Branches density
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) branches_dens;
			 //   Branches incident angle
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) branches_incang;
			 ...
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) branches_xparam;
			 //  ! Parameter y, (r*sin(t))
                         //  !2nd dimension is a branches number, 1st dimension evaluation of parameter y
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) branches_yparam;
			 //  ! Parameter z, (height)
                         //  !2nd dimension is a branch  number, 1st dimension evaluation of parameter z
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) branches_zparam;
		} __ATTR_ALIGN__(64);
		...
		struct TreeScattererHot_t {
			 ...
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) branches_rcs;
			 //   ! Branches reflectivity (varying due to leaves vibration)
                         //     ! ( 1st dimension reflectivity(dbm) pers branch, 2nd dimension is PRF/s))
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) branches_reflect;
			 //   ! Leaves angle of vibration in x-axis per PRF/s
                         //   ! 1st dimension angle values (rad),  2nd dimension PRF/s,
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) leaves_xang;
		         //   ! Leaves sine of vibration angle in x-axis per PRF/s
                         //   ! 1st  dimension sine of vibrational angle (rad),  2nd dimension PRF/s,
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) leaves_sin_xang;
			 //     ! Leaves sine of vibration angle in x-axis per PRF/s
                         //     ! 1st dimension PRF/s, 2nd dimension sine of vibrational angle (rad)
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) leaves_cos_xang;
			 //      ! Leaves angle of vibration in y-axis per PRF/s
                         //      ! 1st dimension PRF/s, 2nd dimension angle values (rad)
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) leaves_yang;
			 //      ! Leaves sine of vibration angle in y-axis per PRF/s
                         //      ! 1st dimension PRF/s, 2nd dimension angle of vibrational angle (rad)
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) leaves_sin_yang;
			 //        ! Leaves sine of vibration angle in y-axis per PRF/s
                         //        ! 1st dimension PRF/s, 2nd dimension sine of vibrational angle (rad)
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) leaves_cos_yang;
			 // ! Branches angle of vibration in x-axis per PRF/s
                         // ! 1st dimension PRF/s, 2nd dimension angle values (rad)
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) branches_xang;
			 //  ! Branches sine of vibration angle in x-axis per PRF/s
                         //  ! 1st dimension PRF/s, 2nd dimension angle  of vibrational angle (rad)
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) branches_sin_xang;
			 //   ! Branches cosine of vibration angle in x-axis per PRF/s
                         //   ! 1st dimension PRF/s, 2nd dimension cosine of vibrational angle (rad)
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) branches_cos_xang;
			 //   
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) branches_yang;
			 //  ! Branches sine of vibration angle in y-axis per PRF/s
                         //  ! 1st dimension PRF/s, 2nd dimension sine  of vibrational angle (rad)
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) branches_sin_yang;
			 //  ! Branches cosine of vibration angle in x-axis per PRF/s
                         //   ! 1st dimension PRF/s, 2nd dimension cosine  of vibrational angle (rad)
			 AVXVec8 * __restrict __ATTR_ALIGN__(64) branches_cos_yang;
		} __ATTR_ALIGN__(64);


		struct TreeScatterer {


		       TreeScattererCold_t m_tsc __ATTR_ALIGN__(64);

		       TreeScattererHot_t  m_tsh __ATTR_ALIGN__(64);


		       TreeScatterer() __ATTR_COLD__ __ATTR_ALIGN__(32);

		       TreeScatterer(const int32_t,
		                     const int32_t,
				     const int32_t,
				     const int32_t,
				     const int32_t,
			...
			TreeScatterer(const TreeScatterer &) = delete;

			TreeScatterer(TreeScatterer &&) = delete;

			~TreeScatterer() noexcept(true);

			TreeScatterer & operator=(const TreeScatterer &) = delete;

			TreeScatterer & operator=(TreeScatterer &&) = delete;
