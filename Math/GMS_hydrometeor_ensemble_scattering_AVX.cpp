...
gms::math::HMScatterersAVX::
HMScatterersAVX(const int32_t np,
                      const int32_t shpts,
                      const int32_t ID,
		      const int32_t nt,
		      const int32_t nxpts,
...
#pragma code_align(32)
      ...
      return (true);
       ...
       float def;
       float rad;
       const int64_t totpts = static_cast<int64_t>(8*m_hsc.m_np*m_hsc.m_nshpts);
...
#pragma omp simd aligned(radii,pcs:64)
       ...
       // Memory first-touch
        gms::common::avxvec8_init_unroll8x(&m_hsc.m_pcs[0],
                                         static_cast<int64_t>(m_hsc.m_np*m_hsc.m_nshpts),
					 ZERO);
        gms::common::avxvec8_init_unroll8x(&m_hsc.m_radii[0],
                                           static_cast<int64_t>(m_hsc.m_np),
					   ZERO);
	 // Copy results back to member arrays.
	gms::common::avxvec8_copy_from_r4(&m_hsc.m_radii[0],
	                                  &radii[0],
					  npts);
	gms::common::avxvec8_copy_from_r4(&m_hsc.m_pcs[0],
	                                  &pcs[0],
					  totpts);
...
ComputeEnsembleShape( const float inz,
     ...
     switch(type) {
          ...
          case EnsembleShapesAVX::Cylindrical : {
               z = inz;
               theta = inth;
#if defined __ICC || defined __INTEL_COMPILER
...
#pragma code_align(32)
	  ...
	  case EnsembleShapesAVX::Spheroidal : {
               theta = inth;
	       phi   = inphi;
#if defined __ICC || defined __INTEL_COMPILER
               __assume_aligned(m_hsc.m_pes,64);
...
#pragma code_align(32)
        ...
        case EnsembleShapesAVX::Chebyshev : {
             theta = inth;
	     phi = inphi;
#if defined __ICC || defined __INTEL_COMPILER
               __assume_aligned(m_hsc.m_pes,64);
...
void
gms::math::HMScatterersAVX::
ComputeHydroMeteorScattering(int32_t vtune_analysis,
                             int32_t pmc_loop744,
			     int32_t pmc_loop965,
			     int32_t pmc_loop1097,
			     int32_t pmc_loop1179,
			     int32_t pmc_loop1778,
			     int32_t pmc_loop1828,
			     int32_t pmc_loop2039,
			     int32_t pmc_loop2092,
			     int32_t pmc_loop2323,
			     int32_t idMie,
			     double  small,
			     int32_t MXINT,
			     int32_t NADD,
            ...
            mod_tmatrix_mps_mp_tmatrix_mps_driver_(&vtune_analysis,
	                                          &pmc_loop744,
						  &pmc_loop965,
						  &pmc_loop1097,
						  &pmc_loop1179,
            ...
            __mod_tmatrix_mps_MOD_tmatrix_mps_driver(&vtune_analysis,
	                                          &pmc_loop744,
						  &pmc_loop965,
						  &pmc_loop1097,
						  &pmc_loop1179,
