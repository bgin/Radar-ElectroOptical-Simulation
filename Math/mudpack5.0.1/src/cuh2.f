...
c ... purpose
c
c     cuh2 attempts to produce a second order finite difference
c     approximation to the complex two dimensional nonseparable elliptic
...
c     cudcom.f
c
      subroutine cuh2(iparm,fparm,wk,iwk,coef,bndyc,rhs,phi,mgopt,
     +                ierror)
      implicit none
     ...
     +             kcycle,iprer,ipost,intpol,kps
      ...
      common/cud2c/kpbgn(50),kcbgn(50),ktxbgn(50),ktybgn(50),
     +nxk(50),nyk(50),isx,jsy
      integer ibeta,ialfa,izmat,idmat
      common/cuh2c/ibeta,ialfa,izmat,idmat
      external coef,bndyc
      data int / 0 /
      save int
      ierror = 1
...
c
	...
	do kb=1,ngrid
	  ...
	  ity = ktybgn(k)
	  klevel = k
	  call disch2(nx,ny,wk(ic),wk(itx),wk(ity),
	...
	return
      ...
      nx = nfx
      ny = nfy
      call cuh21(nx,ny,rhs,phi,coef,bndyc,wk,iwk)
c
...
c
	...
	if (relmax.gt.tolmax) ierror = -1   ! flag convergenc failure
      ...
      subroutine cuh21(nx,ny,rhsf,phif,coef,bndyc,wk,iwk)
      implicit none
      integer nx,ny,iwk(*)
...
c
	do k=1,ngrid
	  ...
	  ip = kpbgn(k)
	  ic = kcbgn(k)
	  call adjch2(nx,ny,wk(ip),wk(ic),bndyc,coef)
	end do
c
...
c
	if (nyc .ne. 0) then
	  call cdir2(nx,ny,wk(ip),wk(ic),wk(ibeta),wk(ialfa),iwk)
	  return
	else
	  call cdir2p(nx,ny,wk(ip),wk(ic),wk(ibeta),wk(ialfa),wk(izmat),
     +                wk(idmat),iwk)
	  return
	end if
      end if
...
c
      do l=1,iprer
	call relch2(nx,ny,wk(ip),wk(ic),wk(itx),wk(ity),wk(kps))
      end do
c
...
c
      ...
      call resch2(nx,ny,wk(ip),ncx,ncy,wk(ipc),wk(irc),wk(ic),wk(kps))
c
...
c
	  ...
	  ncy = nyk(klevel-1)
	  irc = kcbgn(klevel-1)+5*ncx*ncy
	  call resch2(nx,ny,wk(ip),ncx,ncy,wk(ipc),wk(irc),wk(ic),
     +                wk(kps))
c
...
c
	  ...
	  if (nyc .ne. 0) then
	    ...
	    call cdir2p(nx,ny,wk(ip),wk(ic),wk(ibeta),wk(ialfa),
     +                  wk(izmat),wk(idmat),iwk)
	  end if
	  ipc = ip
	  ip = kpbgn(2)
	  ncx = nxk(1)
...
c
	...
	call relch2(nx,ny,wk(ip),wk(ic),wk(itx),wk(ity),wk(kps))
      ...
      subroutine disch2(nx,ny,cof,tx,ty,bndyc,coef,wk,iwk,ier)
c
c     discretize elliptic pde for cuh2, set nonfatal errors
c
      implicit none
      integer intl,nxa,nxb,nyc,nyd,ixp,jyq,iex,jey,nfx,nfy,iguess,
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
      ...
      complex cof(nx,ny,6),tx(nx,ny,*),ty(ny,nx,*),wk(*)
      real x,y,dlx,dlx2,dlxx,dly,dly2,dlyy,cmin,alfmax,cemax
      complex cxx,cyy,cx,cy,ce,c1,c2,c3,c4,c5,alfa,gbdy
...
c
	...
	if (cemax.eq.0.0.and.alfmax.eq.0) then
...
c     factor non-periodic block matrix
	  call clud2(nx,ny,cof,wk(ibeta),wk(ialfa),iwk,nxa)
	  return
	else
c     factor periodic block matrix

	  do j =1,ny-1
	    call csetbeta(nx,ny,cof,wk(ibeta),j,nxa)
	    call csetalfa(nx,ny,cof,wk(ialfa),j)
	  end do
	  call clud2p(nx,ny,cof,wk(ibeta),wk(ialfa),wk(izmat),wk(idmat),
     +               iwk,nxa)
	  return
	end if
      end if
...
c
     ...
     +                   ty(1,1,5),wk(kps))
      ...
      subroutine clud2(nx,ny,cof,beta,alfa,index,nxa)
c
c     decompose complex nonperiodic block coefficient matrix
c
      implicit none
      integer nx,ny,nxa
      complex cof(nx,ny,6),beta(nx,nx,*),alfa(nx,nx,*)
      integer index(nx,ny)
...
c
      jcur = 1
      call csetbeta(nx,ny,cof,beta,jcur,nxa)
      call csgfa(beta,nx,nx,index,iz)
      do jcur=2,ny
	call csetalfa(nx,ny,cof,alfa,jcur)
	call ctransp(nx,alfa(1,1,jcur))
	jm1 = jcur-1
	do l=1,nx
	  call csgsl(beta(1,1,jm1),nx,nx,index(1,jm1),alfa(1,l,jcur),i1)
	end do
	call ctransp(nx,alfa(1,1,jcur))
	call csetbeta(nx,ny,cof,beta,jcur,nxa)
	do i=1,nx
...
c
	call csgfa(beta(1,1,jcur),nx,nx,index(1,jcur),iz)
      end do
      return
      end

      subroutine cdir2(nx,ny,phi,cof,beta,alfa,index)
c
c     direct solve at coarsest grid
c
      implicit none
      integer nx,ny,index(nx,ny)
      complex phi(0:nx+1,0:ny+1),cof(nx,ny,6)
      complex beta(nx,nx,*),alfa(nx,nx,*)
c     forward sweep
      call cfor2(nx,ny,phi,cof(1,1,6),alfa)
c     backward sweep
      call cbkw2(nx,ny,phi,cof,beta,index)
      return
      end

      subroutine cfor2(nx,ny,phi,frhs,alfa)
c
c     forward sweep
c
      implicit none
      integer nx,ny,i,j,l
      complex phi(0:nx+1,0:ny+1),frhs(nx,ny),alfa(nx,nx,*),sum
      do j=1,ny
	do i=1,nx
	  phi(i,j)=frhs(i,j)
	end do
	...
	do i=1,nx
      ...
      subroutine cbkw2(nx,ny,phi,cof,beta,index)
c
c     backward sweep
c
      implicit none
      integer nx,ny,index(nx,ny)
      complex phi(0:nx+1,0:ny+1),cof(nx,ny,6),beta(nx,nx,*)
      integer iz,j,jb,i
      iz = 0
      call csgsl(beta(1,1,ny),nx,nx,index(1,ny),phi(1,ny),iz)
      do jb=2,ny
	j = ny-jb+1
	do i=1,nx
	  phi(i,j) = phi(i,j) - cof(i,j,4)*phi(i,j+1)
	end do
	call csgsl(beta(1,1,j),nx ,nx ,index(1,j),phi(1,j),iz)
      end do
      return                                                                    
      end                                                                       

      subroutine clud2p(nx,ny,cof,beta,alfa,zmat,dmat,index,nxa)
c
c     lud decomposition for complex periodic
c
      implicit none
      integer nx,ny,index(nx,ny),i,iz,j,l,jcur,jm1,i1,nxa
      complex cof(nx,ny,6),alfa(nx,nx,*),beta(nx,nx,*)
      complex dmat(nx,nx,*),zmat(nx,nx,*),sum
      jcur = 1
c
c     set dmat(1)=alfa(1)
c
...
c
      call csetbeta(nx,ny,cof,beta,jcur,nxa)
      call csgfa(beta(1,1,1),nx,nx,index(1,1),iz)
      do jcur=2,ny-2
c
...
c
	...
	zmat(i,i,1) = cof(i,ny-1,4)
      ...
      do l=1,nx
	call csgsl(beta(1,1,1),nx,nx,index(1,1),zmat(1,l,1),i1)
      end do
      call ctransp(nx,zmat(1,1,1))
c
...
c
	...
	do l=1,nx
	 call csgsl(beta(1,1,jcur),nx,nx,index(1,jcur),zmat(1,l,jcur),
     +              i1)
	end do
	call ctransp(nx,zmat(1,1,jcur))
      end do
c
c     solve transpose of zmat(ny-2)umat(ny-2)=alfa(ny-1)-zmat(ny-3)gama(ny-3)
c
      call csetalfa(nx,ny,cof,alfa,ny-1)
      do i=1,nx
	do j=1,nx
      ...
      do l=1,nx
	call csgsl(beta(1,1,ny-2),nx,nx,index(1,ny-2),zmat(1,l,ny-2),i1)
      end do
      call ctransp(nx,zmat(1,1,ny-2))
c
c     set umat(ny-1) = beta(ny-1)-(zmat(1)*dmat(1)+...+zmat(ny-2)*dmat(ny-2))
c     in beta(ny-1)
c
      call csetbeta(nx,ny,cof,beta,ny-1,nxa)
      do  i=1,nx
	do j=1,nx
	  sum = (0.0,0.0)
	  do jcur=1,ny-2
...
c
      call csgfa(beta(1,1,ny-1),nx,nx,index(1,ny-1),iz)
c
...
c
      ...
      subroutine cdir2p(nx,ny,phi,cof,beta,alfa,zmat,dmat,index)
c
c     complex direct method for periodic b.c.
c
      implicit none
      integer nx,ny,index(nx,ny)
      complex phi(0:nx+1,0:ny+1),cof(nx,ny,6)
      complex beta(nx,nx,*),alfa(nx,nx,*)
      complex zmat(nx,nx,*), dmat(nx,nx,*)
c     forward sweep
      call cfor2p(nx,ny,phi,cof(1,1,6),alfa,zmat)
c     backward sweep
      call cbkw2p(nx,ny,phi,cof,beta,dmat,index)
      return
      end

      subroutine cfor2p(nx,ny,phi,frhs,alfa,zmat)
c
c     forward sweep for complex periodic
c
      implicit none
      integer nx,ny,i,j,jcur,l,k
      complex phi(0:nx+1,0:ny+1),frhs(nx,ny),sum
      complex alfa(nx,nx,*),zmat(nx,nx,*)
      do j=1,ny-1
	do i=1,nx
	  phi(i,j)=frhs(i,j)
	end do
...
c
	...
	phi(i,ny-1) = phi(i,ny-1) - sum
      ...
      subroutine cbkw2p(nx,ny,phi,cof,beta,dmat,index)
c
c     backward sweep for complex periodic
c
      implicit none
      integer nx,ny,index(nx,ny),iz,i,l,kb,k
      complex phi(0:nx+1,0:ny+1),cof(nx,ny,6),beta(nx,nx,ny)
      complex dmat(nx,nx,*)
      complex sum
      iz = 0
      call csgsl(beta(1,1,ny-1),nx,nx,index(1,ny-1),phi(1,ny-1),iz)
c
...
c
	...
	phi(i,ny-2) = phi(i,ny-2) - sum
      ...
      call csgsl(beta(1,1,ny-2),nx,nx,index(1,ny-2),phi(1,ny-2),iz)
c
...
c
	...
	call csgsl(beta(1,1,k),nx,nx,index(1,k),phi(1,k),iz)
      end do
c
...
c
	...
	phi(i,ny) = phi(i,1)
      ...
      subroutine csetbeta(nx,ny,cof,beta,jcur,nxa)
c
c     set complex diagonal matrix on block
c
      implicit none
      integer nx,ny,jcur,nxa,i,l,j
      complex cof(nx,ny,6),beta(nx,nx,*)
      j = jcur
      do i=1,nx
	do l=1,nx
	  beta(i,l,jcur)=(0.0,0.0)
	end do
      end do
      do i=1,nx
	...
	beta(nx,2,jcur) = cof(nx,j,2)
      ...
      subroutine csetalfa(nx,ny,cof,alfa,jcur)
      implicit none
      integer nx,ny,jcur,i,l
      complex cof(nx,ny,6),alfa(nx,nx,*)
      do i=1,nx
	do l=1,nx
	  alfa(i,l,jcur)=(0.0,0.0)
	end do
	alfa(i,i,jcur)=cof(i,jcur,3)
      ...
      subroutine adjch2(nx,ny,phi,cof,bndyc,coef)
c
...
c
     ...
     +             kcycle,iprer,ipost,intpol,kps
      ...
      complex cxx,cyy,cx,cy,ce,c1,c2,c3,c4,alfa,gbdy
      common/icud2/intl,nxa,nxb,nyc,nyd,ixp,jyq,iex,jey,nfx,nfy,iguess,
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
      common/fcud2/xa,xb,yc,yd,tolmax,relmax
...
c
	...
	do i=1,nx
      ...
      subroutine resch2(nx,ny,phi,ncx,ncy,phic,rhsc,cof,resf)
c
...
c
C$OMP PARALLEL DO SHARED(resf,cof,phi,nx,ny) PRIVATE(i,j)
      do j=1,ny
	do i=1,nx
	  resf(i,j) =  cof(i,j,6)-(
     +             cof(i,j,1)*phi(i-1,j)+
...
c
      ...
      subroutine relch2(nx,ny,phi,cof,tx,ty,sum)
c
c     relaxation for cuh2
c
      implicit none
      integer nx,ny
	...
	call relch2p(nx,ny,phi,cof)
      else if (method.eq.1) then           ! line x relaxation
	call slxch2(nx,ny,phi,cof,tx,sum)
      else if (method.eq.2) then           ! line y relaxation
	call slych2(nx,ny,phi,cof,ty,sum)
	...
	call slych2(nx,ny,phi,cof,ty,sum)
      ...
      subroutine relch2p(nx,ny,phi,cof)
c
...
c
C$OMP PARALLEL DO SHARED(cof,phi,nx,ny) PRIVATE(i,j)
	do i=1,nx,2
	  do j=1,ny,2
	    phi(i,j) = (cof(i,j,6) -
...
c
	...
	do i=1,nx
      ...
      subroutine slxch2(nx,ny,phi,cof,tx,sum)
c
...
c
     ...
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
      complex phi(0:nx+1,0:ny+1),cof(nx,ny,6),tx(nx,ny,*),sum(ny)
c
...
c
C$OMP PARALLEL DO SHARED(cof,phi,tx,nx,ny) PRIVATE(i,ib,j)
	do j=1,ny,2
	  do i=1,nx
...
c
c     forward sweep thru odd j lines
c
	  do i=2,nx
...
c
C$OMP PARALLEL DO SHARED(sum,cof,phi,tx,nx,ny) PRIVATE(i,j,ib)
	do j=1,ny,2
	  do i=1,nx-1
	    phi(i,j)=cof(i,j,6)-cof(i,j,3)*phi(i,j-1)-cof(i,j,4)*
     +               phi(i,j+1)
...
c
	...
	do i=1,nx
      ...
      subroutine slych2(nx,ny,phi,cof,ty,sum)
      implicit none
     ...
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
      complex phi(0:nx+1,0:ny+1),cof(nx,ny,6),ty(ny,nx,*),sum(nx)
c
...
c
C$OMP PARALLEL DO SHARED(cof,phi,ty,nx,ny) PRIVATE(i,j,jb)
	do i=1,nx,2
	  do j=1,ny
...
c
C$OMP PARALLEL DO SHARED(sum,cof,phi,ty,nx,ny) PRIVATE(i,j,jb)
	do i=1,nx,2
	  do j=1,ny-1
	    phi(i,j)=cof(i,j,6)-cof(i,j,1)*phi(i-1,j)-cof(i,j,2)*
     +               phi(i+1,j)
