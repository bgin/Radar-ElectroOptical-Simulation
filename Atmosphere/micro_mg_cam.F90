...
#if (_OPENMP) > 201500
#define USE_OMP_ALLOCATION 1
#else
#define USE_OMP_ALLOCATION 0
#endif
        ...
        SUBROUTINE micro_mg_cam_tend(dtime, kgen_unit)
            ...
            REAL(KIND=r8), allocatable :: packed_rate1ord_cw2pr_st(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_rate1ord_cw2pr_st(:,:)
            REAL(KIND=r8), allocatable :: packed_tlat(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_tlat(:,:)
            REAL(KIND=r8), allocatable :: packed_qvlat(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_qvlat(:,:)
            REAL(KIND=r8), allocatable :: packed_qctend(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_qctend(:,:)
            REAL(KIND=r8), allocatable :: packed_qitend(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_qitend(:,:)
            REAL(KIND=r8), allocatable :: packed_nctend(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_nctend(:,:)
            REAL(KIND=r8), allocatable  :: packed_nitend(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_nitend(:,:)
            REAL(KIND=r8), allocatable  :: packed_qrtend(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_qrtend(:,:)
            REAL(KIND=r8), allocatable  :: packed_qstend(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_qstend(:,:)
            REAL(KIND=r8), allocatable  :: packed_nrtend(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_nrtend(:,:)
            REAL(KIND=r8), allocatable :: packed_nstend(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_nstend(:,:)
            REAL(KIND=r8), allocatable :: packed_prect(:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_prect(:)
            REAL(KIND=r8), allocatable :: packed_preci(:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_preci(:)
            REAL(KIND=r8), allocatable :: packed_nevapr(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_nevapr(:,:)
            REAL(KIND=r8), allocatable :: packed_evapsnow(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_evapsnow(:,:)
            REAL(KIND=r8), allocatable :: packed_prain(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_prain(:,:)
            REAL(KIND=r8), allocatable :: packed_prodsnow(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_prodsnow(:,:)
            REAL(KIND=r8), allocatable :: packed_cmeout(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_cmeout(:,:)
            REAL(KIND=r8), allocatable :: packed_qsout(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_qsout(:,:)
            REAL(KIND=r8), allocatable :: packed_rflx(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_rflx(:,:)
            REAL(KIND=r8), allocatable :: packed_sflx(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_sflx(:,:)
            REAL(KIND=r8), allocatable :: packed_qrout(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_qrout(:,:)
            REAL(KIND=r8), allocatable :: packed_qcsevap(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_qcsevap(:,:)
            REAL(KIND=r8), allocatable :: packed_qisevap(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_qisevap(:,:)
            REAL(KIND=r8), allocatable :: packed_qvres(:,:)
           ! REAL(KIND=r8), allocatable, target :: ref_packed_qvres(:,:)
            REAL(KIND=r8), allocatable :: packed_cmei(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_cmei(:,:)
            REAL(KIND=r8), allocatable :: packed_vtrmc(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_vtrmc(:,:)
            REAL(KIND=r8), allocatable :: packed_vtrmi(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_vtrmi(:,:)
            REAL(KIND=r8), allocatable :: packed_qcsedten(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_qcsedten(:,:)
            REAL(KIND=r8), allocatable :: packed_qisedten(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_qisedten(:,:)
            REAL(KIND=r8), allocatable :: packed_qrsedten(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_qrsedten(:,:)
            REAL(KIND=r8), allocatable :: packed_qssedten(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_qssedten(:,:)
            REAL(KIND=r8), allocatable :: packed_umr(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_umr(:,:)
            REAL(KIND=r8), allocatable :: packed_ums(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_ums(:,:)
            REAL(KIND=r8), allocatable :: packed_pra(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_pra(:,:)
            REAL(KIND=r8), allocatable :: packed_prc(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_prc(:,:)
            REAL(KIND=r8), allocatable :: packed_mnuccc(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_mnuccc(:,:)
            REAL(KIND=r8), allocatable :: packed_mnucct(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_mnucct(:,:)
            REAL(KIND=r8), allocatable :: packed_msacwi(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_msacwi(:,:)
            REAL(KIND=r8), allocatable :: packed_psacws(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_psacws(:,:)
            REAL(KIND=r8), allocatable :: packed_bergs(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_bergs(:,:)
            REAL(KIND=r8), allocatable :: packed_berg(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_berg(:,:)
            REAL(KIND=r8), allocatable :: packed_melt(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_melt(:,:)
            REAL(KIND=r8), allocatable :: packed_homo(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_homo(:,:)
            REAL(KIND=r8), allocatable :: packed_qcres(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_qcres(:,:)
            REAL(KIND=r8), allocatable :: packed_prci(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_prci(:,:)
            REAL(KIND=r8), allocatable :: packed_prai(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_prai(:,:)
            REAL(KIND=r8), allocatable :: packed_qires(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_qires(:,:)
            REAL(KIND=r8), allocatable :: packed_mnuccr(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_mnuccr(:,:)
            REAL(KIND=r8), allocatable :: packed_pracs(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_pracs(:,:)
            REAL(KIND=r8), allocatable :: packed_meltsdt(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_meltsdt(:,:)
            REAL(KIND=r8), allocatable :: packed_frzrdt(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_frzrdt(:,:)
            REAL(KIND=r8), allocatable :: packed_mnuccd(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_mnuccd(:,:)
            REAL(KIND=r8), allocatable :: packed_nrout(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_nrout(:,:)
            REAL(KIND=r8), allocatable :: packed_nsout(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_nsout(:,:)
            REAL(KIND=r8), allocatable :: packed_refl(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_refl(:,:)
            REAL(KIND=r8), allocatable :: packed_arefl(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_arefl(:,:)
            REAL(KIND=r8), allocatable :: packed_areflz(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_areflz(:,:)
            REAL(KIND=r8), allocatable :: packed_frefl(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_frefl(:,:)
            REAL(KIND=r8), allocatable :: packed_csrfl(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_csrfl(:,:)
            REAL(KIND=r8), allocatable :: packed_acsrfl(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_acsrfl(:,:)
            REAL(KIND=r8), allocatable :: packed_fcsrfl(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_fcsrfl(:,:)
            REAL(KIND=r8), allocatable :: packed_rercld(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_rercld(:,:)
            REAL(KIND=r8), allocatable :: packed_ncai(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_ncai(:,:)
            REAL(KIND=r8), allocatable :: packed_ncal(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_ncal(:,:)
            REAL(KIND=r8), allocatable :: packed_qrout2(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_qrout2(:,:)
            REAL(KIND=r8), allocatable :: packed_qsout2(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_qsout2(:,:)
            REAL(KIND=r8), allocatable :: packed_nrout2(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_nrout2(:,:)
            REAL(KIND=r8), allocatable :: packed_nsout2(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_nsout2(:,:)
            REAL(KIND=r8), allocatable :: packed_freqs(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_freqs(:,:)
            REAL(KIND=r8), allocatable :: packed_freqr(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_freqr(:,:)
            REAL(KIND=r8), allocatable :: packed_nfice(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_nfice(:,:)
            REAL(KIND=r8), allocatable :: packed_prer_evap(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_prer_evap(:,:)
            REAL(KIND=r8), allocatable :: packed_qcrat(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_qcrat(:,:)
            REAL(KIND=r8), allocatable :: packed_rel(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_rel(:,:)
            REAL(KIND=r8), allocatable :: packed_rei(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_rei(:,:)
            REAL(KIND=r8), allocatable :: packed_lambdac(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_lambdac(:,:)
            REAL(KIND=r8), allocatable :: packed_mu(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_mu(:,:)
            REAL(KIND=r8), allocatable :: packed_des(:,:)
            !REAL(KIND=r8), allocatable, target :: ref_packed_des(:,:)
            REAL(KIND=r8), allocatable :: packed_dei(:,:)
#if (USE_OMP_ALLOCATION) == 0 
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_rate1ord_cw2pr_st
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_tlat
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qvlat
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qctend
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qitend
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_nctend
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_nitend
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qrtend
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qstend
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_nrtend
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_prect
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_preci
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_nevapr
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_evapsnow
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_prain
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_prodsnow
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_cmeout
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qsout
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_rflx
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_sflx
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qrout
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qcsevap
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qisevap
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qvres
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_cmei
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_vtrmc
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_vtrmi
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qcsedten
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qisedten
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qrsedten
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qssedten
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_umr
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_ums
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_pra
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_prc
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_mnuccc
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_mnucct
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_msacwi
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_psacws
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_bergs
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_berg
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_melt
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_homo
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qcres
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_prci
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_prai
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qires
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_mnuccr
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_pracs
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_meltsdt
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_frzrdt
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_mnuccd
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_nrout
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_nsout
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_refl
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_arefl
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_frefl
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_csrfl
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_acsrfl
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_fcsrfl
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_rercld
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_ncai
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_ncal
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qrout2
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qsout2
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_nrout2
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_nsout2
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_freqs
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_freqr
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_nfice
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_prer_evap
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_qcrat
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_rel
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_rei
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_lambdac
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_mu
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_des
            !DIR$ ATTRIBUTES ALIGN : 64 :: packed_dei
#endif            
...
packed_npccn, packed_rndst, packed_nacon, packed_tlat, packed_qvlat, packed_qctend, packed_qitend, &
packed_nctend, packed_nitend, packed_qrtend, packed_qstend, packed_nrtend, packed_nstend, packed_rel, &
rel_fn_dum, packed_rei, packed_prect, packed_preci, packed_nevapr, packed_evapsnow, packed_prain, &
packed_prodsnow, packed_cmeout, packed_dei, packed_mu, packed_lambdac, packed_qsout, packed_des, packed_rflx, &
packed_sflx, packed_qrout, reff_rain_dum, reff_snow_dum, packed_qcsevap, packed_qisevap, packed_qvres, &
packed_cmei, packed_vtrmc, packed_vtrmi, packed_umr, packed_ums, packed_qcsedten, packed_qisedten, &
packed_qrsedten, packed_qssedten, packed_pra, packed_prc, packed_mnuccc, packed_mnucct, packed_msacwi, &
packed_psacws, packed_bergs, packed_berg, packed_melt, packed_homo, packed_qcres, packed_prci, &
packed_prai, packed_qires, packed_mnuccr, packed_pracs, packed_meltsdt, packed_frzrdt, packed_mnuccd, &
packed_nrout, packed_nsout, packed_refl, packed_arefl, packed_areflz, packed_frefl, packed_csrfl, &
packed_acsrfl, packed_fcsrfl, packed_rercld, packed_ncai, packed_ncal, packed_qrout2, packed_qsout2, &
packed_nrout2, packed_nsout2, drout_dum, dsout2_dum, packed_freqs, packed_freqr, packed_nfice, packed_qcrat, &
errstring, packed_tnd_qsnow, packed_tnd_nsnow, packed_re_ice, packed_prer_evap, packed_frzimm, packed_frzcnt, &
packed_frzdep)
...
#if (USE_OMP_ALLOCATION) == 1
                type(omp_alloctrait) :: traits(1) 
                integer(omp_memspace_handle_kind) :: memspace = omp_default_memspace
...
#if (USE_OMP_ALLOCATION) == 1
                    traits(1) = [omp_alloctrait(omp_atk_alignment),64]
                    alloc = omp_init_allocator(memspace,1,traits)
                    !$OMP ALLOCATE(var) ALLOCATOR(alloc)
