...
module quadpack
    ...
    !integer(kind=i4), parameter, public :: quadpack_RK = wp !! the real kind used in this module

    real(kind=dp), dimension(5), parameter, private :: d1mach = [tiny(1.0_dp), &
                                                            huge(1.0_dp), &
                                                            real(radix(1.0_dp),kind(1.0_dp))**(-digits(1.0_dp)), &
                                                            epsilon(1.0_dp), &
                                                            log10(real(radix(1.0_dp), kind(1.0_dp)))] !! machine constants
    integer(kind=i4),parameter :: i1mach10 = radix(1.0_dp)
    integer(kind=i4),parameter :: i1mach14 = digits(1.0_dp)

    real(kind=dp), parameter, private :: uflow = d1mach(1) !! the smallest positive magnitude.
    real(kind=dp), parameter, private :: oflow = d1mach(2) !! the largest positive magnitude.
    real(kind=dp), parameter, private :: epmach = d1mach(4) !! the largest relative spacing.
    real(kind=dp), parameter, private :: pi = acos(-1.0_dp) !! pi
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd)
    ...
    recursive subroutine dqag(f, a, b, Epsabs, Epsrel, Key, Result, Abserr, Neval, Ier, &
        ...
        real(kind=dp), intent(in) :: a !! lower limit of integration
        real(kind=dp), intent(out) :: Abserr !! estimate of the modulus of the absolute error,
                                        !! which should equal or exceed `abs(i-result)`
        real(kind=dp), intent(in) :: b !! upper limit of integration
        real(kind=dp), intent(in) :: Epsabs !! absolute accoracy requested
        real(kind=dp), intent(in) :: Epsrel !! relative accuracy requested
                                       !! if epsabs<=0
                                       !! and epsrel<max(50*rel.mach.acc.,0.5e-28),
                                       !! the routine will end with ier = 6.
        real(kind=dp), intent(out) :: Result !! approximation to the integral
        ...
        real(kind=dp) :: Work(Lenw) !! vector of dimension at least `lenw`
                               !! on return
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd)
    ...
    recursive subroutine dqage(f, a, b, Epsabs, Epsrel, Key, Limit, Result, Abserr, &
        ...
        real(kind=dp), intent(in) :: a !! lower limit of integration
        real(kind=dp), intent(in) :: b !! uppwer limit of integration
        real(kind=dp), intent(in) :: Epsabs !! absolute accuracy requested
        real(kind=dp), intent(in) :: Epsrel !! relative accuracy requested
                                       !! if `epsabs<=0`
                                       !! and epsrel<max(50*rel.mach.acc.,0.5e-28),
                                       !! the routine will end with ier = 6.
        ...
        integer(kind=i4), intent(out) :: Ier !!  * ier = 0 normal and reliable termination of the
                                    ...
                                    !!          respectively.
        real(kind=dp), intent(out) :: Alist(Limit) !! vector of dimension at least `limit`, the first
                                              !! `last` elements of which are the left
                                              !! end points of the subintervals in the partition
                                              !! of the given integration range `(a,b)`
        real(kind=dp), intent(out) :: Blist(Limit) !! vector of dimension at least `limit`, the first
                                              !! `last` elements of which are the right
                                              !! end points of the subintervals in the partition
                                              !! of the given integration range `(a,b)`
        real(kind=dp), intent(out) :: Elist(Limit) !! vector of dimension at least `limit`, the first
                                              !! `last` elements of which are the moduli of the
                                              !! absolute error estimates on the subintervals
        real(kind=dp), intent(out) :: Rlist(Limit) !! vector of dimension at least `limit`, the first
        ...
        real(kind=dp) :: area !! sum of the integrals over the subintervals
        real(kind=dp) :: area12 !! `area1 + area2`
        real(kind=dp) :: erro12 !! `error1 + error2`
        real(kind=dp) :: errsum !! sum of the errors over the subintervals
        real(kind=dp) :: errmax !! `elist(maxerr)`
        real(kind=dp) :: errbnd !! requested accuracy `max(epsabs,epsrel*abs(result))`
        integer(kind=i4) :: maxerr !! pointer to the interval with largest error estimate
        real(kind=dp) :: resabs, defabs
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd)
    ...
    recursive subroutine dqagi(f, Bound, Inf, Epsabs, Epsrel, Result, Abserr, Neval, &
                     Ier, Limit, Lenw, Last, Iwork, Work, user_data)
        implicit none

        procedure(func) :: f !! function subprogram defining the integrand function `f(x)`.
        real(kind=dp), intent(out) :: Abserr !! estimate of the modulus of the absolute error,
                                        !! which should equal or exceed `abs(i-result)`
        real(kind=dp), intent(in) :: Bound !! finite bound of integration range
        ...
        integer(kind=i4), intent(out) :: Last !! on return, `last` equals the number of subintervals
                                     ...
                                     !! actually in the work arrays.
        integer(kind=i4), intent(out) :: Neval !! number of integrand evaluations
        class(*),         intent(in)  :: user_data !used with Laplace and Mellin transform subroutines
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd)
    ...
    recursive subroutine dqagie(f, Bound, Inf, Epsabs, Epsrel, Limit, Result, Abserr, &
                      Neval, Ier, Alist, Blist, Rlist, Elist, Iord, Last, user_data)
        implicit none
        ...
        real(kind=dp) :: erro12 !! `error1 + error2`
        real(kind=dp) :: errmax !! `elist(maxerr)`
        real(kind=dp) :: erlast !! error on the interval currently subdivided
                           !! (before that subdivision has taken place)
        real(kind=dp) :: area !! sum of the integrals over the subintervals
        real(kind=dp) :: errsum !! sum of the errors over the subintervals
        real(kind=dp) :: errbnd !! requested accuracy `max(epsabs,epsrel*abs(result))`
        real(kind=dp) :: small !! length of the smallest interval considered up
                          !! to now, multiplied by 1.5
        real(kind=dp) :: erlarg !! sum of the errors over the intervals larger
        ...
        real(kind=dp) :: rlist2(limexp + 2) !! array of dimension at least (`limexp+2`),
                                       !! containing the part of the epsilon table
                                       !! which is still needed for further computations.
        real(kind=dp) :: abseps, boun, correc, defabs, dres, &
        ...
        main: do Last = 2, Limit
            ...
            b2 = Blist(maxerr)
            erlast = errmax
            call dqk15i(f, boun, Inf, a1, b1, area1, error1, resabs, defab1, user_data)
            call dqk15i(f, boun, Inf, a2, b2, area2, error2, resabs, defab2, user_data)
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd)
    ...
    recursive subroutine dqagp(f, a, b, Npts2, Points, Epsabs, Epsrel, Result, Abserr, &
                     Neval, Ier, Leniw, Lenw, Last, Iwork, Work, user_data)
        implicit none
        ...
        real(kind=dp), intent(in) :: Points(Npts2) !! vector of dimension npts2, the first `(npts2-2)`
        ...
        if (Leniw >= (3*Npts2 - 2) .and. Lenw >= (Leniw*2 - Npts2) .and. Npts2 >= 2) then
            ...
            call dqagpe(f, a, b, Npts2, Points, Epsabs, Epsrel, limit, Result, &
                        ...
                        Work(l4), Iwork(1), Iwork(l1), Iwork(l2), Last, user_data)
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd)
    ...
    recursive subroutine dqagpe(f, a, b, Npts2, Points, Epsabs, Epsrel, Limit, Result, &
                      Abserr, Neval, Ier, Alist, Blist, Rlist, Elist, Pts, &
                      Iord, Level, Ndin, Last, user_data)
        implicit none
        ...
        real(kind=dp), intent(in) :: Points(Npts2) !! vector of dimension npts2, the first (npts2-2)
                                              ...
                                              !! ascending sequence there will be an automatic
                                              !! sorting.
        real(kind=dp), intent(out) :: Pts(Npts2) !! vector of dimension at least npts2, containing the
        ...
        real(kind=dp) :: a, abseps, b, correc, defabs, &
        ...
        do i = 1, nint
            b1 = Pts(i + 1)
            call dqk21(f, a1, b1, area1, error1, defabs, resa, user_data)
        ...
        main: do Last = Npts2, Limit
            ...
            b2 = Blist(maxerr)
            erlast = errmax
            call dqk21(f, a1, b1, area1, error1, resa, defab1, user_data)
            call dqk21(f, a2, b2, area2, error2, resa, defab2, user_data)
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd)
    ...
    recursive subroutine dqags(f, a, b, Epsabs, Epsrel, Result, Abserr, Neval, Ier, &
                     Limit, Lenw, Last, Iwork, Work, user_data)
        implicit none
        ...
        if (Limit >= 1 .and. Lenw >= Limit*4) then
            ...
            call dqagse(f, a, b, Epsabs, Epsrel, Limit, Result, Abserr, Neval, Ier, &
                        Work(1), Work(l1), Work(l2), Work(l3), Iwork, Last, user_data)
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd)
    ...
    recursive subroutine dqagse(f, a, b, Epsabs, Epsrel, Limit, Result, Abserr, Neval, &
                      Ier, Alist, Blist, Rlist, Elist, Iord, Last, user_data)
        implicit none
        ...
        real(kind=dp) :: abseps, correc, defabs, dres, &
                    ertest, resabs, reseps, res3la(3)
        integer(kind=i4) :: id, ierro, iroff1, iroff2, iroff3, &
                   jupbnd, k, ksgn, ktmin, nrmax
        real(kind=dp) :: area12 !! `area1 + area2`
        real(kind=dp) :: erro12 !! `error1 + error2`
        real(kind=dp) :: area1, a1, b1, defab1, error1 !! variable for the left interval
        real(kind=dp) :: area2, a2, b2, defab2, error2 !! variable for the right interval
        real(kind=dp) :: rlist2(limexp + 2) !! array of dimension at least `limexp+2` containing
        ...
        if (Ier /= 6) then
            ...
            ierro = 0
            call dqk21(f, a, b, Result, Abserr, defabs, resabs, user_data)
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd)
    ...
    recursive subroutine dqawc(f, a, b, c, Epsabs, Epsrel, Result, Abserr, Neval, Ier, &
                     Limit, Lenw, Last, Iwork, Work, user_data)
        implicit none

        procedure(func) :: f !! function subprogram defining the integrand function `f(x)`.
        real(kind=dp), intent(in) :: a !! under limit of integration
        real(kind=dp), intent(in) :: b !! upper limit of integration
        real(kind=dp), intent(in) :: c !! parameter in the weight function, `c/=a`, `c/=b`.
        ...
        real(kind=dp), intent(in) :: Epsrel !! relative accuracy requested
                                       ...
                                       !! the routine will end with ier = 6.
        real(kind=dp), intent(out) :: Result !! approximation to the integral
        real(kind=dp), intent(out) :: Abserr !! estimate or the modulus of the absolute error,
                                        !! which should equal or exceed `abs(i-result)`
        integer(kind=i4), intent(out) :: Neval !! number of integrand evaluations
        integer(kind=i4), intent(out) :: Ier !! * ier = 0 normal and reliable termination of the
                                    !!   routine. it is assumed that the requested
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd)
    ...
    recursive subroutine dqawce(f, a, b, c, Epsabs, Epsrel, Limit, Result, Abserr, Neval, &
                      Ier, Alist, Blist, Rlist, Elist, Iord, Last, user_data)
        implicit none
        ...
        real(kind=dp), intent(out) :: Rlist(Limit) !! vector of dimension at least `limit`, the first
                                              !! `last` elements of which are the integral
                                              !! approximations on the subintervals
        real(kind=dp), intent(out) :: Elist(Limit) !! vector of dimension `limit`, the first `last`
        ...
        real(kind=dp) :: aa, bb, c
        integer(kind=i4) :: iroff1, iroff2, k, krule, nev, nrmax
        real(kind=dp) :: area1, a1, b1, error1 !! variable for the left subinterval
        real(kind=dp) :: area2, a2, b2, error2 !! variable for the right subinterval
        real(kind=dp) :: area12 !! `area1 + area2`
        real(kind=dp) :: erro12 !! `error1 + error2`
        real(kind=dp) :: errmax !! elist(maxerr)
        real(kind=dp) :: area !! sum of the integrals over the subintervals
        ...
        if (.not. (c == a .or. c == b .or. (Epsabs <= 0.0_dp .and. Epsrel < max &
            ...
            Ier = 0
            krule = 1
            call dqc25c(f, aa, bb, c, Result, Abserr, krule, Neval, user_data)
            Last = 1
            Rlist(1) = Result
            Elist(1) = Abserr
            Iord(1) = 1
            ...
            if (Abserr >= min(0.01_dp*abs(Result), errbnd) .and. Ier /= 1) then
                ...
                do Last = 2, Limit
                    ...
                    a2 = b1
                    krule = 2
                    call dqc25c(f, a1, b1, c, area1, error1, krule, nev, user_data)
                    Neval = Neval + nev
                    call dqc25c(f, a2, b2, c, area2, error2, krule, nev, user_data)
                    Neval = Neval + nev
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd)
    ...
    recursive subroutine dqawf(f, a, Omega, Integr, Epsabs, Result, Abserr, Neval, Ier, &
                     Limlst, Lst, Leniw, Maxp1, Lenw, Iwork, Work, user_data)
        implicit none

        procedure(func) :: f !! function subprogram defining the integrand function `f(x)`.
        real(kind=dp), intent(in) :: a !! lower limit of integration
        real(kind=dp), intent(in) :: Omega !! parameter in the integrand weight function
        integer(kind=i4), intent(in) :: Integr !! indicates which of the weight functions is used:
                                      ...
                                      !! will end with ier = 6.
        real(kind=dp), intent(in) :: Epsabs !! absolute accuracy requested, `epsabs>0`.
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd)
    ...
    recursive subroutine dqawfe(f, a, Omega, Integr, Epsabs, Limlst, Limit, Maxp1, &
                      Result, Abserr, Neval, Ier, Rslst, Erlst, Ierlst, Lst, &
                      Alist, Blist, Rlist, Elist, Iord, Nnlog, Chebmo, user_data)
        implicit none

        procedure(func) :: f !! function subprogram defining the integrand function `f(x)`.
        real(kind=dp), intent(in) :: a !! lower limit of integration
        real(kind=dp), intent(in) :: Omega !! parameter in the weight function
        ...
        real(kind=dp), intent(out) :: Result !! approximation to the integral `x`
        real(kind=dp), intent(out) :: Abserr !! estimate of the modulus of the absolute error,
                                        !! which should equal or exceed `abs(i-result)`
        integer(kind=i4), intent(out) :: Neval !! number of integrand evaluations
        integer(kind=i4), intent(out) :: Ier !! * ier = 0 normal and reliable termination of
                                    ...
                                    !! for `ierlst(k), k = 1`).
        real(kind=dp), intent(out) :: Rslst(Limlst) !! vector of dimension at least `limlst`
        ...
        real(kind=dp), intent(out) :: Erlst(Limlst) !! vector of dimension at least `limlst`
        ...
        integer(kind=i4), intent(out) :: Lst !! number of subintervals needed for the integration
                                    !! if `omega = 0` then lst is set to 1.
        real(kind=dp), intent(out) :: Alist(Limit) !! vector of dimension at least `limit`
        real(kind=dp), intent(out) :: Blist(Limit) !! vector of dimension at least `limit`
        real(kind=dp), intent(out) :: Rlist(Limit) !! vector of dimension at least `limit`
        real(kind=dp), intent(out) :: Elist(Limit) !! vector of dimension at least `limit`
        ...
        integer(kind=i4), intent(out) :: Nnlog(Limit) !! vector of dimension at least `limit`, providing
                                             !! space for the quantities needed in the subdivision
                                             !! process of each cycle
        real(kind=dp), intent(out) :: Chebmo(Maxp1, 25) !! array of dimension at least `(maxp1,25)`, providing
                                                   !! space for the chebyshev moments needed within the
        !! cycles (see also routine [[dqc25f]])
        class(*),  intent(in) :: user_data

        real(kind=dp) :: abseps, correc, dl, dla, drl, ep, eps, fact, p1, reseps, res3la(3)
        integer(kind=i4) :: ktmin, l, last, ll, momcom, nev, nres, numrl2
        real(kind=dp) :: psum(limexp + 2) !! `psum` contains the part of the epsilon table
                                     !! which is still needed for further computations.
                                     !! each element of `psum` is a partial sum of the
                                     !! series which should sum to the value of the
                                     !! integral.
        real(kind=dp) :: c1, c2 !! end points of subinterval (of length cycle)
        real(kind=dp) :: cycle !! `(2*int(abs(omega))+1)*pi/abs(omega)`
        real(kind=dp) :: errsum  !! sum of error estimates over the subintervals,
                            !! calculated cumulatively
        real(kind=dp) :: epsa !! absolute tolerance requested over current
                         !! subinterval

        real(kind=dp), parameter :: p = 0.9_dp
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd)
    ...
    recursive subroutine dqawo(f, a, b, Omega, Integr, Epsabs, Epsrel, Result, Abserr, &
                     Neval, Ier, Leniw, Maxp1, Lenw, Last, Iwork, Work, user_data)
        implicit none
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd)
    ...
    recursive subroutine dqawoe(f, a, b, Omega, Integr, Epsabs, Epsrel, Limit, Icall, &
                      Maxp1, Result, Abserr, Neval, Ier, Last, Alist, Blist, &
                      Rlist, Elist, Iord, Nnlog, Momcom, Chebmo, user_data)
        implicit none
        ...
        integer(kind=i4), intent(inout) :: Momcom !! indicating that the chebyshev moments
                                         ...
                                         !! `momcom<maxp1`
        real(kind=dp), intent(inout) :: Chebmo(Maxp1, 25) !! array of dimension `(maxp1,25)`
        ...
        real(kind=dp) :: errsum !! sum of the errors over the subintervals
        real(kind=dp) :: errbnd !! requested accuracy `max(epsabs,epsrel*abs(result))`
        real(kind=dp) :: a1, area1, b1, error1 !! variable for the left subinterval
        real(kind=dp) :: a2, area2, b2, error2 !! variable for the right subinterval
        integer(kind=i4) :: nres !! number of calls to the extrapolation routine
        integer(kind=i4) :: numrl2 !! number of elements in `rlist2`. if an appropriate
                          !! approximation to the compounded integral has
                          !! been obtained it is put in `rlist2(numrl2)` after
                          !! `numrl2` has been increased by one
        real(kind=dp) :: small !! length of the smallest interval considered
        ...
        real(kind=dp) :: abseps, correc, defab1, defab2, &
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd)
    ...
    recursive subroutine dqaws(f, a, b, alfa, beta, integr, epsabs, epsrel, result, &
                     abserr, neval, ier, limit, lenw, last, iwork, work, user_data)
        implicit none

        procedure(func) :: f !! function subprogram defining the integrand function `f(x)`.
        real(kind=dp), intent(in) :: a !! lower limit of integration
        real(kind=dp), intent(in) :: b !! upper limit of integration, b>a
                                  !! if b<=a, the routine will end with ier = 6.
        real(kind=dp), intent(in) :: alfa !! parameter in the integrand function, `alfa>(-1)`
                                     !! if `alfa<=(-1)`, the routine will end with
                                     !! ier = 6.
        real(kind=dp), intent(in) :: beta !! parameter in the integrand function, `beta>(-1)`
        ...
        integer(kind=i4) :: iwork(limit) !! vector of dimension limit, the first `k`
                                ...
                                !! and `k = limit+1-last` otherwise
        real(kind=dp) :: work(lenw) !! on return:
                               !!
                               !! * `work(1), ..., work(last)` contain the left
                               !!   end points of the subintervals in the
                               !!   partition of `(a,b)`,
        ...
        if (limit >= 2 .and. lenw >= limit*4) then
            ...
            call dqawse(f, a, b, alfa, beta, integr, epsabs, epsrel, limit, result, &
                        abserr, neval, ier, work(1), work(l1), work(l2), work(l3), iwork, last, user_data)
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd)
    ...
    recursive subroutine dqawse(f, a, b, Alfa, Beta, Integr, Epsabs, Epsrel, Limit, &
        ...
        real(kind=dp), intent(in) :: b !! upper limit of integration, `b>a`.
                                  !! if `b<=a`, the routine will end with ier = 6.
        real(kind=dp), intent(in) :: Alfa !! parameter in the weight function, `alfa>(-1)`
                                     !! if `alfa<=(-1)`, the routine will end with
                                     !! ier = 6.
        real(kind=dp), intent(in) :: Beta !! parameter in the weight function, `beta>(-1)`
                                     !! if `beta<=(-1)`, the routine will end with
                                     !! ier = 6.
        integer(kind=i4), intent(in) :: Integr !! indicates which weight function is to be used:
                                      !!
        ...
        real(kind=dp) :: a1, b1, area1, error1 !! variable for the left subinterval
        real(kind=dp) :: a2, b2, area2, error2 !! variable for the right subinterval
        real(kind=dp) :: area12 !! `area1 + area2`
        real(kind=dp) :: erro12 !! `error1 + error2`
        real(kind=dp) :: area !! sum of the integrals over the subintervals
        real(kind=dp) :: errbnd !! requested accuracy `max(epsabs,epsrel*abs(result))`
        ...
        real(kind=dp) :: errsum !! sum of the errors over the subintervals
        integer(kind=i4) :: maxerr !! pointer to the interval with largest error estimate
        real(kind=dp) :: centre, resas1, resas2, rg(25), rh(25), ri(25), rj(25)
            ...
            if (Abserr > errbnd .and. Ier /= 1) then
                ...
                do Last = 3, Limit
                    ...
                    call dqc25s(f, a, b, a1, b1, Alfa, Beta, ri, rj, rg, rh, area1, &
                                error1, resas1, Integr, nev, user_data)
                    Neval = Neval + nev
                    call dqc25s(f, a, b, a2, b2, Alfa, Beta, ri, rj, rg, rh, area2, &
                                error2, resas2, Integr, nev, user_data)
                    Neval = Neval + nev
...
!  * QUADPACK: date written 810101, revision date 830518 (yymmdd)
    ...
    recursive subroutine dqc25c(f, a, b, c, Result, Abserr, Krul, Neval, user_data)
        implicit none

        procedure(func) :: f !! function subprogram defining the integrand function `f(x)`.
        real(kind=dp), intent(in) :: a !! left end point of the integration interval
        real(kind=dp), intent(in) :: b !! right end point of the integration interval, `b>a`
        real(kind=dp), intent(in) :: c !! parameter in the weight function
        real(kind=dp), intent(out) :: Result !! approximation to the integral.
        ...
        real(kind=dp) :: ak22, amom0, amom1, amom2, cc, &
                    p2, p3, p4, resabs, resasc, u
        integer(kind=i4) :: i, isym, k, kp
        real(kind=dp) :: fval(25) !! value of the function `f` at the points
                             !! `cos(k*pi/24)`, `k = 0, ..., 24`
        real(kind=dp) :: cheb12(13) !! chebyshev series expansion coefficients,
                               !! for the function `f`, of degree 12
        real(kind=dp) :: cheb24(25) !! chebyshev series expansion coefficients,
                               !! for the function `f`, of degree 24
        real(kind=dp) :: res12 !! approximation to the integral corresponding
                          !! to the use of cheb12
        real(kind=dp) :: res24 !! approximation to the integral corresponding
                          !! to the use of cheb24
        real(kind=dp) :: hlgth !! half-length of the interval
        real(kind=dp) :: centr !! mid point of the interval

        real(kind=dp), dimension(11), parameter :: x = [(cos(k*pi/24.0_dp), k=1, 11)]
        ...
        if (abs(cc) < 1.1_dp) then
            ...
            fval(13) = f(centr,user_data)
            fval(25) = 0.5_dp*f(centr - hlgth,user_data)
            do i = 2, 12
                u = hlgth*x(i - 1)
                isym = 26 - i
...
!  * QUADPACK: date written 810101, revision date 211011 (yymmdd)
    ...
    subroutine dqc25f(f, a, b, Omega, Integr, Nrmom, Maxp1, Ksave, Result, &
                      Abserr, Neval, Resabs, Resasc, Momcom, Chebmo, user_data)
        implicit none
        ...
        real(kind=dp), intent(out) :: Result !! approximation to the integral i
        real(kind=dp), intent(out) :: Abserr !! estimate of the modulus of the absolute
                                        !! error, which should equal or exceed `abs(i-result)`
        integer(kind=i4), intent(out) :: Neval !! number of integrand evaluations
        real(kind=dp), intent(out) :: Resabs !! approximation to the integral j
        real(kind=dp), intent(out) :: Resasc !! approximation to the integral of `abs(f-i/(b-a))`
        ...
        real(kind=dp), intent(inout) :: Chebmo(Maxp1, 25) !! array of dimension at least `(maxp1,25)` containing
                                                     !! the modified chebyshev moments for the first `momcom`
        !! `momcom` interval lengths
        class(*),      intent(in)    :: user_data

        real(kind=dp) :: ac, an, an2, as, asap, ass, conc, &
                    cons, cospar, d(25), d1(25), d2(25), &
                    estc, ests, parint, par2, par22, &
                    p2, p3, p4, sinpar, v(28)
        integer(kind=i4) :: i, iers, isym, j, k, m, noequ, noeq1
        real(kind=dp) :: centr !! mid point of the integration interval
        real(kind=dp) :: hlgth !! half-length of the integration interval
        real(kind=dp) :: fval(25) !! value of the function `f` at the points
                             !! `(b-a)*0.5*cos(k*pi/12) + (b+a)*0.5`, `k = 0, ..., 24`
        real(kind=dp) :: cheb12(13) !! coefficients of the chebyshev series expansion
                               !! of degree 12, for the function `f`, in the
                               !! interval `(a,b)`
        real(kind=dp) :: cheb24(25) !! coefficients of the chebyshev series expansion
                               !! of degree 24, for the function `f`, in the
                               !! interval `(a,b)`
        real(kind=dp) :: resc12 !! approximation to the integral of
                           !! `cos(0.5*(b-a)*omega*x)*f(0.5*(b-a)*x+0.5*(b+a))`
                           !! over `(-1,+1)`, using the chebyshev series
                           !! expansion of degree 12
        real(kind=dp) :: resc24 !! approximation to the same integral, using the
                           !! chebyshev series expansion of degree 24
        real(kind=dp) :: ress12 !! the analogue of `resc12` for the sine
        real(kind=dp) :: ress24 !! the analogue of `resc24` for the sine
        ...
        if (abs(parint) > 2.0_dp) then
            ...
            fval(1) = 0.5_dp*f(centr + hlgth,user_data)
            fval(13) = f(centr,user_data)
            fval(25) = 0.5_dp*f(centr - hlgth,user_data)
            do i = 2, 12
                isym = 26 - i
...
!  * QUADPACK: date written 810101, revision date 830518 (yymmdd)
    ...
    subroutine dqc25s(f, a, b, Bl, Br, Alfa, Beta, Ri, Rj, Rg, Rh, Result, Abserr, &
        ...
        real(kind=dp), intent(in) :: a !! left end point of the original interval
        real(kind=dp), intent(in) :: b !! right end point of the original interval, `b>a`
        real(kind=dp), intent(in) :: Bl !! lower limit of integration, `bl>=a`
        real(kind=dp), intent(in) :: Br !! upper limit of integration, `br<=b`
        real(kind=dp), intent(in) :: Alfa !! parameter in the weight function
        real(kind=dp), intent(in) :: Beta !! parameter in the weight function
        real(kind=dp), intent(in) :: Ri(25) !! modified chebyshev moments for the application
                                       !! of the generalized clenshaw-curtis
                                       !! method (computed in subroutine [[dqmomo]])
        real(kind=dp), intent(in) :: Rj(25) !! modified chebyshev moments for the application
                                       !! of the generalized clenshaw-curtis
                                       !! method (computed in subroutine [[dqmomo]])
        real(kind=dp), intent(in) :: Rg(25) !! modified chebyshev moments for the application
                                       !! of the generalized clenshaw-curtis
                                       !! method (computed in subroutine [[dqmomo]])
        real(kind=dp), intent(in) :: Rh(25) !! modified chebyshev moments for the application
                                       !! of the generalized clenshaw-curtis
                                       !! method (computed in subroutine [[dqmomo]])
        real(kind=dp), intent(out) :: Result !! approximation to the integral
                                        !! `result` is computed by using a generalized
        ...
        real(kind=dp), intent(out) :: Abserr !! estimate of the modulus of the absolute error,
                                        !! which should equal or exceed `abs(i-result)`
        real(kind=dp), intent(out) :: Resasc !! approximation to the integral of abs(f*w-i/(b-a))
        ...
        real(kind=dp) :: fval(25) !! value of the function f at the points
                             ...
                             !! `k = 0, ..., 24`
        real(kind=dp) :: res12 !! approximation to the integral obtained from `cheb12`
        real(kind=dp) :: res24 !! approximation to the integral obtained from `cheb24`
        real(kind=dp) :: hlgth !! half-length of the interval `(bl,br)`
        real(kind=dp) :: centr !! mid point of the interval `(bl,br)`
        integer(kind=i4) :: k !! counter for `x`
        real(kind=dp) :: dc, factor, fix, resabs, u
        ...
        if (Bl == a .and. (Alfa /= 0.0_dp .or. Integr == 2 .or. Integr == 4)) &
            ...
            centr = 0.5_dp*(Br + Bl)
            fix = b - centr
            fval(1) = 0.5_dp*f(hlgth + centr,user_data)*(fix - hlgth)**Beta
            fval(13) = f(centr,user_data)*(fix**Beta)
            fval(25) = 0.5_dp*f(centr - hlgth,user_data)*(fix + hlgth)**Beta
            do i = 2, 12
                u = hlgth*x(i - 1)
                isym = 26 - i
                fval(i) = f(u + centr,user_data)*(fix - u)**Beta
                fval(isym) = f(centr - u,user_data)*(fix + u)**Beta
            end do
        ...
        elseif (Br == b .and. (Beta /= 0.0_dp .or. Integr == 3 .or. Integr == 4)) then
            ...
            centr = 0.5_dp*(Br + Bl)
            fix = centr - a
            fval(1) = 0.5_dp*f(hlgth + centr,user_data)*(fix + hlgth)**Alfa
            fval(13) = f(centr,user_data)*(fix**Alfa)
            fval(25) = 0.5_dp*f(centr - hlgth,user_data)*(fix - hlgth)**Alfa
            do i = 2, 12
                u = hlgth*x(i - 1)
                isym = 26 - i
                fval(i) = f(u + centr,user_data)*(fix + u)**Alfa
                fval(isym) = f(centr - u,user_data)*(fix - u)**Alfa
            end do
...
!  * QUADPACK: revision date 830518 (yymmdd)
    ...
    subroutine dqcheb(x, Fval, Cheb12, Cheb24)
        ...
        real(kind=dp), intent(in) :: x(11) !! vector of dimension 11 containing the
                                      !! values `cos(k*pi/24), k = 1, ..., 11`
        real(kind=dp), intent(inout) :: Fval(25) !! vector of dimension 25 containing the
        ...
        real(kind=dp), intent(out) :: Cheb12(13) !! vector of dimension 13 containing the
                                            !! chebyshev coefficients for degree 12
        real(kind=dp), intent(out) :: Cheb24(25) !! vector of dimension 25 containing the
                                            !! chebyshev coefficients for degree 24

        real(kind=dp) :: alam, alam1, alam2, part1, part2, part3, v(12)
        integer(kind=i4) :: i, j
...
!  * QUADPACK: revision date 830518 (yymmdd).
    ...
    subroutine dqelg(n, Epstab, Result, Abserr, Res3la, Nres)
        ...
        real(kind=dp), intent(out) :: Abserr !! estimate of the absolute error computed from
                                        !! result and the 3 previous results
        real(kind=dp), intent(inout) :: Epstab(limexp + 2) !! vector of dimension 52 containing the elements
                                                      !! of the two lower diagonals of the triangular
                                                      !! epsilon table. the elements are numbered
                                                      !! starting at the right-hand corner of the
                                                      !! triangle.
        real(kind=dp), intent(out) :: Result !! resulting approximation to the integral
        real(kind=dp), intent(inout) :: Res3la(3) !! vector of dimension 3 containing the last 3
                                             !! results
        integer(kind=i4), intent(inout) :: Nres !! number of calls to the routine
                                       !! (should be zero at first call)

        real(kind=dp) :: delta1, delta2, delta3, epsinf, &
        ...
        real(kind=dp) :: error !! `error = abs(e1-e0)+abs(e2-e1)+abs(new-e2)`
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd).
    ...
    subroutine dqk15(f, a, b, Result, Abserr, Resabs, Resasc, user_data)
        implicit none
        ...
        real(kind=dp), intent(out) :: Resabs !! approximation to the integral j
        real(kind=dp), intent(out) :: Resasc !! approximation to the integral of `abs(f-i/(b-a))` over `(a,b)`
        class(*),      intent(in)  :: user_data
        real(kind=dp) :: dhlgth, fc, fsum, fv1(7), fv2(7)
        integer(kind=i4) :: j, jtw, jtwm1
        real(kind=dp) :: centr !! mid point of the interval
        real(kind=dp) :: hlgth !! half-length of the interval
        real(kind=dp) :: absc !! abscissa
        real(kind=dp) :: fval1 !! function value
        real(kind=dp) :: fval2 !! function value
        real(kind=dp) :: resg !! result of the 7-point gauss formula
        real(kind=dp) :: resk !! result of the 15-point kronrod formula
        real(kind=dp) :: reskh !! approximation to the mean value of `f` over `(a,b)`, i.e. to `i/(b-a)`

        ! the abscissae and weights are given for the interval (-1,1).
        ! because of symmetry only the positive abscissae and their
        ! corresponding weights are given.

        real(kind=dp), dimension(4), parameter :: wg = [ &
                                             1.29484966168869693270611432679082018329e-1_dp, &
                                             2.79705391489276667901467771423779582487e-1_dp, &
                                             3.81830050505118944950369775488975133878e-1_dp, &
                                             4.17959183673469387755102040816326530612e-1_dp] !! weights of the 7-point gauss rule

        real(kind=dp), dimension(8), parameter :: xgk = [ &
                                             9.91455371120812639206854697526328516642e-1_dp, &
                                             9.49107912342758524526189684047851262401e-1_dp, &
                                             8.64864423359769072789712788640926201211e-1_dp, &
                                             7.41531185599394439863864773280788407074e-1_dp, &
                                             5.86087235467691130294144838258729598437e-1_dp, &
                                             4.05845151377397166906606412076961463347e-1_dp, &
                                             2.07784955007898467600689403773244913480e-1_dp, &
                                             0.00000000000000000000000000000000000000e0_dp] !! abscissae of the 15-point kronrod rule:
                                                                                            !!
        ...
        real(kind=dp), dimension(8), parameter :: wgk = [ &
                                             2.29353220105292249637320080589695919936e-2_dp, &
                                             6.30920926299785532907006631892042866651e-2_dp, &
                                             1.04790010322250183839876322541518017444e-1_dp, &
                                             1.40653259715525918745189590510237920400e-1_dp, &
                                             1.69004726639267902826583426598550284106e-1_dp, &
                                             1.90350578064785409913256402421013682826e-1_dp, &
                                             2.04432940075298892414161999234649084717e-1_dp, &
                                             2.09482141084727828012999174891714263698e-1_dp] !! weights of the 15-point kronrod rule
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd).
    ...
    subroutine dqk15i(f, Boun, Inf, a, b, Result, Abserr, Resabs, Resasc, user_data)
        implicit none

        procedure(func) :: f !! function subprogram defining the integrand function `f(x)`.
        real(kind=dp), intent(in) :: Boun !! finite bound of original integration
                                     !! range (set to zero if inf = +2)
        real(kind=dp), intent(in) :: a !! lower limit for integration over subrange of `(0,1)`
        real(kind=dp), intent(in) :: b !! upper limit for integration over subrange of `(0,1)`
        ...
        real(kind=dp), intent(out) :: Abserr !! estimate of the modulus of the absolute error,
                                        !! which should equal or exceed `abs(i-result)`
        real(kind=dp), intent(out) :: Resabs !! approximation to the integral j
        real(kind=dp), intent(out) :: Resasc !! approximation to the integral of
                                        !! `abs((transformed integrand)-i/(b-a))` over `(a,b)`
        class(*),       intent(in)  :: user_data
        real(kind=dp) :: absc, dinf, fc, fsum, fv1(7), fv2(7)
        integer(kind=i4) :: j
        real(kind=dp) :: centr   !! mid point of the interval
        real(kind=dp) :: hlgth   !! half-length of the interval
        real(kind=dp) :: absc1   !! abscissa
        real(kind=dp) :: absc2   !! abscissa
        real(kind=dp) :: tabsc1  !! transformed abscissa
        real(kind=dp) :: tabsc2  !! transformed abscissa
        real(kind=dp) :: fval1   !! function value
        real(kind=dp) :: fval2   !! function value
        real(kind=dp) :: resg    !! result of the 7-point gauss formula
        real(kind=dp) :: resk    !! result of the 15-point kronrod formula
        real(kind=dp) :: reskh   !! approximation to the mean value of the transformed
        ...
        real(kind=dp), dimension(8), parameter :: wg = [ &
                                             0.00000000000000000000000000000000000000e0_dp, &
                                             1.29484966168869693270611432679082018329e-1_dp, &
                                             0.00000000000000000000000000000000000000e0_dp, &
                                             2.79705391489276667901467771423779582487e-1_dp, &
                                             0.00000000000000000000000000000000000000e0_dp, &
                                             3.81830050505118944950369775488975133878e-1_dp, &
                                             0.00000000000000000000000000000000000000e0_dp, &
                                             4.17959183673469387755102040816326530612e-1_dp] !! weights of the 7-point gauss rule, corresponding
        ...
        do j = 1, 7
            ...
            if (Inf == 2) then
                fval1 = fval1 + f(-tabsc1,user_data)
                fval2 = fval2 + f(-tabsc2,user_data)
            end if
...
!  * QUADPACK: date written 810101, revision date 830518 (yymmdd).
    ...
    subroutine dqk15w(f, w, p1, p2, p3, p4, Kp, a, b, Result, Abserr, Resabs, &
        ...
        real(kind=dp), intent(in) :: p1 !! parameter in the weight function
        real(kind=dp), intent(in) :: p2 !! parameter in the weight function
        real(kind=dp), intent(in) :: p3 !! parameter in the weight function
        real(kind=dp), intent(in) :: p4 !! parameter in the weight function
        ...
        real(kind=dp), intent(out) :: Abserr !! estimate of the modulus of the absolute error,
                                        !! which should equal or exceed `abs(i-result)`
        real(kind=dp), intent(out) :: Resabs !! approximation to the integral of `abs(f)`
        real(kind=dp), intent(out) :: Resasc !! approximation to the integral of `abs(f-i/(b-a))`
        class(*),      intent(in)  :: user_data
        real(kind=dp) :: absc1, absc2, dhlgth, fc, fsum, fv1(7), fv2(7)
        integer(kind=i4) :: j, jtw, jtwm1
        real(kind=dp) :: centr !! mid point of the interval
        real(kind=dp) :: hlgth !! half-length of the interval
        real(kind=dp) :: absc !! abscissa
        ...
        real(kind=dp) :: resg !! result of the 7-point gauss formula
        real(kind=dp) :: resk !! result of the 15-point kronrod formula
        real(kind=dp) :: reskh !! approximation to the mean value of f*w over `(a,b)`, i.e. to `i/(b-a)`
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd).
    ...
    subroutine dqk21(f, a, b, Result, Abserr, Resabs, Resasc, user_data)
        implicit none
        ...
        real(kind=dp), intent(out) :: Resasc !! approximation to the integral of `abs(f-i/(b-a))`
                                        !! over `(a,b)`
        class(*),      intent(in)  :: user_data
        real(kind=dp) :: dhlgth, fc, fsum, fv1(10), fv2(10)
        integer(kind=i4) :: j, jtw, jtwm1
        real(kind=dp) :: centr !! mid point of the interval
        real(kind=dp) :: hlgth !! half-length of the interval
        real(kind=dp) :: absc !! abscissa
        real(kind=dp) :: fval1 !! function value
        real(kind=dp) :: fval2 !! function value
        real(kind=dp) :: resg !! result of the 10-point gauss formula
        real(kind=dp) :: resk !! result of the 21-point kronrod formula
        real(kind=dp) :: reskh !! approximation to the mean value of `f` over `(a,b)`, i.e. to `i/(b-a)`
        ...
        real(kind=dp), dimension(5), parameter :: wg = [ &
                                             6.66713443086881375935688098933317928579e-2_dp, &
                                             1.49451349150580593145776339657697332403e-1_dp, &
                                             2.19086362515982043995534934228163192459e-1_dp, &
                                             2.69266719309996355091226921569469352860e-1_dp, &
                                             2.95524224714752870173892994651338329421e-1_dp] !! weights of the 10-point gauss rule

        real(kind=dp), dimension(11), parameter :: xgk = [ &
                                              9.95657163025808080735527280689002847921e-1_dp, &
                                              9.73906528517171720077964012084452053428e-1_dp, &
                                              9.30157491355708226001207180059508346225e-1_dp, &
                                              8.65063366688984510732096688423493048528e-1_dp, &
                                              7.80817726586416897063717578345042377163e-1_dp, &
                                              6.79409568299024406234327365114873575769e-1_dp, &
                                              5.62757134668604683339000099272694140843e-1_dp, &
                                              4.33395394129247190799265943165784162200e-1_dp, &
                                              2.94392862701460198131126603103865566163e-1_dp, &
                                              1.48874338981631210884826001129719984618e-1_dp, &
                                              0.00000000000000000000000000000000000000e0_dp] !! abscissae of the 21-point kronrod rule:
                                                                                             !!
        ...
        real(kind=dp), dimension(11), parameter :: wgk = [ &
                                              1.16946388673718742780643960621920483962e-2_dp, &
                                              3.25581623079647274788189724593897606174e-2_dp, &
                                              5.47558965743519960313813002445801763737e-2_dp, &
                                              7.50396748109199527670431409161900093952e-2_dp, &
                                              9.31254545836976055350654650833663443900e-2_dp, &
                                              1.09387158802297641899210590325804960272e-1_dp, &
                                              1.23491976262065851077958109831074159512e-1_dp, &
                                              1.34709217311473325928054001771706832761e-1_dp, &
                                              1.42775938577060080797094273138717060886e-1_dp, &
                                              1.47739104901338491374841515972068045524e-1_dp, &
                                              1.49445554002916905664936468389821203745e-1_dp] !! weights of the 21-point kronrod rule
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd).
    ...
    subroutine dqk31(f, a, b, Result, Abserr, Resabs, Resasc, user_data)
        implicit none
        ...
        real(kind=dp), intent(out) :: Result !! approximation to the integral i
                                        ...
                                        !! rule (resg).
        real(kind=dp), intent(out) :: Abserr !! estimate of the modulus of the modulus,
                                        !! which should not exceed `abs(i-result)`
        real(kind=dp), intent(out) :: Resabs !! approximation to the integral j
        real(kind=dp), intent(out) :: Resasc !! approximation to the integral of `abs(f-i/(b-a))`
                                        !! over `(a,b)`
        ...
        real(kind=dp) :: fval1 !! function value
        real(kind=dp) :: fval2 !! function value
        real(kind=dp) :: resg !! result of the 15-point gauss formula
        real(kind=dp) :: resk !! result of the 31-point kronrod formula
        real(kind=dp) :: reskh !! approximation to the mean value of `f` over `(a,b)`, i.e. to `i/(b-a)`
        real(kind=dp) :: dhlgth, fc, fsum, fv1(15), fv2(15)
        integer(kind=i4) :: j, jtw, jtwm1
        ...
        real(kind=dp), dimension(8), parameter :: wg = [ &
                                             3.07532419961172683546283935772044177217e-2_dp, &
                                             7.03660474881081247092674164506673384667e-2_dp, &
                                             1.07159220467171935011869546685869303416e-1_dp, &
                                             1.39570677926154314447804794511028322521e-1_dp, &
                                             1.66269205816993933553200860481208811131e-1_dp, &
                                             1.86161000015562211026800561866422824506e-1_dp, &
                                             1.98431485327111576456118326443839324819e-1_dp, &
                                             2.02578241925561272880620199967519314839e-1_dp] !! weights of the 15-point gauss rule

        real(kind=dp), dimension(16), parameter :: xgk = [ &
                                              9.98002298693397060285172840152271209073e-1_dp, &
                                              9.87992518020485428489565718586612581147e-1_dp, &
                                              9.67739075679139134257347978784337225283e-1_dp, &
                                              9.37273392400705904307758947710209471244e-1_dp, &
                                              8.97264532344081900882509656454495882832e-1_dp, &
                                              8.48206583410427216200648320774216851366e-1_dp, &
                                              7.90418501442465932967649294817947346862e-1_dp, &
                                              7.24417731360170047416186054613938009631e-1_dp, &
                                              6.50996741297416970533735895313274692547e-1_dp, &
                                              5.70972172608538847537226737253910641238e-1_dp, &
                                              4.85081863640239680693655740232350612866e-1_dp, &
                                              3.94151347077563369897207370981045468363e-1_dp, &
                                              2.99180007153168812166780024266388962662e-1_dp, &
                                              2.01194093997434522300628303394596207813e-1_dp, &
                                              1.01142066918717499027074231447392338787e-1_dp, &
                                              0.00000000000000000000000000000000000000e0_dp] !! abscissae of the 31-point kronrod rule:
                                                                                             !!
        ...
        real(kind=dp), dimension(16), parameter :: wgk = [ &
                                              5.37747987292334898779205143012764981831e-3_dp, &
                                              1.50079473293161225383747630758072680946e-2_dp, &
                                              2.54608473267153201868740010196533593973e-2_dp, &
                                              3.53463607913758462220379484783600481226e-2_dp, &
                                              4.45897513247648766082272993732796902233e-2_dp, &
                                              5.34815246909280872653431472394302967716e-2_dp, &
                                              6.20095678006706402851392309608029321904e-2_dp, &
                                              6.98541213187282587095200770991474757860e-2_dp, &
                                              7.68496807577203788944327774826590067221e-2_dp, &
                                              8.30805028231330210382892472861037896016e-2_dp, &
                                              8.85644430562117706472754436937743032123e-2_dp, &
                                              9.31265981708253212254868727473457185619e-2_dp, &
                                              9.66427269836236785051799076275893351367e-2_dp, &
                                              9.91735987217919593323931734846031310596e-2_dp, &
                                              1.00769845523875595044946662617569721916e-1_dp, &
                                              1.01330007014791549017374792767492546771e-1_dp] !! weights of the 31-point kronrod rule
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd).
    ...
    subroutine dqk41(f, a, b, Result, Abserr, Resabs, Resasc, user_data)
        implicit none
        ...
        real(kind=dp), intent(out) :: Abserr !! estimate of the modulus of the absolute error,
                                        !! which should not exceed `abs(i-result)`
        real(kind=dp), intent(out) :: Resabs !! approximation to the integral j
        real(kind=dp), intent(out) :: Resasc !! approximation to the integal of abs(f-i/(b-a))
                                        !! over `(a,b)`
        class(*),      intent(in)  :: user_data
        real(kind=dp) :: dhlgth, fc, fsum, fv1(20), fv2(20)
        integer(kind=i4) :: j, jtw, jtwm1
        real(kind=dp) :: centr !! mid point of the interval
        real(kind=dp) :: hlgth !! half-length of the interval
        real(kind=dp) :: absc !! abscissa
        real(kind=dp) :: fval1 !! function value
        real(kind=dp) :: fval2 !! function value
        real(kind=dp) :: resg !! result of the 20-point gauss formula
        real(kind=dp) :: resk !! result of the 41-point kronrod formula
        real(kind=dp) :: reskh !! approximation to mean value of `f` over `(a,b)`, i.e. to `i/(b-a)`

        ! the abscissae and weights are given for the interval (-1,1).
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd).
    ...
    subroutine dqk51(f, a, b, Result, Abserr, Resabs, Resasc, user_data)
        implicit none
        ...
        real(kind=dp) :: fval1 !! function value
        real(kind=dp) :: fval2 !! function value
        real(kind=dp) :: resg !! result of the 25-point gauss formula
        real(kind=dp) :: resk !! result of the 51-point kronrod formula
        real(kind=dp) :: reskh !! approximation to the mean value of `f` over `(a,b)`, i.e. to `i/(b-a)`

        real(kind=dp) :: dhlgth, fc, fsum, fv1(25), fv2(25)
        integer(kind=i4) :: j, jtw, jtwm1
        ...
        real(kind=dp), dimension(13), parameter :: wg = [ &
                                              1.13937985010262879479029641132347736033e-2_dp, &
                                              2.63549866150321372619018152952991449360e-2_dp, &
                                              4.09391567013063126556234877116459536608e-2_dp, &
                                              5.49046959758351919259368915404733241601e-2_dp, &
                                              6.80383338123569172071871856567079685547e-2_dp, &
                                              8.01407003350010180132349596691113022902e-2_dp, &
                                              9.10282619829636498114972207028916533810e-2_dp, &
                                              1.00535949067050644202206890392685826988e-1_dp, &
                                              1.08519624474263653116093957050116619340e-1_dp, &
                                              1.14858259145711648339325545869555808641e-1_dp, &
                                              1.19455763535784772228178126512901047390e-1_dp, &
                                              1.22242442990310041688959518945851505835e-1_dp, &
                                              1.23176053726715451203902873079050142438e-1_dp] !! weights of the 25-point gauss rule

        real(kind=dp), dimension(26), parameter :: xgk = [ &
                                              9.99262104992609834193457486540340593705e-1_dp, &
                                              9.95556969790498097908784946893901617258e-1_dp, &
                                              9.88035794534077247637331014577406227072e-1_dp, &
                                              9.76663921459517511498315386479594067745e-1_dp, &
                                              9.61614986425842512418130033660167241692e-1_dp, &
                                              9.42974571228974339414011169658470531905e-1_dp, &
                                              9.20747115281701561746346084546330631575e-1_dp, &
                                              8.94991997878275368851042006782804954175e-1_dp, &
                                              8.65847065293275595448996969588340088203e-1_dp, &
                                              8.33442628760834001421021108693569569461e-1_dp, &
                                              7.97873797998500059410410904994306569409e-1_dp, &
                                              7.59259263037357630577282865204360976388e-1_dp, &
                                              7.17766406813084388186654079773297780598e-1_dp, &
                                              6.73566368473468364485120633247622175883e-1_dp, &
                                              6.26810099010317412788122681624517881020e-1_dp, &
                                              5.77662930241222967723689841612654067396e-1_dp, &
                                              5.26325284334719182599623778158010178037e-1_dp, &
                                              4.73002731445714960522182115009192041332e-1_dp, &
                                              4.17885382193037748851814394594572487093e-1_dp, &
                                              3.61172305809387837735821730127640667422e-1_dp, &
                                              3.03089538931107830167478909980339329200e-1_dp, &
                                              2.43866883720988432045190362797451586406e-1_dp, &
                                              1.83718939421048892015969888759528415785e-1_dp, &
                                              1.22864692610710396387359818808036805532e-1_dp, &
                                              6.15444830056850788865463923667966312817e-2_dp, &
                                              0.00000000000000000000000000000000000000e0_dp] !! abscissae of the 51-point kronrod rule
                                                                                             !!
        ...
        real(kind=dp), dimension(26), parameter :: wgk = [ &
                                              1.98738389233031592650785188284340988943e-3_dp, &
                                              5.56193213535671375804023690106552207018e-3_dp, &
                                              9.47397338617415160720771052365532387165e-3_dp, &
                                              1.32362291955716748136564058469762380776e-2_dp, &
                                              1.68478177091282982315166675363363158404e-2_dp, &
                                              2.04353711458828354565682922359389736788e-2_dp, &
                                              2.40099456069532162200924891648810813929e-2_dp, &
                                              2.74753175878517378029484555178110786148e-2_dp, &
                                              3.07923001673874888911090202152285856009e-2_dp, &
                                              3.40021302743293378367487952295512032257e-2_dp, &
                                              3.71162714834155435603306253676198759960e-2_dp, &
                                              4.00838255040323820748392844670756464014e-2_dp, &
                                              4.28728450201700494768957924394951611020e-2_dp, &
                                              4.55029130499217889098705847526603930437e-2_dp, &
                                              4.79825371388367139063922557569147549836e-2_dp, &
                                              5.02776790807156719633252594334400844406e-2_dp, &
                                              5.23628858064074758643667121378727148874e-2_dp, &
                                              5.42511298885454901445433704598756068261e-2_dp, &
                                              5.59508112204123173082406863827473468203e-2_dp, &
                                              5.74371163615678328535826939395064719948e-2_dp, &
                                              5.86896800223942079619741758567877641398e-2_dp, &
                                              5.97203403241740599790992919325618538354e-2_dp, &
                                              6.05394553760458629453602675175654271623e-2_dp, &
                                              6.11285097170530483058590304162927119227e-2_dp, &
                                              6.14711898714253166615441319652641775865e-2_dp, &
                                              6.15808180678329350787598242400645531904e-2_dp] !! weights of the 51-point kronrod rule.
...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd).
    ...
    subroutine dqk61(f, a, b, Result, Abserr, Resabs, Resasc, user_data)
        implicit none
        ...
        real(kind=dp), intent(out) :: Resasc !! approximation to the integral of `abs(f-i/(b-a))`
        class(*),      intent(in)  :: user_data
        real(kind=dp) :: dhlgth, fc, fsum, fv1(30), fv2(30)
        integer(kind=i4) :: j, jtw, jtwm1
        real(kind=dp) :: centr !! mid point of the interval
        real(kind=dp) :: hlgth !! half-length of the interval
        real(kind=dp) :: absc !! abscissa
        real(kind=dp) :: fval1 !! function value
        real(kind=dp) :: fval2 !! function value
        real(kind=dp) :: resg !! result of the 30-point gauss rule
        real(kind=dp) :: resk !! result of the 61-point kronrod rule
        real(kind=dp) :: reskh !! approximation to the mean value of `f` over `(a,b)`, i.e. to `i/(b-a)`
        ...
        real(kind=dp), dimension(15), parameter :: wg = [ &
                                              7.96819249616660561546588347467362245048e-3_dp, &
                                              1.84664683110909591423021319120472690962e-2_dp, &
                                              2.87847078833233693497191796112920436396e-2_dp, &
                                              3.87991925696270495968019364463476920332e-2_dp, &
                                              4.84026728305940529029381404228075178153e-2_dp, &
                                              5.74931562176190664817216894020561287971e-2_dp, &
                                              6.59742298821804951281285151159623612374e-2_dp, &
                                              7.37559747377052062682438500221907341538e-2_dp, &
                                              8.07558952294202153546949384605297308759e-2_dp, &
                                              8.68997872010829798023875307151257025768e-2_dp, &
                                              9.21225222377861287176327070876187671969e-2_dp, &
                                              9.63687371746442596394686263518098650964e-2_dp, &
                                              9.95934205867952670627802821035694765299e-2_dp, &
                                              1.01762389748405504596428952168554044633e-1_dp, &
                                              1.02852652893558840341285636705415043868e-1_dp] !! weigths of the 30-point gauss rule

        real(kind=dp), dimension(31), parameter :: xgk = [ &
                                              9.99484410050490637571325895705810819469e-1_dp, &
                                              9.96893484074649540271630050918695283341e-1_dp, &
                                              9.91630996870404594858628366109485724851e-1_dp, &
                                              9.83668123279747209970032581605662801940e-1_dp, &
                                              9.73116322501126268374693868423706884888e-1_dp, &
                                              9.60021864968307512216871025581797662930e-1_dp, &
                                              9.44374444748559979415831324037439121586e-1_dp, &
                                              9.26200047429274325879324277080474004086e-1_dp, &
                                              9.05573307699907798546522558925958319569e-1_dp, &
                                              8.82560535792052681543116462530225590057e-1_dp, &
                                              8.57205233546061098958658510658943856821e-1_dp, &
                                              8.29565762382768397442898119732501916439e-1_dp, &
                                              7.99727835821839083013668942322683240736e-1_dp, &
                                              7.67777432104826194917977340974503131695e-1_dp, &
                                              7.33790062453226804726171131369527645669e-1_dp, &
                                              6.97850494793315796932292388026640068382e-1_dp, &
                                              6.60061064126626961370053668149270753038e-1_dp, &
                                              6.20526182989242861140477556431189299207e-1_dp, &
                                              5.79345235826361691756024932172540495907e-1_dp, &
                                              5.36624148142019899264169793311072794164e-1_dp, &
                                              4.92480467861778574993693061207708795644e-1_dp, &
                                              4.47033769538089176780609900322854000162e-1_dp, &
                                              4.00401254830394392535476211542660633611e-1_dp, &
                                              3.52704725530878113471037207089373860654e-1_dp, &
                                              3.04073202273625077372677107199256553531e-1_dp, &
                                              2.54636926167889846439805129817805107883e-1_dp, &
                                              2.04525116682309891438957671002024709524e-1_dp, &
                                              1.53869913608583546963794672743255920419e-1_dp, &
                                              1.02806937966737030147096751318000592472e-1_dp, &
                                              5.14718425553176958330252131667225737491e-2_dp, &
                                              0.00000000000000000000000000000000000000e0_dp] !! abscissae of the 61-point kronrod rule:
                                                                                             !!
        ...
        real(kind=dp), dimension(31), parameter :: wgk = [ &
...
!  * QUADPACK: date written 820101, revision date 830518 (yymmdd).
    ...
    subroutine dqmomo(Alfa, Beta, Ri, Rj, Rg, Rh, Integr)
        ...
        real(kind=dp), intent(in) :: Alfa !! parameter in the weight function `w(x)`, `alfa>(-1)`
        real(kind=dp), intent(in) :: Beta !! parameter in the weight function `w(x)`, `beta>(-1)`
        real(kind=dp), intent(out) :: Ri(25) !! `i(k)` is the integral over (-1,1) of
                                        !! `(1+x)**alfa*t(k-1,x), k = 1, ..., 25`.
        real(kind=dp), intent(out) :: Rj(25) !! `rj(k)` is the integral over (-1,1) of
                                        !! `(1-x)**beta*t(k-1,x), k = 1, ..., 25`.
        real(kind=dp), intent(out) :: Rg(25) !! `rg(k)` is the integral over (-1,1) of
                                        !! `(1+x)**alfa*log((1+x)/2)*t(k-1,x), k = 1, ..., 25`.
        real(kind=dp), intent(out) :: Rh(25) !! `rh(k)` is the integral over (-1,1) of
        ...
        real(kind=dp) :: alfp1, alfp2, an, anm1, betp1, betp2, ralf, rbet
...
!    kahaner,david,nbs - modified (2/82)
    ...
    subroutine dqng(f, a, b, Epsabs, Epsrel, Result, Abserr, Neval, Ier, user_data)
        implicit none
        ...
        class(*),        intent(in) :: user_data
        real(kind=dp) :: dhlgth, fval1, fval2, fv1(5), fv2(5), fv3(5), fv4(5), reskh
        integer(kind=i4) :: ipx, k, l
        real(kind=dp) :: centr !! mid point of the integration interval
        real(kind=dp) :: hlgth !! half-length of the integration interval
        real(kind=dp) :: fcentr !! function value at mid point
        real(kind=dp) :: absc !! abscissa
        real(kind=dp) :: fval !! function value
        real(kind=dp) :: savfun(21) !! array of function values which have already been computed
        real(kind=dp) :: res10 !! 10-point gauss result
        real(kind=dp) :: res21 !! 21-point kronrod result
        real(kind=dp) :: res43 !! 43-point result
        real(kind=dp) :: res87 !! 87-point result
        real(kind=dp) :: resabs !! approximation to the integral of `abs(f)`
        real(kind=dp) :: resasc !! approximation to the integral of `abs(f-i/(b-a))`

        ! the following data statements contain the
        ! abscissae and weights of the integration rules used.

        real(kind=dp), dimension(5), parameter :: x1 = [ &
                                             9.73906528517171720077964012084452053428e-1_dp, &
                                             8.65063366688984510732096688423493048528e-1_dp, &
                                             6.79409568299024406234327365114873575769e-1_dp, &
                                             4.33395394129247190799265943165784162200e-1_dp, &
                                             1.48874338981631210884826001129719984618e-1_dp] !! abscissae common to the 10-, 21-, 43- and 87-point rule

        real(kind=dp), dimension(5), parameter :: w10 = [ &
                                             6.66713443086881375935688098933317928579e-2_dp, &
                                             1.49451349150580593145776339657697332403e-1_dp, &
                                             2.19086362515982043995534934228163192459e-1_dp, &
                                             2.69266719309996355091226921569469352860e-1_dp, &
                                             2.95524224714752870173892994651338329421e-1_dp] !! weights of the 10-point formula

        real(kind=dp), dimension(5), parameter :: x2 = [ &
                                             9.95657163025808080735527280689002847921e-1_dp, &
                                             9.30157491355708226001207180059508346225e-1_dp, &
                                             7.80817726586416897063717578345042377163e-1_dp, &
                                             5.62757134668604683339000099272694140843e-1_dp, &
                                             2.94392862701460198131126603103865566163e-1_dp] !! abscissae common to the 21-, 43- and 87-point rule

        real(kind=dp), dimension(5), parameter :: w21a = [ &
                                             3.25581623079647274788189724593897606174e-2_dp, &
                                             7.50396748109199527670431409161900093952e-2_dp, &
                                             1.09387158802297641899210590325804960272e-1_dp, &
                                             1.34709217311473325928054001771706832761e-1_dp, &
                                             1.47739104901338491374841515972068045524e-1_dp] !! weights of the 21-point formula for abscissae x1

        real(kind=dp), dimension(6), parameter :: w21b = [ &
                                             1.16946388673718742780643960621920483962e-2_dp, &
                                             5.47558965743519960313813002445801763737e-2_dp, &
                                             9.31254545836976055350654650833663443900e-2_dp, &
                                             1.23491976262065851077958109831074159512e-1_dp, &
                                             1.42775938577060080797094273138717060886e-1_dp, &
                                             1.49445554002916905664936468389821203745e-1_dp] !! weights of the 21-point formula for abscissae x2
        ...
        real(kind=dp), dimension(11), parameter :: x3 = [ &
        ...
        real(kind=dp), dimension(10), parameter :: w43a = [ &
        ...
        real(kind=dp), dimension(12), parameter :: w43b = [ &
        ...
        real(kind=dp), dimension(22), parameter :: x4 = [ &
        ...
        real(kind=dp), dimension(21), parameter :: w87a = [ &
        ...
        real(kind=dp), dimension(23), parameter :: w87b = [ &
        ...
        if (Epsabs > 0.0_dp .or. Epsrel >= max(50.0_dp*epmach, 0.5e-28_dp)) &
            ...
            do l = 1, 3
                ...
                case (3)
                    ...
                    do k = 1, 22
                        absc = hlgth*x4(k)
                        res87 = res87 + w87b(k)*(f(absc + centr,user_data) + f(centr - absc,user_data))
                    end do
                    Result = res87*hlgth
...
!  * QUADPACK: revision date 810101 (yymmdd)
    ...
    subroutine dqpsrt(Limit, Last, Maxerr, Ermax, Elist, Iord, Nrmax)
        ...
        real(kind=dp), intent(out) :: Ermax !! `nrmax`-th largest error estimate
                                       !! `ermax = elist(maxerr)`
        real(kind=dp), intent(in) :: Elist(Last) !! vector of dimension `last` containing
                                            !! the error estimates
...
!  * weight function, cauchy principal value
    ...
    real(kind=dp) function dqwgtc(x, c, p2, p3, p4, Kp)
        implicit none
...
!  * QUADPACK: revision date 810101 (yymmdd)
    ...
    real(kind=dp) function dqwgtf(x, Omega, p2, p3, p4, Integr)
        implicit none
...
!  * QUADPACK: revision date 810101 (yymmdd)
    ...
    real(kind=dp) function dqwgts(x, a, b, Alfa, Beta, Integr)
        implicit none
...
!    jack dongarra, argonne national laboratory.
    ...
    subroutine dgtsl(n, c, d, e, b, info)
        ...
        integer(kind=i4), intent(out) :: info !! * = 0 normal value.
                                     ...
                                     !!   this is detected.
        real(kind=dp), intent(inout) :: c(n) !! the subdiagonal of the tridiagonal matrix.
                                        !! `c(2)` through `c(n) `should contain the subdiagonal.
                                        !! on output `c` is destroyed.
        real(kind=dp), intent(inout) :: d(n) !! the diagonal of the tridiagonal matrix.
                                        !! on output `d` is destroyed.
        real(kind=dp), intent(inout) :: e(n) !! the superdiagonal of the tridiagonal matrix.
                                        !! `e(1)` through `e(n-1)` should contain the superdiagonal.
                                        !! on output `e` is destroyed.
        real(kind=dp), intent(inout) :: b(n) !! input: is the right hand side vector..
...
subroutine dquad(f, a, b, result, epsil, npts, icheck)
    ...
    real(kind=dp),intent(in) :: a !! lower limit of integration.
    real(kind=dp),intent(in) :: b !! upper limit of integration.
    real(kind=dp),intent(out) :: result !! the value of the integral to the
                                   !! specified relative accuracy.
    real(kind=dp),intent(in) :: epsil !! relative accuracy required. when the relative
                                 !! difference of two successive formulae does not
                                 !! exceed `epsil` the last formula computed is taken
                                 !! as the result.
    integer(kind=i4),intent(out) :: npts !! number integrand evaluations.
    integer(kind=i4),intent(out) :: icheck !! on exit normally `icheck=0`. however if convergence
                                  !! to the accuracy requested is not achieved `icheck=1`
                                  !! on exit.

    real(kind=dp) :: acum, diff, funct(127), fzero, sum, x
    integer(kind=i4) :: i, inew, iold, j
    real(kind=dp),dimension(8) :: results !! this array holds the results obtained by
                                     !! the 1,3,7, etc., point formulae. the number of
    real(kind=dp),intent(out) :: err !! a requested error tolerance.  Normally, pick a value
                                !! `0 < ERR < 1.0e-8`.
    real(kind=dp),intent(out) :: ans !! computed value of the integral.  Hopefully, `ANS` is
    ...
    integer(kind=i4),intent(out) :: k !! the number of function evaluations actually used to do
    ...
    real(kind=dp),parameter :: w1 = 41.0_dp/140.0_dp
    real(kind=dp),parameter :: w2 = 216.0_dp/140.0_dp
    real(kind=dp),parameter :: w3 = 27.0_dp/140.0_dp
    real(kind=dp),parameter :: w4 = 272.0_dp/140.0_dp
    real(kind=dp),parameter :: sq2 = sqrt(2.0_dp)
    real(kind=dp),parameter :: ln2 = log(2.0_dp)
    integer(kind=i4),parameter :: nbits = int(d1mach(5)*i1mach14/0.30102000_dp)  !! is 0.30102000 supposed to be log10(2.0_dp) ???
    integer(kind=i4),parameter :: nlmx = min(99, (nbits*4)/5)
    integer(kind=i4),parameter :: nlmn = 2
    integer(kind=i4),parameter :: kml = 7
    integer(kind=i4),parameter :: kmx = 5000      !! JW : is this the max function evals? should be an input
    integer(kind=i4),parameter :: array_size = 99 !! JW : what is this magic number 99 array size ??
                                         !! does it depend on the number of function evals ?
                                         !! (see comment in revision history)

    real(kind=dp) :: ae,area,bank,blocal,c,ce,ee,ef,eps,q13,q7,q7l,test,tol,vr
    integer(kind=i4) :: i,l,lmn,lmx,nib
    real(kind=dp),dimension(13) :: f
    real(kind=dp),dimension(array_size) :: aa,f1,f2,f3,f4,f5,f6,f7,hh,q7r,vl
    integer(kind=i4),dimension(array_size) :: lr
    ...
    if ( b/=0.0_dp ) then
        if ( sign(1.0_dp,b)*a>0.0_dp ) then
            c = abs(1.0_dp-a/b)
            if ( c<=0.1_dp ) then
                if ( c<=0.0_dp ) goto 400
                nib = 0.5_dp - log(c)/ln2
    ...
    tol = max(abs(err),2.0_dp**(5-nbits))
    if ( err==0.0_dp ) tol = sqrt(epmach)
    eps = tol
    hh(1) = (b-a)/12.0_dp
    aa(1) = a
    lr(1) = 1
...
120         if ( l<=1 ) then
                !   exit
                ans = vr
                if ( abs(ce)>2.0_dp*tol*area ) then
                    ierr = 2
...
!  * Jacob Williams : Jan 2022 : refactored SLATEC routine to modern Fortran.
    ...
    procedure(func) :: f !! function subprogram defining the integrand function `f(x)`.
    real(kind=dp),intent(in)   :: a          !! lower bound of the integration
    real(kind=dp),intent(in)   :: b          !! upper bound of the integration
    real(kind=dp),intent(in)   :: error_tol  !! is a requested pseudorelative error tolerance.  normally
    ...
    real(kind=dp),intent(out)  :: ans        !! computed value of integral
    integer(kind=i4),intent(out)   :: ierr       !! status code:
                                        !!
                                        !!  * normal codes:
                                        !!    * 1 : `ans` most likely meets requested error tolerance,
                                        !!      or `a=b`.
                                        ...
                                        !!  * abnormal code:
                                        !!    * 2 : `ans` probably does not meet requested error tolerance.
    real(kind=dp),intent(out)  :: err        !! an estimate of the absolute error in `ans`.
    ...
    real(kind=dp),parameter  :: sq2    = sqrt(2.0_dp)
    real(kind=dp),parameter  :: ln2    = log(2.0_dp)
    integer(kind=i4),parameter   :: kmx    = 5000
    integer(kind=i4),parameter   :: kml    = 6
    real(kind=dp),parameter  :: magic  = 0.30102000_dp   !! is 0.30102000 supposed to be log10(2.0_dp) ???
    integer(kind=i4),parameter   :: iwork  = 60              !! size of the work arrays. ?? Why 60 ??
    integer(kind=i4),parameter   :: nbits  = int(d1mach(5)*i1mach14/magic)
    integer(kind=i4),parameter   :: nlmn   = 1
    integer(kind=i4),parameter   :: nlmx   = min(60,(nbits*5)/8)

    integer(kind=i4) :: k !! number of function evaluations
    integer(kind=i4)                   :: l,lmn,lmx,mxl,nib
    real(kind=dp)                  :: ae,area,c,ee,ef,eps,est,gl,glr,tol
    real(kind=dp),dimension(iwork) :: aa,hh,vl,gr
    integer(kind=i4),dimension(iwork)  :: lr
    ...
    if (error_tol == 0.0_dp) then
        tol = sqrt(epmach)
    else
        tol = max(abs(error_tol),2.0_dp**(5-nbits))/2.0_dp
    end if
    eps = tol
    hh(1) = (b-a)/4.0_dp
    aa(1) = a
    lr(1) = 1
    l = 1
    est = g(aa(l)+2.0_dp*hh(l),2.0_dp*hh(l))
    k = 8
    area = abs(est)
    ef = 0.5_dp
    mxl = 0
    ...
    main : do
        ...
        gl = g(aa(l)+hh(l),hh(l))
        gr(l) = g(aa(l)+3.0_dp*hh(l),hh(l))
        k = k + 16
        area = area + (abs(gl)+abs(gr(l))-abs(est))
        glr = gl + gr(l)
        ee = abs(est-glr)*ef
        ae = max(eps*area,tol*abs(glr))
        if (ee-ae > 0.0_dp) then
    ...
    if ((mxl/=0) .and. (abs(err)>2.0_dp*tol*area)) ierr = 2 ! ans is probably insufficiently accurate
    ...
    !      http://processingjs.nihongoresources.com/bezierinfo/legendre-gauss-values.php
        ...
        !> abscissae:
        real(kind=dp),parameter :: x1 = 0.18343464249564980493947614236018398066675781291297378231718847_dp
        real(kind=dp),parameter :: x2 = 0.52553240991632898581773904918924634904196424312039285775085709_dp
        real(kind=dp),parameter :: x3 = 0.79666647741362673959155393647583043683717173161596483207017029_dp
        real(kind=dp),parameter :: x4 = 0.96028985649753623168356086856947299042823523430145203827163977_dp

        !> weights:
        real(kind=dp),parameter :: w1 = 0.36268378337836198296515044927719561219414603989433054052482306_dp
        real(kind=dp),parameter :: w2 = 0.31370664587788728733796220198660131326032899900273493769026394_dp
        real(kind=dp),parameter :: w3 = 0.22238103445337447054435599442624088443013087005124956472590928_dp
        real(kind=dp),parameter :: w4 = 0.10122853629037625915253135430996219011539409105168495705900369_dp
...
!    BIT Vol. 40, No. 1, March 2000, pp. 84--101.
    ...
    real(kind=dp),intent(in)  :: a !! lower bound of the integration
    real(kind=dp),intent(in)  :: b !! upper bound of the integration
    real(kind=dp),intent(in)  :: error_tol !! relative error tolerance
    real(kind=dp),intent(out) :: ans !! computed value of integral
    integer(kind=i4),intent(out)  :: ierr !! status code:
                                 !!
                                 !! * 1 = success
    ...
    real(kind=dp) :: bma,is,tol,fa,fm,fb
    real(kind=dp),dimension(5) :: yy
    integer(kind=i4) :: k !! number of calls to the recursive function

    real(kind=dp),parameter :: eps = epsilon(1.0_dp)
    real(kind=dp),dimension(5),parameter :: c = [.9501_dp, .2311_dp, .6068_dp, .4860_dp, .8913_dp]
    integer(kind=i4),parameter :: kmax = 10000 !! maximum number of calls to the recursive function (probably should be an input)
    ...
    recursive subroutine adaptive_simpson_step (a,b,fa,fm,fb,is,ans)
        ...
        real(kind=dp),intent(in)   :: a
        real(kind=dp),intent(in)   :: b
        real(kind=dp),intent(in)   :: fa
        real(kind=dp),intent(in)   :: fm
        real(kind=dp),intent(in)   :: fb
        real(kind=dp),intent(in)   :: is
        real(kind=dp),intent(out)  :: ans

        real(kind=dp) :: m,h,fml,fmr,i1,i2,q1,q2
        ...
        fml = f(a + h)
        fmr = f(b - h)
        i1 = h/1.5_dp * (fa + 4.0_dp*fm + fb)
        i2 = h/3.0_dp * (fa + 4.0_dp*(fml + fmr) + 2.0_dp*fm + fb)
        i1 = (16.0_dp*i2 - i1)/15.0_dp
...
!    BIT Vol. 40, No. 1, March 2000, pp. 84--101.
    ...
    real(kind=dp) :: m,h,s,erri1,erri2,is,tol,fa,fb,i1,i2,r
    real(kind=dp),dimension(13) :: x,y
    integer(kind=i4) :: i
    integer(kind=i4) :: k !! number of calls to the recursive function

    integer(kind=i4),parameter :: kmax = 10000 !! maximum number of calls to the recursive function (probably should be an input)
    real(kind=dp),parameter :: eps  = epsilon(1.0_dp)
    real(kind=dp),parameter :: alpha = sqrt(2.0_dp/3.0_dp)
    real(kind=dp),parameter :: beta  = 1.0_dp/sqrt(5.0_dp)
    real(kind=dp),parameter :: x1   = .942882415695480_dp
    real(kind=dp),parameter :: x2   = .641853342345781_dp
    real(kind=dp),parameter :: x3   = .236383199662150_dp
    real(kind=dp),dimension(7) :: c = [.0158271919734802_dp ,&
                                  .0942738402188500_dp ,&
                                  .155071987336585_dp ,&
                                  .188821573960182_dp ,&
                                  .199773405226859_dp ,&
    ...
    fa=y(1)
    fb=y(13)
    i2=(h/6.0_dp)*(y(1)+y(13)+5.0_dp*(y(5)+y(9)))
    i1=(h/1470.0_dp)*(77.0_dp*(y(1)+y(13))+432.0_dp*(y(3)+y(11))+625.0_dp*(y(5)+y(9))+672.0_dp*y(7))
    ...
    erri2 = abs(i2-is)
    r = 1.0_dp
    if (erri2/=0.0_dp) r=erri1/erri2
    if (r>0.0_dp .and. r<1.0_dp) tol=tol/r
    is=s*abs(is)*tol/eps
    if (is==0.0_dp) is=b-a
    ...
    recursive subroutine adaptive_lobatto_step(a,b,fa,fb,is,ans)
        ...
        real(kind=dp) :: h,m,mll,ml,mr,mrr,fmll,fml,fm,fmr,fmrr,i2,i1
        real(kind=dp),dimension(6) :: q
        ...
        i2 = (h/6.0_dp)*(fa+fb+5.0_dp*(fml+fmr))
        i1 = (h/1470.0_dp)*(77.0_dp*(fa+fb)+432.0_dp*(fmll+fmrr)+625.0_dp*(fml+fmr)+672.0_dp*fm)
