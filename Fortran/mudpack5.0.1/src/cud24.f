...
c ... purpose
c
c     cud24 attempts to improve the second order approximation generated
c     by cud2 to a fourth order approximation using difference corrections
...
c     cud2.f, cudcom.f
c
      subroutine cud24(work,phi,ierror)
      implicit none
      complex phi(*),work(*)
     ...
     +nxk(50),nyk(50),isx,jsy
      ...
      ierror = 0
c
c     check adequate grid size for fourth-order
c
      nx = nfx
      ny = nfy
	...
	return
      ...
      call cu24(nx,ny,phi,work)
      end

      subroutine cu24(nx,ny,phi,wk)
      implicit none
      integer nx,ny
      integer intl,nxa,nxb,nyc,nyd,ixp,jyq,iex,jey,nfx,nfy,iguess,
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
...
c
C$OMP PARALLEL DO PRIVATE(i,j,ux3,ux4,uy3,uy4,cxx,cx,cyy,cy,tx,ty)
C$OMP+SHARED (u,cof,nx,ny)
C$OMP+SHARED (dlx,dly,dlxx,dlyy,dlx4,dly4,tdlx3,tdly3)
      do j=3,ny-2
	do i=3,nx-2
