...
c ... purpose
c
c     cuh24cr attempts to improve the second order approximation generated
c     by cuh2cr to a fourth order approximation using difference corrections
...
c
c
      subroutine cuh24cr(wk,iwk,coef,bndyc,phi,ierror)
      implicit none
      complex wk(*),phi(*)
      integer iwk(*),ierror
	...
	return
      ...
      call ch24cr(nx,ny,phi,coef,bndyc,wk,iwk)
      end

      subroutine ch24cr(nx,ny,phi,coef,bndyc,wk,iwk)
      implicit none
...
c
      ...
      call ctrh2cr(nx,ny,phi,wk(icf),wk(irf),coef,bndyc)
c
...
c
	...
	do i=1,nx
      ...
      subroutine ctrh2cr(nx,ny,phi,cof,frhs,coef,bndyc)
c
...
c
     ...
     +               dxxxy4,dxyyy4,dxxyy,dlxxx,dlyyy
      ...
      complex c1,c2,c3,c4,c5,c6,c7,c8,px4,py4,px3,py3
      complex px3y,px2y2,pxy3,px3im1,py3jm1,px3jm1,py3im1
      external coef,bndyc
c
...
c
      call ctrh2crd(nx,ny,phi,cof,frhs)
c
...
c     at (xa,yd)
      ...
      subroutine ctrh2crd(nx,ny,u,cof,frhs)
c
...
C$OMP PARALLEL DO PRIVATE(i,j,px3,px4,py3,py4,px3y,pxy3)
C$OMP+PRIVATE(cxx,cx,cyy,cy,cxy,tx,ty,txx,tyy,txy)
C$OMP+SHARED(u,cof,nx,ny)
C$OMP+SHARED (dlx,dly,dlxx,dlyy,dlx4,dly4,tdlx3,tdly3,dxxxy4,dxyyy4)
      do j=3,ny-2
	do i=3,nx-2
c
