...
c
C$OMP PARALLEL DO PRIVATE(i,j,k), SHARED(phi,nx,ny,nz)
C$OMP+SHARED(ist,ifn,jst,jfn,kst,kfn,cofx,cofy,cofz,rhs)
      do k=kst,kfn
	do j=jst,jfn
	  do i=ist,ifn
...
C$OMP PARALLEL DO SHARED(ist,ifn,jst,jfn,kst,kfn,phi,rhs,cofx,cofy,cofz)
C$OMP+PRIVATE(i,j,k)
      do k=kst,kfn,2
	do i=ist,ifn,2
	  do j=jst,jfn,2
...
c
C$OMP PARALLEL DO SHARED(ist,ifn,jst,jfn,kfn,phi,rhs,cofx,cofy,cofz)
C$OMP+PRIVATE(i,j,k)
      do k=2,kfn,2
	do i=ist,ifn,2
	  do j=2,jfn,2
...
c
C$OMP PARALLEL DO SHARED(ist,ifn,jfn,kst,kfn,phi,rhs,cofx,cofy,cofz)
C$OMP+PRIVATE(i,j,k)
      do k=kst,kfn,2
	do i=ist,ifn,2
	  do j=2,jfn,2
