...
c ... purpose
c
c     cud2 attempts to produce a second order finite difference
c     approximation to the COMPLEX two dimensional nonseparable
...
c
c      using multigrid iteration.  The known coefficients
c      cxx,cyy,cx,cy,ce; the known righthand side r(x,y);
c      and the unknown solution function p(x,y) are all
c      complex valued functions of the real indepedent variables
c      x and y.
c
...
c
c
      subroutine cud2(iparm,fparm,work,coef,bndyc,rhs,phi,mgopt,ierror)
      implicit none
...
c         ip = kpbgn(k)
	  ...
	  ity = ktybgn(k)
	  klevel = k
	  call discd2(nx,ny,work(ic),work(itx),work(ity),
	...
	return
      ...
      nx = nfx
      ny = nfy
      call cud21(nx,ny,rhs,phi,coef,bndyc,work)
      iparm(17) = itero
	...
	if (relmax.gt.tolmax) ierror = -1   ! flag convergenc failure
      ...
      subroutine cud21(nx,ny,rhsf,phif,coef,bndyc,wk)
      implicit none
      integer nx,ny
...
c
	do k=1,ngrid
	  ...
	  ip = kpbgn(k)
	  ic = kcbgn(k)
	  call adjcd2(nx,ny,wk(ip),wk(ic),bndyc,coef)
	end do
c
...
c
      do l=1,iprer
	call relcd2(nx,ny,wk(ip),wk(ic),wk(itx),wk(ity),wk(kps))
      end do
...
c
      ...
      call rescd2(nx,ny,wk(ip),ncx,ncy,wk(ipc),wk(irc),wk(ic),wk(kps))
c
...
c
	  ...
	  ncy = nyk(klevel-1)
	  irc = kcbgn(klevel-1)+5*ncx*ncy
	  call rescd2(nx,ny,wk(ip),ncx,ncy,wk(ipc),wk(irc),wk(ic),
     +                wk(kps))
c
...
c
	...
	call relcd2(nx,ny,wk(ip),wk(ic),wk(itx),wk(ity),wk(kps))
      ...
      subroutine discd2(nx,ny,cof,tx,ty,bndyc,coef,wk,ier)
c
...
c
     ...
     +                  ty(1,1,5),wk(kps))
      ...
      subroutine adjcd2(nx,ny,phi,cof,bndyc,coef)
c
...
c
	...
	do i=1,nx
      ...
      subroutine rescd2(nx,ny,phi,ncx,ncy,phic,rhsc,cof,resf)
c
...
c
      ...
      subroutine relcd2(nx,ny,phi,cof,tx,ty,sum)
c
...
c
	...
	call relcd2p(nx,ny,phi,cof)
      else if (method.eq.1) then           ! line x relaxation
	call slxcd2(nx,ny,phi,cof,tx,sum)
      else if (method.eq.2) then           ! line y relaxation
	call slycd2(nx,ny,phi,cof,ty,sum)
	...
	call slycd2(nx,ny,phi,cof,ty,sum)
      ...
      subroutine relcd2p(nx,ny,phi,cof)
c
...
c
	...
	do i=1,nx
      ...
      subroutine slxcd2(nx,ny,phi,cof,tx,sum)
c
...
c
	...
	do i=1,nx
      ...
      subroutine slycd2(nx,ny,phi,cof,ty,sum)
      implicit none
