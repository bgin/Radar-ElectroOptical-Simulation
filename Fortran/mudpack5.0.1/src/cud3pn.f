...
c ... purpose
c
c     cud3pn.f contains subroutines for planar relaxation in the x or
c     y or z direction.  This file must be loaded with the complex solver
c     cud3 or cuh3
c
      subroutine cplanxy(wk)
c
...
c
	  call ctrscxy(kset,nx,ny,nz,wk(ic),wk(icxy),wk(ip))
	  ixy = ixy+(6+isx+jsy)*nx*ny + (nx+2)*(ny+2)
	  ip = ip + (nx+2)*(ny+2)*(nz+2)
	  ic = ic + 8*nx*ny*nz
	end do
c
c     set 2-d solution for current kset for passage to cup2
c
	nx = nxk(klevel)
	ny = nyk(klevel)
	ip = kpbgn(klevel)
	ixy = 0
	call csetpxy(kset,nx,ny,nz,wk(ip),wk(kps),ixy)
c
c     solve on current z plane with full 2-d multigrid cycling
c
	call cup2(iparm,fparm,wk(kps),mgo)
c
c     reset approx from cup2 in wk(ip)
c
	ixy = 1
	call csetpxy(kset,nx,ny,nz,wk(ip),wk(kps),ixy)
      end do
...
c
	...
	ny = nyk(klevel)
	ip = kpbgn(klevel)
	call cper3vb(nx,ny,nz,wk(ip),nxa,nyc,nze)
      end if
      return
      end

      subroutine ctrscxy(kset,nx,ny,nz,cof,cofxy,phi)
c
...
c
      ...
      complex cof(nx,ny,nz,8),cofxy(nx,ny,6)
	...
	do i=1,nx
     ...
     +                                 cof(i,j,k,6)*phi(i,j,k+1))
      ...
      subroutine csetpxy(kset,nx,ny,nz,phi,phxy,ixy)
      implicit none
      integer kset,nx,ny,nz,i,j,ixy
      complex phi(0:nx+1,0:ny+1,0:nz+1),phxy(0:nx+1,0:ny+1)
      if (ixy.eq.0) then
	do j=0,ny+1
	  do i=0,nx+1
...
c
	  call ctrscxz(jset,nx,ny,nz,wk(ic),wk(icxz),wk(ip))
	  ixz = ixz+(6+isx+ksz)*nx*nz + (nx+2)*(nz+2)
	  ip = ip + (nx+2)*(ny+2)*(nz+2)
	  ic = ic + 8*nx*ny*nz
	end do
c
c     set 2-d solution for current jset for passage to cup2
c
	nx = nxk(klevel)
	nz = nzk(klevel)
	ip = kpbgn(klevel)
	ixz = 0
	call csetpxz(jset,nx,ny,nz,wk(ip),wk(kps),ixz)
c
...
c
	...
	call cper3vb(nx,ny,nz,wk(ip),nxa,nyc,nze)
      ...
      subroutine ctrscxz(jset,nx,ny,nz,cof,cofxz,phi)
c
c     transfer coefficients from 3-d to 2-d
c     to allow 2-d relaxation
c
      ...
      complex cof(nx,ny,nz,8),cofxz(nx,nz,6)
      complex phi(0:nx+1,0:ny+1,0:nz+1)
      j = jset
      do k=1,nz
	do i=1,nx
     ...
     +                                 cof(i,j,k,4)*phi(i,j+1,k))
      ...
      subroutine csetpxz(jset,nx,ny,nz,phi,phxz,ixz)
      implicit none
      integer jset,nx,ny,nz,i,k,ixz
      complex phi(0:nx+1,0:ny+1,0:nz+1),phxz(0:nx+1,0:nz+1)
      if (ixz.eq.0) then
	do k=0,nz+1
	  do i=0,nx+1
...
c
	  call ctrscyz(iset,nx,ny,nz,wk(ic),wk(icyz),wk(ip))
	  iyz = iyz+(6+jsy+ksz)*ny*nz + (ny+2)*(nz+2)
	  ip = ip + (nx+2)*(ny+2)*(nz+2)
	  ic = ic + 8*nx*ny*nz
	end do
c
c     set 2-d solution for current iset for passage to cup2
c
	ny = nyk(klevel)
	nz = nzk(klevel)
	ip = kpbgn(klevel)
	iyz = 0
	call csetpyz(iset,nx,ny,nz,wk(ip),wk(kps),iyz)
c
c     solve on current y plane with full 2-d multigrid cycling
c
	call cup2(iparm,fparm,wk(kps),mgo)
...
c
	...
	call cper3vb(nx,ny,nz,wk(ip),nxa,nyc,nze)
      ...
      subroutine ctrscyz(iset,nx,ny,nz,cof,cofyz,phi)
c
c     transfer coefficients from 3-d to 2-d
c     to allow 2-d relaxation
c
      ...
      complex cof(nx,ny,nz,8),cofyz(ny,nz,6)
      complex phi(0:nx+1,0:ny+1,0:nz+1)
      i = iset
      do k=1,nz
	do j=1,ny
     ...
     +                                 cof(i,j,k,2)*phi(i+1,j,k))
      ...
      subroutine csetpyz(iset,nx,ny,nz,phi,phyz,iyz)
      implicit none
      integer iset,nx,ny,nz,iyz,j,k
      complex phi(0:nx+1,0:ny+1,0:nz+1),phyz(0:ny+1,0:nz+1)
      if (iyz.eq.0) then
	do k=0,nz+1
	  do j=0,ny+1
	...
	do k=0,nz+1
      ...
      subroutine cup2(iparm,fparm,work,mgopt)
c
c     modification of cud2 for planar with cud3
c     whenever cup2 is called coefficients from discretization
...
c
     ...
     +             kcycle,iprer,ipost,intpol,kps
      ...
      complex work(*)
      common/icup2/intl,nxa,nxb,nyc,nyd,ixp,jyq,iex,jey,nfx,nfy,iguess,
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
      common/fcup2/xa,xb,yc,yd,tolmax,relmax
      common/cup2c/kpbgn(50),kcbgn(50),ktxbgn(50),ktybgn(50),
...
c
	...
	do kb=1,ngrid
	  ...
	  ity = ktybgn(k)
	  klevel = k
	  call discp2(nx,ny,work(ic),work(itx),work(ity),work(kps))
	end do
      call cup21(work)
      return
      end
...
c
      do l=1,iprer
	call relcp2(nx,ny,wk(ip),wk(ic),wk(itx),wk(ity),wk(kps))
      end do
...
c
      ...
      call rescp2(nx,ny,wk(ip),ncx,ncy,wk(ipc),wk(irc),wk(ic),wk(kps))
c
...
c
	  ...
	  ncy = nyk(klevel-1)
	  irc = kcbgn(klevel-1)+5*ncx*ncy
	  call rescp2(nx,ny,wk(ip),ncx,ncy,wk(ipc),wk(irc),wk(ic),
     +                wk(kps))
c
...
c
	...
	call relcp2(nx,ny,wk(ip),wk(ic),wk(itx),wk(ity),wk(kps))
      ...
      subroutine discp2(nx,ny,cof,tx,ty,sum)
c
...
c
     ...
     +             kcycle,iprer,ipost,intpol,kps
      ...
      complex tx(nx,ny,*),ty(ny,nx,*),cof(nx,ny,6),sum(*)
      common/icup2/intl,nxa,nxb,nyc,nyd,ixp,jyq,iex,jey,nfx,nfy,iguess,
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
...
c
     ...
     +                  ty(1,1,5),sum)
      ...
      subroutine rescp2(nx,ny,phi,ncx,ncy,phic,rhsc,cof,resf)
c
...
c
      ...
      subroutine relcp2(nx,ny,phi,cof,tx,ty,sum)
c
...
c
	...
	call relcp2p(nx,ny,phi,cof)
      else if (method.eq.1) then           ! line x relaxation
	call slxcp2(nx,ny,phi,cof,tx,sum)
      else if (method.eq.2) then           ! line y relaxation
	call slycp2(nx,ny,phi,cof,ty,sum)
	...
	call slycp2(nx,ny,phi,cof,ty,sum)
      ...
      subroutine relcp2p(nx,ny,phi,cof)
c
...
c
	...
	do i=1,nx
      ...
      subroutine slxcp2(nx,ny,phi,cof,tx,sum)
c
...
c
C$OMP PARALLEL DO SHARED(cof,phi,tx,nx,ny) PRIVATE(i,j,ib)
	do j=2,ny,2
	  do i=1,nx
	    phi(i,j)=cof(i,j,6)-cof(i,j,3)*phi(i,j-1)-cof(i,j,4)*
     +               phi(i,j+1)
...
c
	...
	do i=1,nx
      ...
      subroutine slycp2(nx,ny,phi,cof,ty,sum)
      implicit none
...
c
C$OMP PARALLEL DO SHARED(cof,phi,ty,sum,nx,ny) PRIVATE(i,j,jb)
	do i=1,nx,2
	  do j=1,ny-1
	    phi(i,j)=cof(i,j,6)-cof(i,j,1)*phi(i-1,j)-cof(i,j,2)*
     +               phi(i+1,j)
