...
c ... purpose
c
c     cuh34 attempts to improve the second order approximation generated
c     by cuh3 to a fourth order approximation using difference corrections
...
c
c
      subroutine cuh34(wk,iwk,phi,ierror)
      implicit none
      integer iwk(*)
      complex phi(*),wk(*)
     ...
     +kcycle,iprer,ipost,intpol
      ...
      call ch34(nx,ny,nz,phi,wk,iwk)
      end

      subroutine ch34(nx,ny,nz,phi,wk,iwk)
      implicit none
      integer nx,ny,nz,iwk(*),ncx,ncy,ncz
...
c
C$OMP PARALLEL DO PRIVATE(i,j,k,ux3,ux4,uy3,uy4,uz3,uz4,cxx,cx,cyy,cy)
C$OMP+PRIVATE(czz,cz,tx,ty,tz)
C$OMP+SHARED (u,cof,nx,ny,nz)
C$OMP+SHARED (dlx,dly,dlz,dlxx,dlyy,dlzz,dlx4,dly4,dlz4)
C$OMP+SHARED (tdlx3,tdly3,tdlz3)
      do k=3,nz-2
	do j=3,ny-2
	  do i=3,nx-2
