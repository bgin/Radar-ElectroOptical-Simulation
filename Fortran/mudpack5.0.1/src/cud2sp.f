...
c
C$OMP PARALLEL DO SHARED(jst,jfn,ist,ifn,rhs,resf,cofx,cofy,phi)
C$OMP+PRIVATE(i,j)
      do j=jst,jfn
	do i=ist,ifn
	  resf(i,j) =  rhs(i,j)-(
...
C$OMP PARALLEL DO SHARED(ist,ifn,phi,rhs,cofy,ny) PRIVATE(i,j)
	do j=3,ny-1,2
	  do i=ist,ifn
...
c     forward sweep
c
C$OMP PARALLEL DO SHARED(jst,jfn,phi,tx,nx) PRIVATE(i,ib,j)
      do j=jst,jfn,2
	do i=2,nx
...
c
	jfn = ny-1
C$OMP PARALLEL DO SHARED(ist,ifn,cofy,rhs,phi,ny) PRIVATE(i,j)
	do j=2,ny-1,2
	  do i=ist,ifn
	    phi(i,j)=rhs(i,j)-(cofy(j,1)*phi(i,j-1)+cofy(j,2)*phi(i,j+1))
	  end do
...
c     forward sweep
c
C$OMP PARALLEL DO SHARED(jfn,phi,tx,nx) PRIVATE(i,ib,j)
	do j=2,jfn,2
	  do i=2,nx
	    phi(i,j) = phi(i,j)-tx(i-1,j,1)*phi(i-1,j)
	  end do
...
C$OMP PARALLEL DO SHARED(cofy,rhs,phi,nx,ny) PRIVATE(i,j)
	do j=3,ny-1,2
	  do i=1,nx-1
	    phi(i,j)=rhs(i,j)-cofy(j,1)*phi(i,j-1)-cofy(j,2)*phi(i,j+1)
	  end do
...
c     forward sweep
c
C$OMP PARALLEL DO SHARED(sum,jst,jfn,phi,tx,nx) PRIVATE(i,ib,j)
	do j=jst,jfn,2
	  do i=2,nx-2
...
c
	jfn = ny-1
C$OMP PARALLEL DO SHARED(cofy,phi,rhs,nx,ny) PRIVATE(i,j)
	do j=2,ny-1,2
	  do i=1,nx-1
	    phi(i,j)=rhs(i,j)-cofy(j,1)*phi(i,j-1)-cofy(j,2)*phi(i,j+1)
	  end do
...
c     forward sweep
c
C$OMP PARALLEL DO SHARED(sum,jfn,phi,tx,nx) PRIVATE(i,ib,j)
	do j=2,jfn,2
	  do i=2,nx-2
	    phi(i,j) = phi(i,j)-tx(i,j,1)*phi(i-1,j)
	  end do
