...
c     approximation to the two dimensional complex nonseparable elliptic
...
c     cudcom.f
c
      subroutine cuh2cr(iparm,fparm,wk,iwk,coef,bndyc,rhs,phi,mgopt,
     +                  ierror)
      implicit none
     ...
     +nxk(50),nyk(50),isx,jsy
      ...
      common/ch2cr/ibeta,ialfa,izmat,idmat
      external coef,bndyc
      data int / 0 /
      save int
      ierror = 1
...
c
	...
	do kb=1,ngrid
	  ...
	  ity = ktybgn(k)
	  klevel = k
	  call disch2cr(nx,ny,wk(ic),wk(itx),wk(ity),
	...
	return
      ...
      nx = nfx
      ny = nfy
      call cuh2cr1(nx,ny,rhs,phi,coef,bndyc,wk,iwk)
      iparm(17) = itero
	...
	if (relmax.gt.tolmax) ierror = -1   ! flag convergenc failure
      ...
      subroutine cuh2cr1(nx,ny,rhsf,phif,coef,bndyc,wk,iwk)
      implicit none
      integer nx,ny,iwk(*)
...
c
	do k=1,ngrid
	  ...
	  ip = kpbgn(k)
	  ic = kcbgn(k)
	  call adjch2cr(nx,ny,wk(ip),wk(ic),bndyc,coef)
	end do
c
...
c
	if (nyc .ne. 0) then
	  call cdir2cr(nx,ny,wk(ip),wk(ic),wk(ibeta),wk(ialfa),iwk,nxa)
	  return
	else
	  call cdir2crp(nx,ny,wk(ip),wk(ic),wk(ibeta),wk(ialfa),
...
c
      do l=1,iprer
	call relch2cr(nx,ny,wk(ip),wk(ic),wk(itx),wk(ity),wk(kps))
      end do
c
...
c
      ...
      call resch2cr(nx,ny,wk(ip),ncx,ncy,wk(ipc),wk(irc),wk(ic),wk(kps))
c
...
c
	  ...
	  ncy = nyk(klevel-1)
	  irc = kcbgn(klevel-1)+9*ncx*ncy
	  call resch2cr(nx,ny,wk(ip),ncx,ncy,wk(ipc),wk(irc),wk(ic),
     +                wk(kps))
c
...
c
	  ...
	  ny = nyk(1)
	  if (nyc .ne. 0) then
	    call cdir2cr(nx,ny,wk(ip),wk(ic),wk(ibeta),wk(ialfa),iwk,
     +                   nxa)
	  else
	    call cdir2crp(nx,ny,wk(ip),wk(ic),wk(ibeta),wk(ialfa),
     +                    wk(izmat),wk(idmat),iwk,nxa)
...
c
	...
	call relch2cr(nx,ny,wk(ip),wk(ic),wk(itx),wk(ity),wk(kps))
      ...
      subroutine disch2cr(nx,ny,cf,tx,ty,bndyc,coef,wk,iwk,ier)
c
c     discretize elliptic pde for cuh2cr, set nonfatal errors
c
      implicit none
      integer intl,nxa,nxb,nyc,nyd,ixp,jyq,iex,jey,nfx,nfy,iguess,
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
      ...
      complex cf(nx,ny,10),tx(nx,ny,*),ty(ny,nx,*),wk(*)
      real x,y,dlx,dlx2,dlxx,dly,dly2,dlyy,cmin,alfmax,cemax
      complex cxx,cxy,cyy,cx,cy,ce,c1,c2,c3,c4,c5,c6,c7,c8,c9
...
c
	  if (klevel.eq.ngrid) then
	    if (cabs(cx)*dlx.gt.2*cabs(cxx) .or.
     +          cabs(cy)*dly.gt.2*cabs(cyy)) then
	      ier = -4
	    end if
	  end if
...
c     factor non-periodic block matrix
	  call clud2cr(nx,ny,cf,wk(ibeta),wk(ialfa),iwk,nxa)
	  return
	else
c     factor periodic block matrix

	  do j =1,ny-1
	    call csetbcr(nx,ny,cf,wk(ibeta),j,nxa)
	    call csetacr(nx,ny,cf,wk(ialfa),j,nxa)
	  end do
	  call clud2crp(nx,ny,cf,wk(ibeta),wk(ialfa),wk(izmat),
     +                  wk(idmat),iwk,nxa)
	  return
	end if
      end if
...
c
     ...
     +                   ty(1,1,5),wk(kps))
      ...
      subroutine clud2cr(nx,ny,cof,beta,alfa,index,nxa)
c
c     decompose nonperiodic block coefficient matrix
c
      implicit none
      integer nx,ny,nxa,index(nx,ny)
      complex cof(nx,ny,10),beta(nx,nx,*),alfa(nx,nx,*)
...
c
      jcur = 1
      call csetbcr(nx,ny,cof,beta,jcur,nxa)
...
c
	call csetacr(nx,ny,cof,alfa,jcur,nxa)
...
c
	...
	call csgfa(beta(1,1,jcur),nx,nx,index(1,jcur),iz)
      ...
      subroutine cdir2cr(nx,ny,phi,cof,beta,alfa,index,nxa)
c
...
c     forward sweep
      call cfor2cr(nx,ny,phi,cof(1,1,10),alfa)
c     backward sweep
      call cbkw2cr(nx,ny,phi,cof,beta,index,nxa)
      return
      end

      subroutine cfor2cr(nx,ny,phi,frhs,alfa)
c
c     forward sweep
c
      implicit none
	...
	do i=1,nx
      ...
      subroutine cbkw2cr(nx,ny,phi,cof,beta,index,nxa)
      implicit none
      integer nx,ny,index(nx,ny),nxa
      complex beta(nx,nx,*),sum
      complex phi(0:nx+1,0:ny+1),cof(nx,ny,10)
      integer iz,jcur,jb,j,i
      iz = 0
      jcur=ny
      call csgsl(beta(1,1,jcur),nx ,nx ,index(1,jcur),phi(1,jcur),iz)
      do jb=2,ny
	j=ny-jb+1
	jcur=j
	do i=2,nx-1
	...
	call csgsl(beta(1,1,jcur),nx ,nx ,index(1,jcur),phi(1,jcur),iz)
      ...
      subroutine clud2crp(nx,ny,cof,beta,alfa,zmat,dmat,index,nxa)
c
c     decompose periodic block tridiagonal matrix for direct at coarsest grid
c
      implicit none
      integer nx,ny,index(nx,ny),nxa
      complex cof(nx,ny,10),alfa(nx,nx,*),beta(nx,nx,*)
      complex dmat(nx,nx,*),zmat(nx,nx,*),sum,gama
...
c
	...
	do l=1,nx
	  call csgsl(beta(1,1,jcur),nx,nx,index(1,jcur),zmat(1,l,jcur),i1)
	end do
	call ctransp(nx,zmat(1,1,jcur))
      end do
c
...
c
	...
	do i=1,nx
     ...
     +                      cof(1,jcur-1,4)
      ...
      call csetacr(nx,ny,cof,alfa,ny-1,nxa)
      do i=1,nx
	do l=1,nx
...
c     in beta(ny-1)
c
      call csetbcr(nx,ny,cof,beta,ny-1,nxa)
      do i=1,nx
	do j=1,nx
	  sum = zer
	  do jcur=1,ny-2
...
c
      ...
      subroutine cdir2crp(nx,ny,phi,cof,beta,alfa,zmat,dmat,index,nxa)
      implicit none
      integer nx,ny,index(nx,ny),nxa
      complex phi(0:nx+1,0:ny+1),cof(nx,ny,10)
      complex beta(nx,nx,*),alfa(nx,nx,*)
...
c     forward sweep
      call cfor2crp(nx,ny,phi,cof(1,1,10),alfa,zmat)
c     backward sweep
      call cbkw2crp(nx,ny,phi,cof,beta,dmat,index,nxa)
      return
      end

      subroutine cfor2crp(nx,ny,phi,frhs,alfa,zmat)
      implicit none
      integer nx,ny,i,j,l,jcur,k
      complex frhs(nx,ny)
      complex phi(0:nx+1,0:ny+1)
...
c
	...
	phi(i,ny-1) = phi(i,ny-1) - sum
      ...
      subroutine cbkw2crp(nx,ny,phi,cof,beta,dmat,index,nxa)
      implicit none
      integer nx,ny,index(nx,ny),nxa
      complex phi(0:nx+1,0:ny+1),cof(nx,ny,10)
      complex beta(nx,nx,ny),dmat(nx,nx,*)
      integer iz,i,l,kb,k
      complex sum
      iz = 0
...
c
	...
	phi(i,ny) = phi(i,1)
      ...
      subroutine csetbcr(nx,ny,cof,beta,jcur,nxa)
c
c     set diagonal matrix on block
c
      implicit none
      integer nx,ny,jcur,nxa,i,l
      complex cof(nx,ny,10),beta(nx,nx,*)
      do i=1,nx
	do l=1,nx
	  beta(i,l,jcur)=(0.,0.)
	end do
	...
	beta(nx,2,jcur) = cof(nx,jcur,1)
      ...
      subroutine csetacr(nx,ny,cof,alfa,jcur,nxa)
      implicit none
      integer nx,ny,jcur,nxa,i,j
      complex cof(nx,ny,10),alfa(nx,nx,*)
      do i=1,nx
	do j=1,nx
	  alfa(i,j,jcur)=(0.,0.)
	end do
...
c     adjust for x periodicity
	...
	alfa(nx,2,jcur)=cof(nx,jcur,8)
      ...
      subroutine adjch2cr(nx,ny,phi,cf,bndyc,coef)
c
...
c
	...
	do i=1,nx
      ...
      subroutine resch2cr(nx,ny,phi,ncx,ncy,phic,rhsc,cof,resf)
c
...
c
      ...
      subroutine relch2cr(nx,ny,phi,cof,tx,ty,sum)
c
c     relaxation for cuh2cr
c
      implicit none
      integer nx,ny
	...
	call relch2crp(nx,ny,phi,cof)
      else if (method.eq.1) then           ! line x relaxation
	call slxch2cr(nx,ny,phi,cof,tx,sum)
      else if (method.eq.2) then           ! line y relaxation
	call slych2cr(nx,ny,phi,cof,ty,sum)
	...
	call slych2cr(nx,ny,phi,cof,ty,sum)
      ...
      subroutine relch2crp(nx,ny,phi,cof)
c
...
c
C$OMP PARALLEL DO SHARED(i1,cof,phi,nx,ny) PRIVATE(i,j)
      do lcolor=1,4
	do j=1,ny,4
	  do i=i1,nx,4
	      phi(i,j) = (cof(i,j,10) - (
...
C$OMP PARALLEL DO SHARED(i2,cof,phi,nx,ny) PRIVATE(i,j)
	do j=2,ny,4
	  do i=i2,nx,4
	      phi(i,j) = (cof(i,j,10) - (
     +                    cof(i,j,1)*phi(i+1,j)   +
...
C$OMP PARALLEL DO SHARED(i3,cof,phi,nx,ny) PRIVATE(i,j)
	do j=3,ny,4
	  do i=i3,nx,4
	      phi(i,j) = (cof(i,j,10) - (
     +                    cof(i,j,1)*phi(i+1,j)   +
...
C$OMP PARALLEL DO SHARED(i4,cof,phi,nx,ny) PRIVATE(i,j)
	do j=4,ny,4
	  do i=i4,nx,4
	      phi(i,j) = (cof(i,j,10) - (
     +                    cof(i,j,1)*phi(i+1,j)   +
...
c
	...
	i1 = it
      ...
      subroutine slxch2cr(nx,ny,phi,cof,tx,sum)
c
...
c
C$OMP PARALLEL DO SHARED(sum,cof,phi,tx,nx,ny) PRIVATE(i,ib,j)
	do j=1,ny,2
	  do i=1,nx-1
	    phi(i,j) = cof(i,j,10)-(cof(i,j,2)*phi(i+1,j+1)+
     +                              cof(i,j,3)*phi(i,j+1)+
...
c
	...
	do i=1,nx
      ...
      subroutine slych2cr(nx,ny,phi,cof,ty,sum)
      implicit none
...
c
C$OMP PARALLEL DO SHARED(cof,phi,ty,nx,ny) PRIVATE(i,jb,j)
	do i=1,nx,2
	  do j=1,ny
...
c
C$OMP PARALLEL DO SHARED(sum,cof,phi,ty,nx,ny) PRIVATE(i,jb,j)
	do i=1,nx,2
	  do j=1,ny-1
	    phi(i,j) = cof(i,j,10)-(cof(i,j,1)*phi(i+1,j)+
     +                              cof(i,j,2)*phi(i+1,j+1)+
