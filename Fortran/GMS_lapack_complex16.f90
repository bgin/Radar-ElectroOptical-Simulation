...
!*
          ...
          IF (INCY.EQ.1) THEN
             ...
             !$OMP PARALLEL DO SCHEDULE(STATIC,4) DEFAULT(NONE) SHARED(A,N,M,ALPHA,N,M) 
   ...
   60         CONTINUE
          ELSE
             ...
             !$OMP PARALLEL DO SCHEDULE(STATIC,4) DEFAULT(NONE) SHARED(X,Y,A,ALPHA,N,M)
...
!*
          ...
          IF (INCX.EQ.1) THEN
             ...
             !$OMP PARALLEL DO SCHEDULE(STATIC,4) DEFAULT(NONE) SHARED(A,X,N,M,NOCONJ,ALPHA,INCY) 
  ...
  110         CONTINUE
           ELSE
             ...
             !$OMP PARALLEL DO SCHEDULE(STATIC,4) DEFAULT(NONE) SHARED(A,X,NOCONJ,N,M,ALPHA) 
...
!*
      ...
      IF (INCX.EQ.1) THEN
         ...
         !$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC,4) SHARED(Y,X,N,M,ALPHA,INCY) 
   ...
   20     CONTINUE
      ELSE
         ...
         !$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC,4) SHARED(Y,X,N,M,ALPHA,KX) 
...
!*
      ...
      IF (INCX.EQ.1) THEN
         ...
         !$OMP PARALLEL DO SCHEDULE(STATIC,4) DEFAULT(NONE) SHARED(Y,X,N,M,ALPHA,INCY) 
         !$OMP& REDUCTION(+:A) FIRSTPRIVATE(JY) PRIVATE(J,TEMP,I)
          DO 20 J = 1,N
              IF (Y(JY).NE.ZERO) THEN
                 TEMP = ALPHA*Y(JY)
   ...
   20     CONTINUE
      ELSE
         ...
         !$OMP PARALLEL DO SCHEDULE(STATIC,4) DEFAULT(NONE) SHARED(Y,X,N,M,ALPHA,INCX,KX) 
         !$OMP REDUCTION(+:A) FIRSTPRIVATE(JY) PRIVATE(J,TEMP,I,IX)
         DO 40 J = 1,N
              IF (Y(JY).NE.ZERO) THEN
                 TEMP = ALPHA*Y(JY)
...
!*              C1 := C1 - W
               ...
               !$OMP PARALLEL DO SCHEDULE(STATIC,4) DEFAULT(NONE) 
...
!*              C2 := C2 - W**H
               ...
               !$OMP PARALLEL DO SCHEDULE(STATIC,4) DEFAULT(NONE) SHARED(WORK,K,M) 
...
!*
             IF (UPPER) THEN
                ...
                !$OMP PARALLEL DO SCHEDULE(GUIDED,1) DEFAULT(NONE) SHARED(A,N,M,NOUNIT,ALPHA) PRIVATE(J,K,I,TEMP) 
                !$OMP& REDUCTION(+:B)
                  DO 50 J = 1,N
                      DO 40 K = 1,M
   ...
   50             CONTINUE
              ELSE
                ...
                !$OMP PARALLEL DO SCHEDULE(GUIDED,1) DEFAULT(NONE) SHARED(A,ALPHA,NOUNIT,N,M) PRIVATE(J,K,I,TEMP)
                !$OMP& REDUCTION(+:B)
                DO 80 J = 1,N
                      DO 70 K = M,1,-1
                          IF (B(K,J).NE.ZERO) THEN
...
!*
             IF (UPPER) THEN
                ...
                !$OMP PARALLEL DO  SCHEDULE(STATIC,4) DEFAULT(NONE) SHARED(A,N,M,ALPHA)  PRIVATE(J,TEMP,I,K)
                !$OMP& REDUCTION(+:B)
                DO 200 J = N,1,-1
                      TEMP = ALPHA
  ...
  200             CONTINUE
               ELSE
                ...
                !$OMP PARALLEL DO SCHEDULE(GUIDED,1) DEFAULT(NONE) SHARED(A,N,ALPHA,NOUNIT) PRIVATE(J,I,TEMP,K)
                !$OMP& REDUCTION(+:B)
                DO 240 J = 1,N
                      TEMP = ALPHA
                      IF (NOUNIT) TEMP = TEMP*A(J,J)
...
!*
             IF (UPPER) THEN
                ...
                !$OMP PARALLEL DO SCHEDULE(GUIDED,1) DEFAULT(NONE)  SHARED(A,N,M,NOCONJ,ALPHA,NOUNIT) PRIVATE(K,J,TEMP,I)
                !$OMP& REDUCTION(+:B) 
                  DO 280 K = 1,N
                      DO 260 J = 1,K - 1
  ...
  280             CONTINUE
              ELSE
                    !$OMP PARALLEL DO SCHEDULE(STATIC,1) DEFAULT(NONE)  SHARED(A,N,NOCONJ,ALPHA,ONE,ZERO,M,NOUNIT) 
...
!*           Form  C := alpha*A*B + beta*C.
             !*
             !$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC,4) SHARED(A,N,BETA,M,K,ALPHA) PRIVATE(J,I,L,TEMP) 
             !$OMP& REDUCTION(+:C)
              DO 90 J = 1,N
                 IF (BETA.EQ.ZERO) THEN
                      !$OMP SIMD ALIGNED(C:64) LINEAR(I:1) UNROLL PARTIAL(8)
...
!*           Form  C := alpha*A**H*B + beta*C.
             ...
             !$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC,4) SHARED(A,B,C,N,M,K,BETA,ALPHA) PRIVATE(J,I,L)
             !$OMP& REDUCTION(+:TEMP)
              DO 120 J = 1,N
                  DO 110 I = 1,M
                     TEMP = ZERO
...
!*           Form  C := alpha*A*B**H + beta*C.
             ...
             !$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC,4) SHARED(B,A,BETA,ALPHA,N,M,K) PRIVATE(J,I,L,TEMP) 
             !$OMP& REDUCTION(+:C)
             DO 200 J = 1,N
                 IF (BETA.EQ.ZERO) THEN
                       !$OMP SIMD ALIGNED(C:64) LINEAR(I:1) UNROLL PARTIAL(8)    
...
!*           Form  C := alpha*A*B**T + beta*C
             ...
             !$OMP PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC,4) SHARED(B,A,N,M,ALPHA,BETA) PRIVATE(J,I,L,TEMP) 
             !$OMP& REDUCTION(+:C)
              DO 250 J = 1,N
                 IF (BETA.EQ.ZERO) THEN
                      !$OMP SIMD ALIGNED(C:64) LINEAR(I:1) UNROLL PARTIAL(8)    
...
!*
          IF (LSAME(UPLO,'U')) THEN
             IF (INCX.EQ.1) THEN
                ...
                !$OMP PARALLEL DO SCHEDULE(STATIC,4) DEFAULT(NONE) SHARED(A,ZERO,N,NOUNIT) PRIVATE(J,TEMP,I)
                !$OMP& REDUCTION(+:X)
                  DO 20 J = 1,N
                      IF (X(J).NE.ZERO) THEN
                         TEMP = X(J)
   ...
   20             CONTINUE
              ...
              ELSE
                ...
                !$OMP PARALLEL DO SCHEDULE(GUIDED,1) DEFAULT(NONE) SHARED(A,INCX,N) FIRSTPRIVATE(JX) PRIVATE(J,TEMP,IX,I)
                !$OMP& REDUCTION(+:X)
                DO 40 J = 1,N
   ...
   40              CONTINUE
          ...
          ELSE
             IF (INCX.EQ.1) THEN
                ...
                !$OMP PARALLEL DO SCHEDULE(GUIDED,1) DEFAULT(NONE) SHARED(A,NOUNIT,ZERO,N) PRIVATE(J,TEMP,I)
                !$OMP& REDUCTION(+:X)
                DO 60 J = N,1,-1
                      IF (X(J).NE.ZERO) THEN
                         TEMP = X(J)
   ...
   60             CONTINUE
              ...
              ELSE
                ...
                !$OMP PARALLEL DO SCHEDULE(GUIDED,1) DEFAULT(NONE) SHARED(A,N,NOUNIT,KX) FIRSTPRIVATE(JX) PRIVATE(J,TEMP,IX,I) 
                !$OMP& REDUCTION(+:X)
                DO 80 J = N,1,-1
                      IF (X(JX).NE.ZERO) THEN
                          TEMP = X(JX)
...
140               CONTINUE
  ...
  170              CONTINUE
              ...
              ELSE
                ...
                !$OMP PARALLEL DO SCHEDULE(GUIDED,1) DEFAULT(NONE) REDUCTION(+:TEMP)  SHARED(X,A,N,NOCONJ,NOUNIT) 
