...
module mod_tmatrix
 ...
 !          
 !          Purpose:
 !                        This module contains a slightly modified version
 !                        of Michael Mishchenko T-MATRIX code.
 !          History:
 !                        Date: 27-05-2018
 !                        Time: 11:02 GMT+2
 !                        Modified
 ...
 !==================================================================================85
    ...
    ! Major version
    integer(kind=int4), parameter, public :: MOD_TMATRIX_MAJOR = 1
    
    ! Minor version
    integer(kind=int4), parameter, public :: MOD_TMATRIX_MINOR = 1
    
    ! Micro version
    integer(kind=int4), parameter, public :: MOD_TMATRIX_MICRO = 0
    
    ! Module full version
    integer(kind=int4), parameter, public :: MOD_TMATRIX_FULLVER = 1000*MOD_TMATRIX_MAJOR + &
                                                                   100*MOD_TMATRIX_MINOR  + &
                                                                   10*MOD_TMATRIX_MICRO
    ...
    ! Originally a part of tmd.par.f file
    !   PARAMETER (NPN1=100, NPNG1=300, NPNG2=2*NPNG1, NPN2=2*NPN1,  
    ! &           NPL=NPN2+1, NPN3=NPN1+1,  
    ! &           NPN4=80, NPN5=2*NPN4, NPN6=NPN4+1, NPL1=NPN5+1)
    
    
    integer(kind=int4), parameter, public :: NPN1  = 100
    
    integer(kind=int4), parameter, public :: NPNG1 = 300
    
    integer(kind=int4), parameter, public :: NPNG2 = 2*NPNG1
    
    integer(kind=int4), parameter, public :: NPN2  = 2*NPN1
    
    integer(kind=int4), parameter, public :: NPL   = NPN2+1
    
    integer(kind=int4), parameter, public :: NPN3  = NPN1+1
    
    integer(kind=int4), parameter, public :: NPN4  = 80
    
    integer(kind=int4), parameter, public :: NPN5  = 2*NPN4
    
    integer(kind=int4), parameter, public :: NPN6  = NPN4+1
    
    integer(kind=int4), parameter, public :: NPL1  = NPN5+1
    
    real(kind=dp),    parameter, private :: PI    = 3.1415926535897932384626433832795_dp
    
    ! Types declarations
    ! Originally common blocks
...
!DIR$ ATTRIBUTES CODE_ALIGN : 32 :: TMATRIX_Driver         
          real(kind=dp),    intent(in)      :: RAT
          integer(kind=int4), intent(in)      :: NDISTR
          real(kind=dp),    intent(in)      :: AXMAX
          integer(kind=int4), intent(in)      :: NPNAX
          real(kind=dp),    intent(in)      :: B
          real(kind=dp),    intent(in)      :: GAM
          integer(kind=int4), intent(in)      :: NKMAX
          real(kind=dp),    intent(in)      :: EPS
          integer(kind=int4), intent(in)      :: NP
          real(kind=dp),    intent(in)      :: LAM
          real(kind=dp), intent(in)         :: MRR
          real(kind=dp), intent(in)         :: MRI
          real(kind=dp),    intent(in)      :: DDELT
          integer(kind=int4), intent(in)      :: NPNA  ! Number of scattering angles
          integer(kind=int4), intent(in)      :: NDGS
          real(kind=dp),    intent(inout)   :: CEXT  ! Result
          real(kind=dp),    intent(inout)   :: CSCA  ! Result
          real(kind=dp),    intent(inout)   :: W     ! Result
          real(kind=dp),    intent(inout)   :: COSPH ! Result
          real(kind=dp),    intent(inout)   :: REFF  ! Result
          real(kind=dp),    intent(inout)   :: VEFF  ! Result
          real(kind=dp), dimension(NPL), intent(inout) :: ALPHA1
!DIR$     ASSUME_ALIGNED ALPHA1:64
          real(kind=dp), dimension(NPL), intent(inout) :: ALPHA2
!DIR$     ASSUME_ALIGNED ALPHA2:64
          real(kind=dp), dimension(NPL), intent(inout) :: ALPHA3
!DIR$     ASSUME_ALIGNED ALPHA3:64
          real(kind=dp), dimension(NPL), intent(inout) :: ALPHA4
!DIR$     ASSUME_ALIGNED ALPHA4:64
          real(kind=dp), dimension(NPL), intent(inout) :: BETA1
!DIR$     ASSUME_ALIGNED BETA1:64
          real(kind=dp), dimension(NPL), intent(inout) :: BETA2
!DIR$     ASSUME_ALIGNED BETA2
          real(kind=dp), dimension(6,NPNA), intent(inout) :: SCATMAT   ! contains elements of the normalized scattering matrix
...
!DIR$     ATTRIBUTES ALIGN : 64 ::  BE2
          real(kind=dp), dimension(NPN6,NPN4,NPN4) :: RT11,RT12,RT21,RT22, &
                                                   IT11,IT12,IT21,IT22
          integer(kind=int4) :: NCHECK,IAX,I,NK,II,L1MAX,INK,NNM,N2,M,NMAX, &
                           NM,NM1,N11,NN2,NKMAX,IXXX,INM1,NMA,NGAUSS,MMAX, &
                           N,N1,NMIN,NMAX1,NNNGGG,NGAUS,NGGG,NN1,M1,N22,   &
                           L1M,LMAX,L1
          real(kind=dp)    :: DAX,R1,R2,AXI,Z1,Z2,Z3,CSCAT,CEXTIN,   &
                           A,XEV,QEXT1,QSCA1,QEXT,QSCA,TR1NN,DN1, &
                           TI1NN,TR1NN1,TI1NN1,DSCA,DEXT,DQSCA,   &
                           DQEXT,ZZ1,ZZ2,ZZ3,ZZ4,ZZ5,ZZ6,ZZ7,ZZ8, &
                           QSC,QXT,COEFF1,WGII,WGI,WALB,ASYMM
...
! 5454     FORMAT ('NCHECK=',I1)
          DAX=AXMAX/NPNAX
          IF (DABS(RAT-1._dp).GT.1.0E-8_dp.AND.NP.EQ.-1) CALL SAREA (EPS,RAT)
          IF (DABS(RAT-1._dp).GT.1.0E-8_dp.AND.NP.GE.0) CALL SURFCH(NP,EPS,RAT)
          IF (DABS(RAT-1._dp).GT.1.0E-8_dp.AND.NP.EQ.-2) CALL SAREAC (EPS,RAT)
         ! PRINT 8000, RAT
...
!DIR$ SIMD VECTORLENGTHFOR(REAL(KIND=8))
             ...
             DO 500 INK=1,NK
                ...
                A=RAT*XG1(I)
                XEV=2._dp*PI*A/LAM
                IXXX=XEV+4.05_dp*XEV**0.333333_dp
                INM1=MAX0(4,IXXX)
...
!DIR$ SIMD VECTORLENGTHFOR(REAL(KIND=8))
  ...
  510    CONTINUE
         ...
         ASYMM=ALPHA1(2)*0.33333333333333333333_dp
...
!DIR$ ATTRIBUTES CODE_ALIGN : 32 :: CONST
          integer(kind=int4) :: NGAUSS,NMAX,MMAX
          real(kind=dp)    :: P
...
!DIR$ SIMD VECTORLENGTHFOR(REAL(KIND=8))
           DO 12 I=1,NG1
              W(I)=0.5_dp*(XX+1D0)*W1(I)
              X(I)=0.5_dp*(XX+1D0)*X1(I)+0.5_dp*(XX-1._dp)
        12 CONTINUE
!DIR$ VECTOR ALIGNED
!DIR$ SIMD VECTORLENGTHFOR(REAL(KIND=8))
           DO 14 I=1,NG2
...
!DIR$     ATTRIBUTES ALIGN : 64 :: ZI
          real(kind=dp), dimension(NPNG2,NPN1) :: J,Y,JR,JI,DJ,DJR, &
                                               DJI,DY
          integer(kind=int4) :: NG,I,NNMAX1,NNMAX2
          real(kind=dp)    :: V,PRR,PRI,TA,VV,V1,V2,TB
...
!DIR$ ATTRIBUTES CODE_ALIGN : 32 :: RSP1         
          real(kind=dp), dimension(NG) :: X,R,DR
!DIR$     ASSUME_ALIGNED X:64,R:64,DR:64
          ...
          integer(kind=int4) :: I
          ! Exec code ...
          A = REV*EPS**(0.333333333333333333_dp)
          AA = A*A
          EE = EPS*EPS
          EE1 = EE-1._dp
!DIR$  VECTOR ALIGNED
...
!DIR$     ASSUME_ALIGNED X:64,R:64,DR:64
          ...
          integer(kind=int4) :: N
          ! Locals
          real(kind=dp) :: DNP,DN,DN4,EP,A,R0,XI,RI,R0
          integer(kind=int4) :: I
          ! Exec code ...
           DNP=DFLOAT(N)
           DN=DNP*DNP
           ...
           R0=REV*A**(-0.33333333333333333333_dp)
!DIR$      VECTOR ALIGNED
!DIR$      SIMD VECTORLENGTHFOR(REAL(KIND=8))
           DO 50 I=1,NG
                 XI=DACOS(X(I))*DNP
...
!DIR$ ATTRIBUTES CODE_ALIGN : 32 :: RSP3
          real(kind=dp), dimension(NG) :: X,R,DR
!DIR$     ASSUME_ALIGNED X:64,R:64,DR:64
          integer(kind=int4) :: N,NGAUSS
          real(kind=dp)    :: REV,EPS
          ! Locals
          real(kind=dp) :: H,A,CO,SI,RAD,RTHET,SI
          integer(kind=int4) :: I
          ! Exec code
          H=REV*( (2._dp/(3._dp*EPS*EPS))**(0.3333333333333333333333333_dp) )
          A=H*EPS
          DO 50 I=1,NGAUSS
                CO=-X(I)
                SI=DSQRT(1._dp-CO*CO)
...
!DIR$     ASSUME_ALIGNED X:64,XR:64,XI:64
          ...
          real(kind=dp), dimension(NPNG2,NPN1) :: J,Y,JR,JI,DJ,DY,  &
                                               DJR,DJI
          ! Locals
...
!DIR$     ATTRIBUTES ALIGN : 64 :: CZI
          integer(kind=int4) :: L,L1,I,I1
          real(kind=dp)    :: XRXI,CXXR,XRXI,CXXI,QF,AR,ARI,AI,  &
...
!     & 8X,'F44',8X,'F12',8X,'F34')
  ...
  350       IF(L.LT.2) GO TO 400
            ...
            P2=PP2
            PP2=P
            P=(PL1*(PL2+4._R64P0)*PP3-PL3*P3)*PL4
            P3=PP3
            PP3=P
