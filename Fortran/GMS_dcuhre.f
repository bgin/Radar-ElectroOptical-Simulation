...
C            Alan Genz, Math Department, Washington State
C            University, Pullman, WA 99164-3113, USA
...
C     PROGRAM DTEST1
C     EXTERNAL FTEST
C     integer(kind=4) ::   KEY, N, NF, NDIM, MINCLS, MAXCLS, IFAIL, NEVAL, NW
C     PARAMETER (NDIM = 5, NW = 5000, NF = NDIM+1)
C     real(kind=8) ::  A(NDIM), B(NDIM), WRKSTR(NW)
C     real(kind=8) ::  ABSEST(NF), FINEST(NF), ABSREQ, RELREQ
C     DO 10 N = 1,NDIM
C        A(N) = 0
C        B(N) = 1
C  10 CONTINUE
...
C     integer(kind=4) ::   N, NDIM, NFUN
C     real(kind=8) ::  Z(NDIM), F(NFUN), SUM
C     SUM = 0
C     DO 10 N = 1,NDIM
C        SUM = SUM + N*Z(N)**2
C  10 CONTINUE
...
C
#endif
      integer(kind=4) ::   MDIV,MAXWT,WTLENG,MAXDIM,LENW2,MAXSUB,MINSUB
      integer(kind=4) ::   NUM,NSUB,LENW,KEYF
      PARAMETER (MDIV=1)
      PARAMETER (MAXDIM=15)
      PARAMETER (MAXWT=14)
      PARAMETER (LENW2=2*MDIV*MAXDIM* (MAXWT+1)+12*MAXWT+2*MAXDIM)
      integer(kind=4) ::   WRKSUB,I1,I2,I3,I4,I5,I6,I7,I8,K1,K2,K3,K4,K5,K6,K7,K8
      real(kind=8) ::  WORK2(LENW2+10)
#if defined(__INTEL_COMPILER) || defined(__ICC)
...
C
#endif
      integer(kind=4) ::   NDIM,NUMFUN,MDIV,MINPTS,MAXPTS,KEY,NW,MINSUB,MAXSUB
      integer(kind=4) ::   RESTAR,NUM,KEYF,IFAIL,MAXDIM,WTLENG
      real(kind=8) ::  A(NDIM),B(NDIM),EPSABS,EPSREL
#if defined(__INTEL_COMPILER) || defined(__ICC)
...
!C
      ...
      END IF
!C
!C   Check on big enough real(kind=8) ::  workspace.
!C
...
#endif
      EXTERNAL FUNSUB
      integer(kind=4) ::   NDIM,NUMFUN,MDIV,MINSUB,MAXSUB,KEY,LENW,RESTAR
      integer(kind=4) ::   NUM,NEVAL,NSUB,IFAIL,WTLENG
      real(kind=8) ::  A(NDIM),B(NDIM),EPSABS,EPSREL
      real(kind=8) ::  RESULT(NUMFUN),ABSERR(NUMFUN)
      real(kind=8) ::  VALUES(NUMFUN,MAXSUB),ERRORS(NUMFUN,MAXSUB)
      real(kind=8) ::  CENTRS(NDIM,MAXSUB)
      real(kind=8) ::  HWIDTS(NDIM,MAXSUB)
      real(kind=8) ::  GREATE(MAXSUB),DIR(MAXSUB)
      real(kind=8) ::  OLDRES(NUMFUN,MDIV)
      real(kind=8) ::  WORK(LENW),RULPTS(WTLENG)
      real(kind=8) ::  G(NDIM,WTLENG,2*MDIV),W(5,WTLENG)
      real(kind=8) ::  CENTER(NDIM),HWIDTH(NDIM),X(NDIM,2*MDIV)
      real(kind=8) ::  SCALES(3,WTLENG),NORMS(3,WTLENG)
...
!C
      ...
      integer(kind=4) ::   INTSGN,SBRGNS
      integer(kind=4) ::   L1
      integer(kind=4) ::   NDIV,POINTR,DIRECT,INDEX
      real(kind=8) ::  OLDCEN,EST1,EST2,ERRCOF(6)
!C
...
C
      ...
      real(kind=8) ::  G(NDIM,WTLENG),W(5,WTLENG),ERRCOF(6)
      real(kind=8) ::  RULPTS(WTLENG),SCALES(3,WTLENG)
      real(kind=8) ::  NORMS(3,WTLENG)
!C
!C   Local variables.
!C
      integer(kind=4) ::   I,J,K
...
C
#endif
      integer(kind=4) ::   WTLENG
      real(kind=8) ::  W(5,WTLENG),G(2,WTLENG),ERRCOF(6)
      real(kind=8) ::  RULPTS(WTLENG)
!C
!C   Local variables.
!C
      integer(kind=4) ::   I,J
      real(kind=8) ::  DIM2G(16)
      real(kind=8) ::  DIM2W(14,5)
!C
...
#endif
      integer(kind=4) ::   WTLENG
      real(kind=8) ::  W(5,WTLENG),G(3,WTLENG),ERRCOF(6)
      real(kind=8) ::  RULPTS(WTLENG)
!C
!C   Local variables.
!C
      integer(kind=4) ::   I,J
      real(kind=8) ::  DIM3G(14)
      real(kind=8) ::  DIM3W(13,5)
!C
...
C
      ...
      real(kind=8) ::  W(5,WTLENG),G(NDIM,WTLENG),ERRCOF(6)
      real(kind=8) ::  RULPTS(WTLENG)
!C
!C   Local Variables
!C
...
#endif
      EXTERNAL FUNSUB
      integer(kind=4) ::   WTLENG,NUMFUN,NDIM
      real(kind=8) ::  CENTER(NDIM),X(NDIM),HWIDTH(NDIM),BASVAL(NUMFUN),   &
...
!C   Local variables.
!C
      real(kind=8) ::  RGNVOL,DIFSUM,DIFMAX,FRTHDF
      integer(kind=4) ::   I,J,K,DIVAXN
      real(kind=8) ::  SEARCH,RATIO
!C
...
C
#endif
      integer(kind=4) ::   DVFLAG,NDIM,NUMFUN,SBRGNS
      real(kind=8) ::  VALUES(NUMFUN,*),ERRORS(NUMFUN,*)
      real(kind=8) ::  CENTRS(NDIM,*)
      real(kind=8) ::  HWIDTS(NDIM,*)
      real(kind=8) ::  GREATE(*)
      real(kind=8) ::  ERROR(NUMFUN),VALUE(NUMFUN)
      real(kind=8) ::  CENTER(NDIM),HWIDTH(NDIM)
      real(kind=8) ::  DIR(*)
!C
!C   Local variables.
!C
...
!C
      integer(kind=4) ::   J,SUBRGN,SUBTMP
      real(kind=8) ::  GREAT,DIRECT
!C
