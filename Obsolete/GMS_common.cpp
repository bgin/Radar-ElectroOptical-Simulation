...
void
gms::common
::init_unroll2x_cmplxr4(std::complex<float> * __restrict __ATTR_ALIGN__(64) vc4,
			const int64_t len,
...
void
gms::common
::init_unroll4x_cmplxr4(std::complex<float> * __restrict __ATTR_ALIGN__(64) vc4,
			const int64_t len,
			const std::complex<float> val) {
#if defined __GNUC__ && !defined __INTEL_COMPILER
  vc4 = (std::complex<float>*)__builtin_assume_aligned(vc4,64);
...
void
gms::common
::init_unroll8x_cmplxr4(std::complex<float> * __restrict __ATTR_ALIGN__(64) vc4,
			const int64_t len,
			const std::complex<float> val) {
#if defined __GNUC__ && !defined __INTEL_COMPILER
  vc4 = (std::complex<float>*)__builtin_assume_aligned(vc4,64);
...
void
gms::common::
avx512vec16_init_unroll2x(AVX512Vec16 * __restrict vec16,
			  const int64_t len,
			  const AVX512Vec16 v) {
#if defined __GNUC__ && !defined __INTEL_COMPILER
...
void
gms::common::
avx512vec16_init_unroll4x(AVX512Vec16 * __restrict vec16,
			  const int64_t len,
			  const AVX512Vec16 v) {
#if defined __GNUC__ && !defined __INTEL_COMPILER
     vec16 = (AVX512Vec16*)__builtin_assume_aligned(vec16,64);
...
void
gms::common::
avx512vec16_init_unroll8x(AVX512Vec16 * __restrict vec16,
			  const int64_t len,
			  const AVX512Vec16 v) {
#if defined __GNUC__ && !defined __INTEL_COMPILER
     vec16 = (AVX512Vec16*)__builtin_assume_aligned(vec16,64);
...
void
gms::common::
avx512vec16_copy_unroll2x(AVX512Vec16 * __restrict dst,
                          const AVX512Vec16 * __restrict src,
			  const int64_t len) {
#if defined __GNUC__ && !defined __INTEL_COMPILER
     dst = (AVXV512ec16*)__builtin_assume_aligned(dst,64);
     src = (AVX512Vec16*)__builtin_assume_aligned(src,64);
#elif defined __INTEL_COMPILER
     __assume_aligned(dst,64);
     __assume_aligned(src,64);
#endif
...
void
gms::common::
avx512vec16_copy_unroll4x(AVX512Vec16 * __restrict dst,
                          const AVX512Vec16 * __restrict src,
			  const int64_t len) {
#if defined __GNUC__ && !defined __INTEL_COMPILER
     dst = (AVXV512ec16*)__builtin_assume_aligned(dst,64);
...
void
gms::common::
avx512vec16_copy_unroll8x(AVX512Vec16 * __restrict dst,
                          const AVX512Vec16 * __restrict src,
			  const int64_t len) {
#if defined __GNUC__ && !defined __INTEL_COMPILER
     dst = (AVXV512ec16*)__builtin_assume_aligned(dst,64);
...
#pragma code_align(32)
	     ...
	     _mm256_storeu_ps(&v[i+0LL], ymm0);
	     _mm256_storeu_ps(&v[i+8LL], ymm0);
	   }
	 ...
	 if((reinterpret_cast<uintptr_t>(v) & 0x1F) != 0ULL) {
	   for(i = 0LL; i != ROUND_TO_EIGHT(vlen,8LL); i += 16LL) {
	    ...
	    v = (float*)__builtin_assume_aligned(v,32);
           for(i = 0LL; i != ROUND_TO_EIGHT(vlen,8LL); i += 16LL) {
             _mm256_store_ps(&v[i+0LL], ymm0);
	     _mm256_store_ps(&v[i+8LL], ymm0);
	   }
	 }
	 for(; i != vlen; ++i) {
	   v[i] = val;
...
#pragma code_align(32)
	   ...
	   _mm256_storeu_ps(&v[i+0LL],  ymm0);
	   _mm256_storeu_ps(&v[i+8LL],  ymm0);
	   _mm256_storeu_ps(&v[i+16LL], ymm0);
	   _mm256_storeu_ps(&v[i+24LL], ymm0);
	   _mm256_storeu_ps(&v[i+32LL], ymm0);
	   _mm256_storeu_ps(&v[i+40LL], ymm0);
	   _mm256_storeu_ps(&v[i+48LL], ymm0);
	   _mm256_storeu_ps(&v[i+56LL], ymm0);
	 }
#pragma loop_count min(1),avg(4),max(7)
          for(; i != vlen; ++i) {
	     v[i] = val;
	 ...
	 for(i = 0LL; i != ROUND_TO_EIGHT(vlen,8LL); i += 64LL) {
           _mm256_store_ps(&v[i+0LL],  ymm0);
	   _mm256_store_ps(&v[i+8LL],  ymm0);
	   _mm256_store_ps(&v[i+16LL], ymm0);
	   _mm256_store_ps(&v[i+24LL], ymm0);
	   _mm256_store_ps(&v[i+32LL], ymm0);
	   _mm256_store_ps(&v[i+40LL], ymm0);
	   _mm256_store_ps(&v[i+48LL], ymm0);
	   _mm256_store_ps(&v[i+56LL], ymm0);
	 }
       }
       for(; i != vlen; ++i) {
	 v[i] = val;
