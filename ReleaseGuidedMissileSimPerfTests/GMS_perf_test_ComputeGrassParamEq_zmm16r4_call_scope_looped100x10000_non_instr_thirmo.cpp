...
#include "call_scope_looped_tests_cast_to_f64_loop_body.c"
     ...
     double * __restrict PMC0_mnt  = NULL; // 3rd moment
     double PMC0_xmean = 0.0;
     constexpr int32_t lag1 = 2001;
     PMC0_acor = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==PMC0_acor) { MALLOC_FAILED}
     PMC0_acov = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==PMC0_acov)  { MALLOC_FAILED};
     PMC0_mnt = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*lag1*sizeof(double),alignment));
     if(NULL==PMC0_mnt) { MALLOC_FAILED};
     // Initialize the result arrays
     for(int32_t i = 0; i != lag1; i += 16) {
     ...
     for(int32_t i = 0; i != (lag1*lag1); i += 16) {
         _mm512_stream_pd(&PMC0_mnt[i+0], vzf64);
	 _mm512_stream_pd(&PMC0_mnt[i+8], vzf64);
     }
	...
	std::exit(EXIT_FAILURE);
     ...
     for(int32_t i = 0; i != tot_samples; ++i) { fprintf(fp1,"%.16f\n",PMC0_f64[i]);}
     ...
     double * __restrict PMC1_mnt  = NULL; // 3rd moment
     double PMC1_xmean = 0.0;
     PMC1_acor = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==PMC1_acor) { MALLOC_FAILED}
     PMC1_acov = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==PMC1_acov)  { MALLOC_FAILED};
     PMC1_mnt = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*lag1*sizeof(double),alignment));
     ...
     for(int32_t i = 0; i != (lag1*lag1); i += 16) {
         _mm512_stream_pd(&PMC1_mnt[i+0], vzf64);
	 _mm512_stream_pd(&PMC1_mnt[i+8], vzf64);
     }
     // Call F77 C-interface to THIRMOF subroutine
	...
	std::exit(EXIT_FAILURE);
     ...
     for(int32_t i = 0; i != tot_samples; ++i) { fprintf(fp2,"%.16f\n",PMC1_f64[i]);}
     ...
     PMC2_acor = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==PMC2_acor) { MALLOC_FAILED}
     PMC2_acov = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==PMC2_acov)  { MALLOC_FAILED};
     PMC2_mnt = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*lag1*sizeof(double),alignment));
     ...
     for(int32_t i = 0; i != (lag1*lag1); i += 16) {
         _mm512_stream_pd(&PMC2_mnt[i+0], vzf64);
	 _mm512_stream_pd(&PMC2_mnt[i+8], vzf64);
     }
     // Call F77 C-interface to THIRMOF subroutine
	...
	std::exit(EXIT_FAILURE);
     ...
     for(int32_t i = 0; i != tot_samples; ++i) { fprintf(fp3,"%.16f\n",PMC2_f64[i]);}
     ...
     double * __restrict PMC3_mnt  = NULL; // 3rd moment
     double PMC3_xmean = 0.0;
     PMC3_acor = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==PMC3_acor) { MALLOC_FAILED}
     PMC3_acov = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==PMC3_acov)  { MALLOC_FAILED};
     PMC3_mnt = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*lag1*sizeof(double),alignment));
     ...
     for(int32_t i = 0; i != (lag1*lag1); i += 16) {
         _mm512_stream_pd(&PMC3_mnt[i+0], vzf64);
	 _mm512_stream_pd(&PMC3_mnt[i+8], vzf64);
     }
     // Call F77 C-interface to THIRMOF subroutine
	...
	std::exit(EXIT_FAILURE);
     ...
     for(int32_t i = 0; i != tot_samples; ++i) { fprintf(fp4,"%.16f\n",PMC3_f64[i]);}
     ...
     FILE * fp5;
     double * __restrict FCRef_acor = NULL; // 
     double * __restrict FCRef_acov = NULL; // 
     double * __restrict FCRef_mnt  = NULL; // 
     double FCRef_xmean = 0.0;
     FCRef_acor = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==FCRef_acor) { MALLOC_FAILED}
     FCRef_acov = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==FCRef_acov)  { MALLOC_FAILED};
     FCRef_mnt = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*lag1*sizeof(double),alignment));
     if(NULL==FCRef_mnt) { MALLOC_FAILED};
     // Initialize the result arrays
     for(int32_t i = 0; i != lag1; i += 16) {
       _mm512_stream_pd(&FCRef_acor[i+0], vzf64);
       _mm512_stream_pd(&FCRe_acov[i+0], vzf64);
       _mm512_stream_pd(&FCRef_acor[i+8], vzf64);
       _mm512_stream_pd(&FCRef_acov[i+8], vzf64);
     }
     for(int32_t i = 0; i != (lag1*lag1); i += 16) {
         _mm512_stream_pd(&FCRef_mnt[i+0], vzf64);
	 _mm512_stream_pd(&FCRef_mnt[i+8], vzf64);
     }
     // Call F77 C-interface to THIRMOF subroutine
	...
	std::exit(EXIT_FAILURE);
     ...
     for(int32_t i = 0; i != tot_samples; ++i) { fprintf(fp5,"%.16f\n",FCRef_f64[i]);}
     ...
     FILE * fp6;
     double * __restrict FCAct_acor = NULL; //
     double * __restrict FCAct_acov = NULL; // 
     double * __restrict FCAct_mnt  = NULL; //
     double FCAct_xmean = 0.0;
     FCAct_acor = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==FCAct_acor) { MALLOC_FAILED}
     FCAct_acov = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==FCAct_acov)  { MALLOC_FAILED};
     FCAct_mnt = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*lag1*sizeof(double),alignment));
     ...
     for(int32_t i = 0; i != (lag1*lag1); i += 16) {
         _mm512_stream_pd(&FCAct_mnt[i+0], vzf64);
	 _mm512_stream_pd(&FCAct_mnt[i+8], vzf64);
     }
     // Call F77 C-interface to THIRMOF subroutine
	...
	std::exit(EXIT_FAILURE);
     ...
     for(int32_t i = 0; i != tot_samples; ++i) { fprintf(fp6,"%.16f\n",FCAct_f64[i]);}
     ...
     FILE * fp7;
     double * __restrict FCIns_acor = NULL; //
     double * __restrict FCIns_acov = NULL; // 
     double * __restrict FCIns_mnt  = NULL; // 
     double FCIns_xmean = 0.0;
     FCIns_acor = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==FCIns_acor) { MALLOC_FAILED}
     FCIns_acov = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==FCIns_acov)  { MALLOC_FAILED};
     FCIns_mnt = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*lag1*sizeof(double),alignment));
     ...
     for(int32_t i = 0; i != (lag1*lag1); i += 16) {
         _mm512_stream_pd(&FCIns_mnt[i+0], vzf64);
	 _mm512_stream_pd(&FCIns_mnt[i+8], vzf64);
     }
     // Call F77 C-interface to THIRMOF subroutine
	...
	std::exit(EXIT_FAILURE);
     ...
     for(int32_t i = 0; i != tot_samples; ++i) { fprintf(fp7,"%.16f\n",FCIns_f64[i]);}
     ...
     FILE * fp8;
     double * __restrict TSC_acor = NULL; //
     double * __restrict TSC_acov = NULL; // 
     double * __restrict TSC_mnt  = NULL; //
     double TSC_xmean = 0.0;
     TSC_acor = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==TSC_acor) { MALLOC_FAILED}
     TSC_acov = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==TSC_acov)  { MALLOC_FAILED};
     TSC_mnt = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*lag1*sizeof(double),alignment));
     ...
     for(int32_t i = 0; i != (lag1*lag1); i += 16) {
         _mm512_stream_pd(&TSC_mnt[i+0], vzf64);
	 _mm512_stream_pd(&TSC_mnt[i+8], vzf64);
     }
     // Call F77 C-interface to THIRMOF subroutine
	...
	std::exit(EXIT_FAILURE);
     ...
     for(int32_t i = 0; i != tot_samples; ++i) { fprintf(fp8,"%.16f\n",TSC_f64[i]);}
     ...
     FILE * fp9;
     double * __restrict Core_util_acor = NULL; 
     double * __restrict Core_util_acov = NULL; // 
     double * __restrict Core_util_mnt  = NULL; // 
     double Core_util_xmean = 0.0;
     Core_util_acor = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==Core_util_acor) { MALLOC_FAILED}
     Core_util_acov = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==Core_util_acov)  { MALLOC_FAILED};
     Core_util_mnt = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*lag1*sizeof(double),alignment));
     ...
     for(int32_t i = 0; i != (lag1*lag1); i += 16) {
         _mm512_stream_pd(&Core_util_mnt[i+0], vzf64);
	 _mm512_stream_pd(&Core_util_mnt[i+8], vzf64);
     }
     // Call F77 C-interface to THIRMOF subroutine
	...
	std::exit(EXIT_FAILURE);
     ...
     for(int32_t i = 0; i != tot_samples; ++i) { fprintf(fp9,"%.16f\n",Core_util_f64[i]);}
     ...
     FILE * fp10;
     double * __restrict Core_avg_f_acor = NULL; 
     double * __restrict Core_avg_f_acov = NULL; // 
     double * __restrict Core_avg_f_mnt  = NULL; // 
     double Core_avg_f_xmean = 0.0;
     Core_avg_f_acor = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==Core_avg_f_acor) { MALLOC_FAILED}
     Core_avg_f_acov = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*sizeof(double),alignment));
     if(NULL==Core_avg_f_acov)  { MALLOC_FAILED};
     Core_avg_f_mnt = reinterpret_cast<double*>(_mm_malloc((std::size_t)lag1*lag1*sizeof(double),alignment));
     ...
     for(int32_t i = 0; i != (lag1*lag1); i += 16) {
         _mm512_stream_pd(&Core_avg_f_mnt[i+0], vzf64);
	 _mm512_stream_pd(&Core_avg_f_mnt[i+8], vzf64);
     }
     // Call F77 C-interface to THIRMOF subroutine
	...
	std::exit(EXIT_FAILURE);
     ...
     for(int32_t i = 0; i != tot_samples; ++i) { fprintf(fp10,"%.16f\n",Core_avg_f_f64[i]);}
     ...
     _mm_free(Core_avg_f_acov);   _mm_free(Core_avg_f_acor); _mm_free(Core_avg_f_mnt);
     _mm_free(Core_util_acov);    _mm_free(Core_util_acor);  _mm_free(Core_util_mnt);
     _mm_free(TSC_acov);          _mm_free(TSC_acor);        _mm_free(TSC_mnt);
     _mm_free(FCIns_acov);        _mm_free(FCIns_acor);      _mm_free(FCIns_mnt);
     _mm_free(FCAct_acov);        _mm_free(FCAct_acor);      _mm_free(FCAct_mnt);
     _mm_free(FCRef_acov);        _mm_free(FCRef_acor);      _mm_free(FCRef_mnt);
     _mm_free(PMC3_acov);         _mm_free(PMC3_acor);       _mm_free(PMC3_mnt);
     _mm_free(PMC2_acov);         _mm_free(PMC2_acor);       _mm_free(PMC2_mnt);
     _mm_free(PMC1_acov);         _mm_free(PMC1_acor);       _mm_free(PMC1_mnt);
     _mm_free(PMC0_acov);         _mm_free(PMC0_acor);       _mm_free(PMC0_mnt);
}
