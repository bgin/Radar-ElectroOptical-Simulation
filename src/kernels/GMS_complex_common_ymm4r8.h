...
#ifndef __GMS_COMPLEX_COMMON_YMM4R8_H__
#define __GMS_COMPLEX_COMMON_YMM4R8_H__ 061020191431

namespace file_info {


	const unsigned int GMS_COMPLEX_COMMON_YMM4R8_MAJOR = 1U;

	const unsigned int GMS_COMPLEX_COMMON_YMM4R8_MINOR = 0U;

	const unsigned int GMS_COMPLEX_COMMON_YMM4R8_MICRO = 0U;

	const unsigned int GMS_COMPLEX_COMMON_YMM4R8_FULLVER = 
	 1000U*gGMS_COMPLEX_COMMON_YMM4R8_MAJOR+100U*gGMS_COMPLEX_COMMON_YMM4R8_MINOR+10U*gGMS_COMPLEX_COMMON_YMM4R8_MICRO;

	const char * const GMS_COMPLEX_COMMON_YMM4R8_CREATE_DATE = "06-10-2019 14:31 +00200 (SUN 06 OCT 2019 GMT+2)";

	const char * const GMS_COMPLEX_COMMON_YMM4R8_BUILD_DATE = __DATE__ ":" __TIME__;

	const char * const GMS_COMPLEX_COMMON_YMM4R8_AUTHOR = "Programmer: Bernard Gingold, contact: beniekg@gmail.com";

	const char * const GMS_COMPLEX_COMMON_YMM4R8_SYNOPSIS = "Common procedures for AVX complex field (1D) classes.";
...
#define AVX_COMPLEX_ADDITION(out,v1,v2,idx,off) \
	(out) = _mm256_add_pd(_mm256_mul_pd(_mm256_load_pd(&(v1).m_Re[(idx)+(off)]), \
	_mm256_load_pd(&(v2).m_Re[(idx)+(off)])), _mm256_mul_pd(_mm256_load_pd(&(v1).m_Im[(idx)+(off)]), \
	_mm256_load_pd(&(v2).m_Im[(idx)+(off)])));
#endif

#if !defined (AVX_COMPLEX_SUBTRACTION)
#define AVX_COMPLEX_SUBTRACTION(out,v1,v2,idx,off) \
	(out) = _mm256_sub_pd(_mm256_mul_pd(_mm256_load_pd(&(v1).m_Im[(idx)+(off)]), \
	_mm256_load_pd(&(v2).m_Re[(idx)+(off)])), _mm256_mul_pd(_mm512_load_pd(&(v1).m_Re[(idx)+(off)]), \
...
#define AVX_COMPLEX_MAGNITUDE(out,v1,v2,idx,off) \
	(out) = _mm256_sqrt_pd(_mm256_add_pd(_mm256_mul_pd(_mm256_load_pd(&(v1).m_Re[(idx)+(off)]), \
	_mm256_load_pd(&(v2).m_Re[(idx)+(off)])), _mm256_mul_pd(_mm256_load_pd(&(v1).m_Im[(idx)+(off)]), \
	_mm256_load_pd(&(v2).m_Im[(idx)+(off)]))));
#endif
...
#define AVX_HORIZONTAL_ADD(sum,ymmx,tmp) \
	...
	(sum) = _mm_cvtsd_f64(_mm_add_sd(_mm256_castpd256_pd128((tmp)),_mm256_extractf128_pd((tmp),1)));
			...
			avx256_cnormalize_prod(AVXVField1D &out,
				...
				int32_t i;
				if (do_nt_store) {
					for (i = 0; i != ROUND_TO_FOUR(v1.m_nsize, 4); i += 4) {
						const __m256d ymm0(_mm256_load_pd(&v1.m_Re[i]));
						const __m256d ymm1(_mm256_load_pd(&v2.m_Re[i]));
						const __m256d ymm2(_mm256_load_pd(&v1.m_Im[i]));
						const __m256d ymm3(_mm256_load_pd(&v2.m_Im[i]));
						...
						_mm256_stream_pd(&out.m_Re[i], _mm256_div_pd(re_part,mag_part));
						_mm256_stream_pd(&out.m_Im[i], _mm256_div_pd(im_part,mag_term));
					...
					for (i = 0; i != ROUND_TO_FOUR(v1.m_nsize, 4); i += 4) {
						...
						const __m256d sqrt_term2(_mm256_mul_pd(im_part, im_part));
						const __m256d mag_term(_mm256_sqrt_pd(_mm256_add_pd(sqrt_term1, sqrt_term2)));
						_mm256_store_pd(&out.m_Re[i], _mm256_div_pd(re_part, mag_part));
						_mm256_store_pd(&out.m_Im[i], _mm256_div_pd(im_part, mag_term));
			...
			avx256_cmean_prod(std::complex<double> &mean,
			    ...
			    __ATTR_ALIGN__(64) struct {
					...
					int32_t i; char pad[28];
			    }ca;

				for (ca.i = 0; i != ROUND_TO_FOUR(v1.m_nsize, 4); ca.i += 4) {
					const __m256d ymm0(_mm256_load_pd(&v1.m_Re[i]));
					const __m256d ymm1(_mm256_load_pd(&v2.m_Re[i]));
					const __m256d ymm2(_mm256_load_pd(&v1.m_Im[i]));
					const __m256d ymm3(_mm256_load_pd(&v2.m_Im[i]));
			...
			avx256_cconj_prod(AVXVField1D &out,
				...
				int32_t i;
				if (do_nt_store) {
					for (i = 0; i != ROUND_TO_FOUR(out.m_nsize, 4); i += 4) {
						const __m256d ymm0(_mm256_load_pd(&v1.m_Re[i]));
						const __m256d ymm1(_mm256_load_pd(&v2.m_Re[i]));
						const __m256d ymm2(_mm256_load_pd(&v1.m_Im[i]));
						const __m256d ymm3(_mm256_load_pd(&v2.m_Im[i]));
						_mm256_stream_pd(&out.m_Re[i], _mm256_add_pd(
							_mm256_mul_pd(ymm0, ymm1), _mm256_mul_pd(ymm2,ymm3)));
						_mm256_stream_pd(&out.m_Im[i], _mm256_sub_pd(
					...
					for (i = 0; i != ROUND_TO_FOUR(out.m_nsize, 4); i += 4) {
						...
						const __m256d ymm2(_mm256_load_pd(&v1.m_Im[i]));
						const __m256d ymm3(_mm256_load_pd(&v2.m_Im[i]));
						_mm256_store_pd(&out.m_Re[i], _mm256_add_pd(
							_mm256_mul_pd(ymm0, ymm1), _mm256_mul_pd(ymm2, ymm3)));
						_mm256_store_pd(&out.m_Im[i], _mm256_sub_pd(
							_mm256_mul_pd(ymm2, ymm1), _mm256_mul_pd(ymm0, ymm3)));
					}
					for (; i != v1.m_nsize; ++i) { 
						out.m_Re[i] = (v1.m_Re[i] * v2.m_Re[i]) + (v1.m_Im[i] * v2.m_Im[i]);
			...
			avx256_cnorm_conjprod(AVXVField1D &out,
				...
				if (do_nt_store) {
					for (i = 0; i != ROUND_TO_FOUR(out.m_nsize, 4); i += 4) {
						...
						_mm256_stream_pd(&out.m_Re[i], _mm256_div_pd(re_part,vcmag));
						_mm256_stream_pd(&out.m_Im[i], _mm256_div_pd(im_part,vcmag));
					}
					...
					for (i = 0; i != ROUND_TO_FOUR(out.m_nsize, 4); i += 4) {
						...
						const __m256d mag_c2(_mm256_mul_pd(im_part, im_part));
						const __m256d vcmag(_mm256_sqrt_pd(_mm256_add_pd(mag_c1, mag_c2)));
						_mm256_store_pd(&out.m_Re[i], _mm256_div_pd(re_part, vcmag));
						_mm256_store_pd(&out.m_Im[i], _mm256_div_pd(im_part, vcmag));
					}
					for (; i != out.m_nsize; ++i) { 
						const double re = (v1.m_Re[i] * v2.m_Re[i]) - (v1.m_Im[i] * v2.m_Im[i]);
						const double im = (v1.m_Im[i] * v2.m_Re[i]) + (v1.m_Re[i] * v2.m_Im[i]);
			...
			avx256_cmean_conjprod(std::complex<double> &mean,
				...
				for (ca.i = 0; i != ROUND_TO_FOUR(v1.m_nsize, 4); i += 4) {
			...
			avx256_arith_mean(std::complex<double> &mean,
				...
				for (ca.i = 0; i != ROUND_TO_FOUR(v.m_nsize, 4); ca.i += 4) {
					ca2.re_part = _mm256_load_pd(&v.m_Re[i]);
					AVX_HORIZONTAL_ADD(ca.sumre,ca2.re_part,ca2.tmp1)
					ca.accre += ca.sumre;
					ca2.im_part = _mm256_load_pd(&v.m_Im[i]);
				...
				if (do_nt_store) {
					...
					for (i = 0, i != ROUND_TO_FOUR(out.m_nsize,4); i += 4) {
						AVX_COMPLEX_MAGNITUDE(cvmag, v1, v2, i, 0)
							const __m256d re_part(_mm256_load_pd(&v1.m_Re[i]));
						_mm256_stream_pd(&out.m_Re[i], _mm256_div_pd(re_part, cvmag));
						const __m256d im_part(_mm256_load_pd(&v1.m_Im[i]));
						_mm256_stream_pd(&out.m_Im[i], _mm256_div_pd(im_part, cvmag));
					}
					for (; i != v1.m_nsize; ++i) {
						const double cmag = std::sqrt(v1.m_Re[i] * v2.m_Re[i] + v1.m_Im[i] * v2.m_Im[i]);
					...
					for (i = 0, i != ROUND_TO_FOUR(out.m_nsize, 4); i += 4) {
						AVX_COMPLEX_MAGNITUDE(cvmag, v, cv, i, 0)
							const __m256d re_part(_mm256_load_pd(&v1.m_Re[i]));
						_mm256_store_pd(&out.m_Re[i], _mm256_div_pd(re_part, cvmag));
						    const __m256d im_part(_mm256_load_pd(&v1.m_Im[i]));
						_mm256_store_pd(&out.m_Im[i], _mm256_div_pd(im_part, cvmag));
					}
					for (; i != v1.m_nsize; ++i) {
						const double cmag = std::sqrt(v1.m_Re[i] * v2.m_Re[i] + v1.m_Im[i] * v2.m_Im[i]);
						out.m_Re[i] = v1.m_Re[i] / cmag;
			...
			avx256_cmagnitude(AVXVField1D &out,
				...
				for (i = 0; i != ROUND_TO_FOUR(out.m_nsize, 4); i += 4) {
						...
						_mm256_store_pd(&out.m_Re[i], vcmag);
				}
				for (; i != out.m_nsize; ++i) {
...
#endif /*__GMS_COMPLEX_COMMON_YMM4R8_H__*/
