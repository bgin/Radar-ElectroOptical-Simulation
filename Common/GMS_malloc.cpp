...
::gms_edmmap_4KiB(   const std::size_t length,
      ...
      ptr = reinterpret_cast<double*>(mmap(NULL,static_cast<std::size_t>(len),
                                           prot,flag,fd,0));
      if((ptr == (void*)(-1))) {
#if (PRINT_CALLSTACK_ON_ERROR) == 1
...
::gms_efmmap_4KiB(   const std::size_t length,
      ...
      len = n4KiB_pages*4096;
      ptr = reinterpret_cast<float*>(mmap(NULL,static_cast<std::size_t>(len),
                                           prot,flag,fd,0));
      if((ptr == (void*)(-1))) {
#if (PRINT_CALLSTACK_ON_ERROR) == 1
	  std::cerr << "Requested stack-backtrace -- not implemented yet!!"
...
#define MMAP_FUNC_4KIB_BODY(type)   int32_t * (type) = nullptr;  \
                                    int32_t n4KiB_pages = 0;     \
                                    int32_t len = 0;             \
                                    len = static_cast<int32_t>(length);  \
                                    n4KiB_pages = len/4096;              \
...
::gms_immap_4KiB(    const std::size_t length,
      ...
      len = n4KiB_pages*4096;
      ptr = reinterpret_cast<int32_t*>(mmap(NULL,static_cast<std::size_t>(len),
                                           prot,flag,fd,0));
      if((ptr == (void*)(-1))) {
#if (PRINT_CALLSTACK_ON_ERROR) == 1
	  std::cerr << "Requested stack-backtrace -- not implemented yet!!"
...
::gms_ec4mmap_4KiB(  const std::size_t length,
      ...
      len = n4KiB_pages*4096;
      ptr = reinterpret_cast<std::complex<float>*>(mmap(NULL,static_cast<std::size_t>(len),
                                           prot,flag,fd,0));
      if((ptr == (void*)(-1))) {
#if (PRINT_CALLSTACK_ON_ERROR) == 1
	  std::cerr << "Requested stack-backtrace -- not implemented yet!!"
...
::gms_avxc8f32_mmap_4KiB( const std::size_t length,
      ...
      len = n4KiB_pages*4096;
      ptr = reinterpret_cast<AVXc8f32*>(mmap(NULL,static_cast<std::size_t>(len),
                                           prot,flag,fd,0));
      if((ptr == (void*)(-1))) {
#if (PRINT_CALLSTACK_ON_ERROR) == 1
	  std::cerr << "Requested stack-backtrace -- not implemented yet!!"
