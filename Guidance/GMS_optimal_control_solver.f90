...
#if defined(__GFORTRAN__) && (!defined(__ICC) || !defined(__INTEL_COMPILER))
SUBROUTINE SPLINE (MODE, N, X, Y, P, A, B, C) !GCC$ ATTRIBUTES hot :: SPLINE !GCC$ ATTRIBUTES aligned(32) :: SPLINE
#elif defined(__INTEL_COMPILER) || defined(__ICC)
  SUBROUTINE SPLINE (MODE, N, X, Y, P, A, B, C)
       !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: SPLINE
    !DIR$ OPTIMIZE : 3
    !DIR$ ATTRIBUTES OPTIMIZATION_PARAMETER: TARGET_ARCH=skylake_avx512 :: SPLINE
#endif
     use omp_lib
     implicit none
...
!C   SUBROUTINES REQUIRED: NONE
      ...
      DOUBLE PRECISION, DIMENSION(:), ALLOCATABLE :: C
      DOUBLE PRECISION :: C1,C2,D1,D2,H,HP,U,V,W,Z, &
...
!C**  SOLUTION OF THE TRIDIAGONAL SYSTEM  **
  ...
  160     C(IBACK) = (C(IBACK)-B(IBACK+1)*C(IBACK+1))/A(IBACK)
       ...
       !$OMP SIMD ALIGNED(X:64,A,B,P) LINEAR(I:1)    
      DO 170 I = 1,N-1
          H = X(I+1)-X(I)
          HP = H*P(I)
          IF(HP.GT.HALF) THEN
...
#if defined(__GFORTRAN__) && (!defined(__ICC) || !defined(__INTEL_COMPILER))
DOUBLE PRECISION FUNCTION SPLINT (MODE,N,X,Y,P,A,B,C,XS) !GCC$ ATTRIBUTES hot :: SPLINT !GCC$ ATTRIBUTES aligned(32) :: SPLINT
#elif defined(__INTEL_COMPILER) || DEFINED(__ICC)
...
!C      * LEFT
      ...
      INTEGER :: IP,IS,LEFT,MFLAG,MODE,N
      
      DOUBLE PRECISION :: X(N),Y(N),P(N),A(N),B(N),C(N),H,U,V,Z,D1,D2,HP,XS,XT, &
...
#if defined(__GFORTRAN__) && (!defined(__ICC) || !defined(__INTEL_COMPILER))
SUBROUTINE GENERA(N,X,Y,P,D) !GCC$ ATTRIBUTES inline :: GENERA !GCC$ ATTRIBUTES aligned(32) :: GENERA
#elif defined(__INTEL_COMPILER) || defined(__ICC)
  SUBROUTINE GENERA(N,X,Y,P,D)
      !DIR$ ATTIRBUTES FORCEINLINE :: GENERA
      !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: GENERA
    !DIR$ OPTIMIZE : 3
#endif
  implicit none
...
#if defined(__GFORTRAN__) && (!defined(__ICC) || !defined(__INTEL_COMPILER))
INTEGER FUNCTION LEFT(LXT,XT,X,MFLAG) !GCC$ ATTRIBUTES inline :: LEFT !GCC$ ATTRIBUTES aligned(32) :: LEFT
#elif defined(__INTEL_COMPILER) || defined(__ICC)
  INTEGER FUNCTION LEFT(LXT,XT,X,MFLAG)
      !DIR$ ATTIRBUTES FORCEINLINE :: LEFT
      !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: LEFT
    !DIR$ OPTIMIZE : 3
#endif
     implicit none
...
#if defined(__GFORTRAN__) && (!defined(__ICC) || !defined(__INTEL_COMPILER))
SUBROUTINE RK54 (F,N,Y,T,TOUT,RELERR,ABSERR,IFLAG,WORK,IWORK,W,IW) !GCC$ ATTRIBUTES inline :: RK54 !GCC$ ATTRIBUTES aligned(32) :: RK54
#elif defined(__INTEL_COMPILER) || defined(__ICC)
  SUBROUTINE RK54 (F,N,Y,T,TOUT,RELERR,ABSERR,IFLAG,WORK,IWORK,W,IW)
      !DIR$ ATTIRBUTES FORCEINLINE :: RK54
      !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: RK54
    !DIR$ OPTIMIZE : 3
#endif
    implicit none
...
SUBROUTINE RKC54(F,N,Y,T,TOUT,RELERR,ABSERR,IFLAG,YP,H,F1,F2,F3, &
     F4,F5,F6,F7,SAVRE,SAVAE,NFE,KOP,INIT,JFLAG,KFLAG,W,IW) !GCC$ ATTRIBUTES hot :: RKC54 !GCC$ ATTRIBUTES aligned(32) :: RKC54
#elif defined(__INTEL_COMPILER) || defined(__ICC)
SUBROUTINE RKC54(F,N,Y,T,TOUT,RELERR,ABSERR,IFLAG,YP,H,F1,F2,F3, &
     F4,F5,F6,F7,SAVRE,SAVAE,NFE,KOP,INIT,JFLAG,KFLAG,W,IW)
    !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: RKC54
    !DIR$ OPTIMIZE : 3
    !DIR$ ATTRIBUTES OPTIMIZATION_PARAMETER: TARGET_ARCH=Haswell :: RKC54
#endif
     implicit none
...
!C-----------------------------------------------------------------------
      ...
      DOUBLE PRECISION Y(N),YP(N),F1(N),F2(N),F3(N),F4(N),F5(N),F6(N), &
                       F7(N),W(*),A,ABSERR,AE,ALF,TOL,TOLN,YPK, &
                       DT,EE,EEOET,ESTOLD,ESTTOL,ET,H,HMIN,HOLD, &
                       RELERR,REMIN,RE,S,SAVAE,SAVRE,SCALE,T,TOUT,U,U26
...
SUBROUTINE PD54(F, N, Y, T, H, F1, F2, F3, F4, F5, F6, F7, &
     E, S, W, IW) !GCC$ ATTRIBUTES hot :: PD54 !GCC$ ATTRIBUTES aligned(32) :: PD54
#elif defined(__INTEL_COMPILER) || defined(__ICC)
     !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: PD54
    !DIR$ OPTIMIZE : 3
    !DIR$ ATTRIBUTES OPTIMIZATION_PARAMETER: TARGET_ARCH=Haswell :: PD54
#endif
     implicit none
...
!C   RUNGE-KUTTA FORMULAE
   ...
   20 E(I) = Y(I)+T1*F1(I)+T2*F2(I)
      CALL F (T+0.33333333333333333333333333D0*H, E, F3, W, IW)
      T1 = 0.9777777777777777777778D0*H
      T2 = −3.7333333333333333333333D0*H
      T3 = 3.5555555555555555555556D0*H
     ! T1 = (          44.0D0
    ! .     /          45.0D0) * H
     ! T2 = (         -56.0D0
    ! .     /          15.0D0) * H
   ...
   30 E(I) = Y(I)+T1*F1(I)+T2*F2(I)+T3*F3(I)
      CALL F (T+0.8D0*H, E, F4, W, IW)
      T1 = 2.9525986892242036274958D0*H
      T2 = −11.595793324188385916781D0*H
      T3 = 9.822892851699436061576D0*H
      T4 = −0.2908093278463648834019D0*H
      !T1 = (       19372.0D0
     !.     /        6561.0D0) * H
      !T2 = (      -25360.0D0
    ! .     /        2187.0D0) * H
   ...
   40 E(I) = Y(I)+T1*F1(I)+T2*F2(I)+T3*F3(I)+T4*F4(I)
      CALL F (T+0.8888888888888888888889D0*H, E, F5, W, IW)
      T1 = 2.8462752525252525252525D0*H
      T2 = −10.7575757575757575757576D0*H
      T3 = 8.9064227177434724604536D0*H
      T4 = 0.2784090909090909090909D0*H
      T5 = −0.2735313036020583190395D0*H
     ! T1 = (        9017.0D0
    ! .     /        3168.0D0) * H
     ! T2 = (        -355.0D0
    ! .     /          33.0D0) * H
   ...
   50 E(I) = Y(I)+T1*F1(I)+T2*F2(I)+T3*F3(I)+T4*F4(I)+T5*F5(I)
      ...
      T1 = 0.0911458333333333333333D0*H
      T3 = 0.449236298292902066487D0*H
      T4 = 0.6510416666666666666667D0*H
      T5 = −0.3223761792452830188679D0*H
      T6 = 0.130952380952380952381D0*H
     ! T1 = (          35.0D0
    ! .     /         384.0D0) * H
     ! T3 = (         500.0D0
    ! .     /        1113.0D0) * H
...
!C   LOCALLY EXTRAPOLATED SOLUTION & ERROR
   ...
   !  .     /          40.0D0) * F7(I))
     ! 70 S(I) = Y(I)+H*S(I)
      DO I = 1, N
         S(I) = T1*F1(I)+T3*F3(I)+T4*F4(I)+T5*F5(I)+T6*F6(I)
         E(I) = S(I)-0.0899131944444444444444D0*F1(I)+ &
                     0.4534890685834082060497D0*F3(I)+ &
                     0.6140625D0*F4(I)+ &
                     −0.2715123820754716981132*F5(I)+ &
                     0.089047619047619047619D0*F6(I)+ &
                     0.025D0*F7(I)
         S(I) = Y(I)+H*S(I)
      END DO
END SUBROUTINE
...
#if defined(__GFORTRAN__) && (!defined(__ICC) || !defined(__INTEL_COMPILER))
SUBROUTINE RK87 (F,N,Y,T,TOUT,RELERR,ABSERR,IFLAG,WORK,IWORK,W,IW) !GCC$ ATTRIBUTES inline :: RK87 !GCC$ ATTRIBUTES aligned(32) :: RK87
#elif defined(__INTEL_COMPILER) || defined(__ICC)
  SUBROUTINE RK87 (F,N,Y,T,TOUT,RELERR,ABSERR,IFLAG,WORK,IWORK,W,IW)
      !DIR$ ATTIRBUTES FORCEINLINE :: RK87
      !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: RK87
    !DIR$ OPTIMIZE : 3
#endif
    implicit none
...
!C-----------------------------------------------------------------------
      ...
      CALL RKC87(F,N,Y,T,TOUT,RELERR,ABSERR,IFLAG,WORK(1),WORK(K1M), &
       ...
       WORK(K7),WORK(K8),WORK(K9),WORK(K10),WORK(K11),WORK(K12), &
       WORK(K12+1),IWORK(1),IWORK(2),IWORK(3),IWORK(4),IWORK(5),W,IW)
     
END SUBROUTINE

#if defined(__GFORTRAN__) && (!defined(__ICC) || !defined(__INTEL_COMPILER))
SUBROUTINE RKC87(F,N,Y,T,TOUT,RELERR,ABSERR,IFLAG,YP,H,F1,F2,F3,F4, &
     F5,F6,F7,F8,F9,F10,F11,SAVRE,SAVAE,NFE,KOP,INIT,JFLAG,KFLAG,W,IW) !GCC$ ATTRIBUTES hot :: RKC87 !GCC$ ATTRIBUTES aligned(32) :: RKC87
#elif defined(__INTEL_COMPILER) || defined(__ICC)
SUBROUTINE RKC87(F,N,Y,T,TOUT,RELERR,ABSERR,IFLAG,YP,H,F1,F2,F3,F4, &
     F5,F6,F7,F8,F9,F10,F11,SAVRE,SAVAE,NFE,KOP,INIT,JFLAG,KFLAG,W,IW)
    !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: RKC87
    !DIR$ OPTIMIZE : 3
    !DIR$ ATTRIBUTES OPTIMIZATION_PARAMETER: TARGET_ARCH=Haswell :: RKC87
#endif
  implicit none
...
!C-----------------------------------------------------------------------
      ...
      INTEGER          IFLAG, INIT, JFLAG, K, KFLAG, KOP, &
                      MAXNFE, MFLAG, N, NFE
      DOUBLE PRECISION Y(N),YP(N),F1(N),F2(N),F3(N),F4(N),F5(N),F6(N), &
                      F7(N),F8(N),F9(N),F10(N),F11(N),A,ABSERR,AE,ALF, &
                      DT,EE,EEOET,ESTOLD,ESTTOL,ET,H,HMIN,HOLD, &
                      RELERR,REMIN,RE,S,SAVAE,SAVRE,SCALE,T,TOUT,U,U26, &
                      TOL,TOLN,YPK,W(*)
      LOGICAL          FIRST,HFAILD,OUTPUT
...
SUBROUTINE PD87(F, N, Y, T, H, F1, F2, F3, F4, F5, &
     F6, F7, F8, F9, F10, F11, E, S, W, IW) !GCC$ ATTRIBUTES hot :: PD87 !GCC$ ATTRIBUTES aligned(32) :: PD87
#elif defined(__INTEL_COMPILER) || defined(__ICC)
SUBROUTINE PD87(F, N, Y, T, H, F1, F2, F3, F4, F5, &
     F6, F7, F8, F9, F10, F11, E, S, W, IW)
   !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: PD87
    !DIR$ OPTIMIZE : 3
    !DIR$ ATTRIBUTES OPTIMIZATION_PARAMETER: TARGET_ARCH=Haswell :: PD87
#endif
      implicit none
...
!C   RUNGE-KUTTA FORMULAE
    ...
    !  T1=  (           1.0D0
      ! .     /          18.0D0) * H
      T1 = 0.0555555555555555555556D0*H
      DO 10 I=1,N
   10 E(I) = Y(I)+T1*F1(I)
   ...
   50 E(I) = Y(I)+T1*F1(I)+T2*F2(I)+T3*F3(I)
      ...
      T4 = 0.0128468238888888888889D0*H
      !T1 = (    29443841.0D0
     !.!     /   614563906.0D0) * H
      !T2 = (    77736538.0D0
     !.     /   692538347.0D0) * H
   ...
   60 E(I) = Y(I)+T1*F1(I)+T2*F2(I)+T3*F3(I)+T4*F4(I)
      ...
      T4 = 0.1969702142156660597788D0*H
      T5 = 0.1727138523405018379264D0*H
      !T1 = (    16016141.0D0
     !.     /   946692911.0D0) * H
      !T2 = (    61564180.0D0
     !.     /   158732637.0D0) * H
   ...
   70 E(I) = Y(I)+T1*F1(I)+T2*F2(I)+T3*F3(I)+T4*F4(I)+T5*F5(I)
      ...
      T4 = 0.1386503094588252542832D0*H
      T5 = 0.9409286140357562682278D0*H
      T6 = 0.2116363264819439814061D0*H
      !T1 = (    39632708.0D0
     !.     /   573591083.0D0) * H
      !T2 = (  -433636366.0D0
     !.     /   683701615.0D0) * H
   ...
   90 E(I) = Y(I)+T1*F1(I)+T2*F2(I)+T3*F3(I)+T4*F4(I)+T5*F5(I)+T6*F6(I)+ &
      ...
      T4 = -6.0566058043574709372611D0*H
      T5 = -16.0035735941561780837938D0*H
      T6 = 14.8493030862976625183158D0*H
      T7 = -13.3715757352898492874184D0*H
      T8 = 5.1341826481796379245786D0*H
      !T1 = ( -1028468189.0D0
     !.     /   846180014.0D0) * H
      !T2 = (  8478235783.0D0
     !.     /   508512852.0D0) * H
