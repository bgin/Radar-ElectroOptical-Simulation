...
!   USAGE :
!     CALL d_TOMP (SLV, RHS, VALUES, OUTPUT, IV, X, Y,
!    *              Z, Z0, Z1, ACC, F, C, DF, DC, LC)
...
C     WATCOM-WATFOR77
C     date and time routines are de-activate
#endif      

!-----------------------------------------------------------------------

!   DECLARATION OF VARIABLES , ARRAYS , COMMON AND NAMELIST
!   IBM-MAINFRAME NAMELIST DEACTIVATED
...
!C  FORWARD DIFFERENCES APPROXIMATE THE GRADIENT
            ...
            DF(KP) = (E-F)/DEL
            !$OMP SIMD ALIGNED(DC:64,W,C) LINEAR(I:1) 
            DO 470 I=1,M
...
!C  GRADIENT FOR INITIAL AND FINAL BREAKPOINT OF A CONTROL SEQUENCE
 ...
 480        DF(KP) = ZERO
              !$OMP SIMD ALIGNED(DC:64) LINEAR(I:1) 
            DO 490 I=1,M
               DC(I,KP) = ZERO
  490       CONTINUE
...
!C  EXTREMELY DENSE OUTPUT (IN-LINE CODED)
      ...
      !$OMP SIMD ALIGNED(W:64,Z) LINEAR(I:1)
      DO 550 I=1,NZ
         W(I) = Z(I)
  550 CONTINUE
...
#if defined(__GFORTRAN__) && (!defined(__ICC) || !defined(__INTEL_COMPILER))
      SUBROUTINE CONTRL (T,X,IV,Y) !GCC$ ATTRIBUTES hot :: CONTRL !GCC$ ATTRIBUTES aligned(32) :: CONTRL
#elif defined(__INTEL_COMPILER) || defined(__ICC)
      SUBROUTINE CONTRL (T,X,IV,Y)
         !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: CONTRL
    !DIR$ OPTIMIZE : 3
#endif
