...
/*
   icpc -o unit_test_atan_vec_zmm16r4 -fp-model fast=2 -ftz -ggdb -ipo -qopt-zmm-usage=high -march=skylake-avx512 -mavx512f -falign-functions=32 -w1 -qopt-report=5  \
   GMS_config.h GMS_malloc.h GMS_atan_vec_zmm16r4.h GMS_atan_vec_zmm16r4.cpp unit_test_atan_vec_zmm16r4.cpp
   ASM: 
   icpc -S -fverbose-asm -masm=intel -qopt-zmm-usage=high -march=skylake-avx512 -mavx512f -falign-functions=32 GMS_config.h GMS_malloc.h GMS_atan_vec_zmm16r4.h GMS_atan_vec_zmm16r4.cpp unit_test_atan_vec_zmm16r4.cpp
*/

__attribute__((hot))
__attribute__((noinline))
__attribute__((aligned(32)))
void unit_test_atanv_zmm16r4_unroll_10x_a();
...
void unit_test_atanv_zmm16r4_unroll_10x_a()
{
    ...
    const char * __restrict__ fout{"UNIT-TEST_Output_atanv_zmm16r4_unroll_10x_a.csv"};
    ...
    switch (which)
    {
        case 0 : 
        {
            ...
            printf("[UNIT-TEST]: -- START: cosv_zmm16r4_unroll_10x_a\n");
            gms::math::atanv_zmm16r4_unroll_10x_a(&p_x[0],&p_y[0],a,b,c,nelems);
            fp = fopen(fout,"w+");
            if(!fp)
            {
            ...
            fclose(fp);
            printf("[UNIT-TEST]: -- END: atanv_zmm16r4_unroll_10x_a\n");
        }
        break;
        case 1 :
        {
            ...
            printf("[UNIT-TEST]: -- End of data initialization\n");
            printf("[UNIT-TEST]: -- START: atanv_zmm16r4_unroll_10x_a\n");
            gms::math::atanv_zmm16r4_unroll_10x_a(&p_x[0],&p_y[0],a,b,c,nelems);
            fp = fopen(fout,"w+");
            if(!fp)
            {
...
__attribute__((noinline))
__attribute__((aligned(32)))
void unit_test_atanv_zmm16r4_unroll_6x_a();
...
void unit_test_atanv_zmm16r4_unroll_6x_a()
{
    ...
    float * __restrict__ p_y{NULL};
    FILE  *  __restrict__ fp{NULL};
    const char * __restrict__ fout{"UNIT-TEST_Output_atanv_zmm16r4_unroll_6x_a.csv"};
    constexpr std::size_t nelems{67456};
    constexpr std::size_t nbytes{4ull*nelems};
    std::clock_t seed_d{0ull};
    std::clock_t seed_x{0ull};
    ...
    switch (which)
    {
        case 0 : 
        {
            ...
            printf("[UNIT-TEST]: -- End of data initialization\n");
            printf("[UNIT-TEST]: -- START: atanv_zmm16r4_unroll_6x_a\n");
            gms::math::atanv_zmm16r4_unroll_6x_a(&p_x[0],&p_y[0],a,b,c,nelems);
            fp = fopen(fout,"w+");
            if(!fp)
            {
                 std::perror("fopen failed to open a file -- TERMINATING!!");
            ...
            if(p_y) gms_mm_free(p_y);
            fclose(fp);
            printf("[UNIT-TEST]: -- END: atanv_zmm16r4_unroll_6x_a\n");
        }
        break;
        case 1 :
        {
...
int main()
{
    unit_test_atanv_zmm16r4_unroll_10x_a();
    unit_test_atanv_zmm16r4_unroll_6x_a();
    return 0;
}