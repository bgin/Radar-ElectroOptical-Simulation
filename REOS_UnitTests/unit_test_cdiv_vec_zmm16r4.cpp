...
/*
   icpc -o unit_test_cdiv_vec_zmm16r4 -fp-model fast=2 -ftz -ggdb -ipo -qopt-zmm-usage=high -march=skylake-avx512 -mavx512f -falign-functions=32 -w1 -qopt-report=5 -fopenmp \
   GMS_config.h GMS_malloc.h GMS_cdiv_vec_zmm16r4.h GMS_cdiv_vec_zmm16r4.cpp unit_test_cdiv_vec_zmm16r4.cpp
   ASM: 
   icpc -S -fverbose-asm -masm=intel -qopt-zmm-usage=high -march=skylake-avx512 -mavx512f -fopenmp -falign-functions=32 GMS_config.h GMS_malloc.h GMS_cdiv_vec_zmm16r4.h GMS_cdiv_vec_zmm16r4.cpp unit_test_cdiv_vec_zmm16r4.cpp
*/
...
void unit_test_cdivv_zmm16r4_unroll_10x_a();
...
void unit_test_cdivv_zmm16r4_unroll_10x_a()
{
    ...
    uint32_t coreID_b{9999};
    uint32_t coreID_a{9999};
    float rxim,rxre;
    float ryim,ryre;
    int32_t zcnt1{0};
    int32_t zcnt2{0};
    ...
    switch (which)
    {
        case 0 :
        {
            ...
            const char * __restrict__ fout{"UNIT-TEST_uni_distro_cdivv_zmm16r4_unroll_10x_a.csv"};
...
#pragma omp parallel sections 
{
        ...
        #pragma omp section 
        {
            ...
            printf("[UNIT-TEST]: -- START: cdivv_zmm16r4_unroll_10x_a\n");
            start   = __rdtscp(&coreID_b);
            gms::math::cdivv_zmm16r4_unroll_10x_a(&xre[0],&xim[0],&yre[0],&yim[0],
                                                  &zre[0],&zim[0],nelems);
            end     = __rdtscp(&coreID_a);
            start_c = start-RDTSCP_LAT;
            end_c   = end-RDTSCP_LAT;
            elapsed_tsc = end_c-start_c;
            printf("[UNIT-TEST]: cdivv_zmm16r4_unroll_10x_a -- TSC=%llu,coreID(before)=%d,coreID(after)=%d\n",elapsed_tsc,coreID_b,coreID_a);
            fp = fopen(fout,"w+");
            if(!fp)
            {
            ...
            if(xim_neq_0) gms_mm_free(xim);
            fclose(fp);
            printf("[UNIT-TEST]: -- END: cdivv_zmm16r4_unroll_10x_a\n");
        }
        break;
        case 1 :
        {
            std::normal_distribution<float> normal_distro;
            const char * __restrict__ fout{"UNIT-TEST_norm_distro_cdivv_zmm16r4_unroll_10x_a.csv"};
            char * distro_name{NULL};
            int32_t status{9999};
...
__attribute__((noinline))
__attribute__((aligned(32)))
void unit_test_cdivv_zmm16r4_unroll_8x_a();
...
void unit_test_cdivv_zmm16r4_unroll_8x_a()
{
    ...
    switch (which)
    {
        ...
        {
            std::uniform_real_distribution<float> uniform_distro;
            const char * __restrict__ fout{"UNIT-TEST_uni_distro_cdivv_zmm16r4_unroll_8x_a.csv"};
            char * distro_name{NULL};
            int32_t status{9999};
            distro_name = demangle(typeid(uniform_distro).name(),status);
            if(distro_name != NULL && status == 0)
...
#pragma omp parallel sections 
{
        ...
        #pragma omp section 
        {
            ...
            printf("[UNIT-TEST]: -- START: cdivv_zmm16r4_unroll_8x_a\n");
            start = __rdtscp(&coreID_b);
            gms::math::cdivv_zmm16r4_unroll_8x_a(&xre[0],&xim[0],&yre[0],&yim[0],
                                                  &zre[0],&zim[0],nelems);
            end   = __rdtscp(&coreID_a);
            start_c = start-RDTSCP_LAT;
            end_c   = end-RDTSCP_LAT;
            elapsed_tsc = end_c-start_c;
            printf("[UNIT-TEST]: cdivv_zmm16r4_unroll_8x_a -- TSC=%llu,coreID(before)=%d,coreID(after)=%d\n",elapsed_tsc,coreID_b,coreID_a);
            fp = fopen(fout,"w+");
            if(!fp)
            {
                 std::perror("fopen failed to open a file -- TERMINATING!!");
            ...
            if(xim) gms_mm_free(xim);
            fclose(fp);
            printf("[UNIT-TEST]: -- END: cdivv_zmm16r4_unroll_8x_a\n");
        }
        break;
        case 1 :
        {
            std::normal_distribution<float> normal_distro;
            const char * __restrict__ fout{"UNIT-TEST_norm_distro_cdivv_zmm16r4_unroll_8x_a.csv"};
            char * distro_name{NULL};
            int32_t status{9999};
            distro_name = demangle(typeid(normal_distro).name(),status);
            if(distro_name != NULL && status == 0)
...
__attribute__((noinline))
__attribute__((aligned(32)))
void unit_test_cdivv_zmm16r4_unroll_6x_a();
...
void unit_test_cdivv_zmm16r4_unroll_6x_a()
{
    ...
    switch (which)
    {
        ...
        {
            std::uniform_real_distribution<float> uniform_distro;
            const char * __restrict__ fout{"UNIT-TEST_uni_distro_cdivv_zmm16r4_unroll_6x_a.csv"};
            char * distro_name{NULL};
            int32_t status{9999};
            distro_name = demangle(typeid(uniform_distro).name(),status);
            if(distro_name != NULL && status == 0)
...
#pragma omp parallel sections 
{
        ...
        #pragma omp section 
        {
            ...
            printf("[UNIT-TEST]: -- End of data initialization\n");
            printf("[UNIT-TEST]: Number(s) of zeros found: xre=%d,xim%d,yre=%d,yim=%d\n",zcnt1,zcnt2,zcnt3,zcnt4);
            printf("[UNIT-TEST]: -- START: cdivv_zmm16r4_unroll_6x_a\n");
            start  = __rdtscp(&coreID_b);
            gms::math::cdivv_zmm16r4_unroll_6x_a(&xre[0],&xim[0],&yre[0],&yim[0],
                                                  &zre[0],&zim[0],nelems);
            end    = __rdtscp(&coreID_a);
            start_c = start-RDTSCP_LAT;
            end_c   = end-RDTSCP_LAT;
            elapsed_tsc = end_c-start_c;
            printf("[UNIT-TEST]: cdivv_zmm16r4_unroll_6x_a -- TSC=%llu,coreID(before)=%d,coreID(after)=%d\n",elapsed_tsc,coreID_b,coreID_a);
            fp = fopen(fout,"w+");
            if(!fp)
            {
                 std::perror("fopen failed to open a file -- TERMINATING!!");
            ...
            if(xim) gms_mm_free(xim);
            fclose(fp);
            printf("[UNIT-TEST]: -- END: cdivv_zmm16r4_unroll_6x_a\n");
        }
        break;
        case 1 :
        {
            std::normal_distribution<float> normal_distro;
            const char * __restrict__ fout{"UNIT-TEST_norm_distro_cdivv_zmm16r4_unroll_6x_a.csv"};
            char * distro_name{NULL};
            int32_t status{9999};
            distro_name = demangle(typeid(normal_distro).name(),status);
            if(distro_name != NULL && status == 0)
...
int main()
{
     unit_test_cdivv_zmm16r4_unroll_10x_a();
     unit_test_cdivv_zmm16r4_unroll_8x_a();
     unit_test_cdivv_zmm16r4_unroll_6x_a();
     return 0;
}