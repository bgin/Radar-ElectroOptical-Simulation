...
/*
   icpc -o unit_test_cdiv_vec_zmm16r4_u -fp-model fast=2 -ftz -ggdb -ipo -qopt-zmm-usage=high -march=skylake-avx512 -mavx512f -falign-functions=32 -w1 -qopt-report=5 -fopenmp \
   GMS_config.h GMS_malloc.h GMS_cdiv_vec_zmm16r4.h GMS_cdiv_vec_zmm16r4.cpp unit_test_cdiv_vec_zmm16r4_u.cpp
   ASM: 
   icpc -S -fverbose-asm -masm=intel -qopt-zmm-usage=high -march=skylake-avx512 -mavx512f -fopenmp -falign-functions=32 GMS_config.h GMS_malloc.h GMS_cdiv_vec_zmm16r4.h GMS_cdiv_vec_zmm16r4.cpp unit_test_cdiv_vec_zmm16r4_u.cpp
*/
...
void unit_test_cdivv_zmm16r4_unroll_10x_u();
...
#pragma omp parallel sections 
{
        ...
        #pragma omp section 
        {
            ...
            printf("[UNIT-TEST]: -- START: cdivv_zmm16r4_unroll_10x_u\n");
            start   = __rdtscp(&tsc_aux);
            gms::math::cdivv_zmm16r4_unroll_10x_u(&xre[0],&xim[0],&yre[0],&yim[0],
                                                  &zre[0],&zim[0],nelems);
            end     = __rdtscp(&tsc_aux);
            start_c = start-RDTSCP_LAT;
            end_c   = end-RDTSCP_LAT;
            elapsed_tsc = end_c-start_c;
            printf("[UNIT-TEST]: cdivv_zmm16r4_unroll_10x_u -- TSC=%llu,TSC_AUX=%d\n",elapsed_tsc,tsc_aux);
            fp = fopen(fout,"w+");
            if(!fp)
            {
        ...
        case 1 :
        {
            ...
            const char * __restrict__ fout{"UNIT-TEST_norm_distro_cdivv_zmm16r4_unroll_10x_u.csv"};
            char * distro_name{NULL};
            int32_t status{9999};
...
#pragma omp parallel sections 
{
        ...
        #pragma omp section 
        {
            ...
            printf("[UNIT-TEST]: -- End of data initialization\n");
            printf("[UNIT-TEST]: Number(s) of zeros found: xre=%d,xim%d,yre=%d,yim=%d\n",zcnt1,zcnt2,zcnt3,zcnt4);
            printf("[UNIT-TEST]: -- START: cdivv_zmm16r4_unroll_8x_u\n");
            start  = __rdtscp(&tsc_aux);
            gms::math::cdivv_zmm16r4_unroll_10x_u(&xre[0],&xim[0],&yre[0],&yim[0],
                                                  &zre[0],&zim[0],nelems);
            end     = __rdtscp(&tsc_aux);
            ...
            if(xim) std::free(xim);
            fclose(fp);
            printf("[UNIT-TEST]: -- END: cdivv_zmm16r4_unroll_10x_u\n");
        }
        break;
        default : 
...
__attribute__((noinline))
__attribute__((aligned(32)))
void unit_test_cdivv_zmm16r4_unroll_8x_u();
...
void unit_test_cdivv_zmm16r4_unroll_8x_u()
{
    ...
    switch (which)
    {
        ...
        {
            std::uniform_real_distribution<float> uniform_distro;
            const char * __restrict__ fout{"UNIT-TEST_uni_distro_cdivv_zmm16r4_unroll_8x_u.csv"};
            char * distro_name{NULL};
            int32_t status{9999};
            distro_name = demangle(typeid(uniform_distro).name(),status);
            if(distro_name != NULL && status == 0)
...
#pragma omp parallel sections 
{
        ...
        #pragma omp section 
        {
            ...
            printf("[UNIT-TEST]: -- START: cdivv_zmm16r4_unroll_8x_u\n");
            start = __rdtscp(&tsc_aux);
            gms::math::cdivv_zmm16r4_unroll_8x_u(&xre[0],&xim[0],&yre[0],&yim[0],
                                                  &zre[0],&zim[0],nelems);
            end     = __rdtscp(&tsc_aux);
            start_c = start-RDTSCP_LAT;
            end_c   = end-RDTSCP_LAT;
            elapsed_tsc = end_c-start_c;
            printf("[UNIT-TEST]: cdivv_zmm16r4_unroll_8x_u() -- TSC=%llu,TSC_AUX=%d\n",elapsed_tsc,tsc_aux);
            fp = fopen(fout,"w+");
            if(!fp)
            {
                 std::perror("fopen failed to open a file -- TERMINATING!!");
            ...
            if(xim) std::free(xim);
            fclose(fp);
            printf("[UNIT-TEST]: -- END: cdivv_zmm16r4_unroll_8x_u\n");
        }
        break;
        case 1 :
        {
            std::normal_distribution<float> normal_distro;
            const char * __restrict__ fout{"UNIT-TEST_norm_distro_cdivv_zmm16r4_unroll_8x_u.csv"};
            char * distro_name{NULL};
            int32_t status{9999};
            distro_name = demangle(typeid(normal_distro).name(),status);
            if(distro_name != NULL && status == 0)
...
__attribute__((noinline))
__attribute__((aligned(32)))
void unit_test_cdivv_zmm16r4_unroll_6x_u();
...
void unit_test_cdivv_zmm16r4_unroll_6x_u()
{
    ...
    switch (which)
    {
        ...
        {
            std::uniform_real_distribution<float> uniform_distro;
            const char * __restrict__ fout{"UNIT-TEST_uni_distro_cdivv_zmm16r4_unroll_6x_u.csv"};
            char * distro_name{NULL};
            int32_t status{9999};
            distro_name = demangle(typeid(uniform_distro).name(),status);
            if(distro_name != NULL && status == 0)
...
#pragma omp parallel sections 
{
        ...
        #pragma omp section 
        {
            ...
            printf("[UNIT-TEST]: -- End of data initialization\n");
            printf("[UNIT-TEST]: Number(s) of zeros found: xre=%d,xim%d,yre=%d,yim=%d\n",zcnt1,zcnt2,zcnt3,zcnt4);
            printf("[UNIT-TEST]: -- START: cdivv_zmm16r4_unroll_6x_u\n");
            start  = __rdtscp(&tsc_aux);
            gms::math::cdivv_zmm16r4_unroll_6x_u(&xre[0],&xim[0],&yre[0],&yim[0],
                                                  &zre[0],&zim[0],nelems);
            end     = __rdtscp(&tsc_aux);
            start_c = start-RDTSCP_LAT;
            end_c   = end-RDTSCP_LAT;
            elapsed_tsc = end_c-start_c;
            printf("[UNIT-TEST]: cdivv_zmm16r4_unroll_6x_u() -- TSC=%llu,TSC_AUX=%d\n",elapsed_tsc,tsc_aux);
            fp = fopen(fout,"w+");
            if(!fp)
            {
                 std::perror("fopen failed to open a file -- TERMINATING!!");
            ...
            if(xim) std::free(xim);
            fclose(fp);
            printf("[UNIT-TEST]: -- END: cdivv_zmm16r4_unroll_6x_u\n");
        }
        break;
        case 1 :
        {
            std::normal_distribution<float> normal_distro;
            const char * __restrict__ fout{"UNIT-TEST_norm_distro_cdivv_zmm16r4_unroll_6x_u.csv"};
            char * distro_name{NULL};
            int32_t status{9999};
            distro_name = demangle(typeid(normal_distro).name(),status);
            if(distro_name != NULL && status == 0)
...
int main()
{
     unit_test_cdivv_zmm16r4_unroll_10x_u();
     unit_test_cdivv_zmm16r4_unroll_8x_u();
     unit_test_cdivv_zmm16r4_unroll_6x_u();
     return 0;
}