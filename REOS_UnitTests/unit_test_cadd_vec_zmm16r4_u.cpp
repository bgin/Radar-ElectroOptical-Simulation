...
/*
   icpc -o unit_test_cadd_vec_zmm16r4_u -fp-model fast=2 -ftz -ggdb -ipo -qopt-zmm-usage=high -march=skylake-avx512 -mavx512f -falign-functions=32 -w1 -qopt-report=5 -fopenmp \
   GMS_config.h GMS_malloc.h GMS_cadd_vec_zmm16r4.h GMS_cadd_vec_zmm16r4.cpp unit_test_cadd_vec_zmm16r4_u.cpp
   ASM: 
   icpc -S -fverbose-asm -masm=intel -qopt-zmm-usage=high -march=skylake-avx512 -mavx512f -fopenmp -falign-functions=32 GMS_config.h GMS_malloc.h GMS_cadd_vec_zmm16r4.h GMS_cadd_vec_zmm16r4.cpp unit_test_cadd_vec_zmm16r4_u.cpp
...
void unit_test_caddv_zmm16r4_unroll_16x_u();
...
void unit_test_caddv_zmm16r4_unroll_16x_u()
{
    ...
    std::string fname1 = "UNIT-TEST_uni_distro_cadd_vec_zmm16r4_u16x_u_" + std::to_string(funi_distro_idx++)+".csv";
    std::string fname2 = "UNIT-TEST_norm_distro_cadd_vec_zmm16r4_u16x_u_" + std::to_string(fnorm_distr_idx++)+".csv";
    ...
    switch (which)
    {
        case 0 :
        {
            ...
            printf("[UNIT-TEST]: -- START: caddv_zmm16r4_unroll_16x_u\n");
            start   = __rdtscp(&tsc_aux);
            gms::math::caddv_zmm16r4_unroll_16x_u(&xre[0],&xim[0],&yre[0],&yim[0],
                                                  &zre[0],&zim[0],nelems);
            end     = __rdtscp(&tsc_aux);
            start_c = start-RDTSCP_LAT;
            end_c   = end-RDTSCP_LAT;
            elapsed_tsc = end_c-start_c;
            printf("[UNIT-TEST]: caddv_zmm16r4_unroll_16x_u -- TSC=%llu,TSC_AUX=%d\n",elapsed_tsc,tsc_aux);
            fp = fopen(fout,"w+");
            if(!fp)
            {
            ...
            if(xim_neq_0) std::free(xim);
            fclose(fp);
            printf("[UNIT-TEST]: -- END: caddv_zmm16r4_unroll_16x_u\n");
        }
        break;
        case 1 :
        {
...
__attribute__((noinline))
__attribute__((aligned(32)))
void unit_test_caddv_zmm16r4_unroll_10x_u();
...
void unit_test_caddv_zmm16r4_unroll_10x_u()
{
    ...
    constexpr std::size_t nbytes{reinterpret_cast<std::size_t>(sizeof(float)*nelems)};
    constexpr unsigned long long RDTSCP_LAT{42ull};
    std::string fname1 = "UNIT-TEST_uni_distro_cadd_vec_zmm16r4_u10x_u_" + std::to_string(funi_distro_idx++)+".csv";
    std::string fname2 = "UNIT-TEST_norm_distro_cadd_vec_zmm16r4_u10x_u_" + std::to_string(fnorm_distr_idx++)+".csv";
    float * __restrict__ xre{NULL};
    float * __restrict__ xim{NULL};
    float * __restrict__ yre{NULL};
    float * __restrict__ yim{NULL};
    ...
    switch (which)
    {
        case 0 :
        {
            ...
            printf("[UNIT-TEST]: -- End of data initialization\n");
            printf("[UNIT-TEST]: -- START: caddv_zmm16r4_unroll_10x_u\n");
            start   = __rdtscp(&tsc_aux);
            gms::math::caddv_zmm16r4_unroll_10x_u(&xre[0],&xim[0],&yre[0],&yim[0],
                                                  &zre[0],&zim[0],nelems);
            end     = __rdtscp(&tsc_aux);
            start_c = start-RDTSCP_LAT;
            end_c   = end-RDTSCP_LAT;
            elapsed_tsc = end_c-start_c;
            printf("[UNIT-TEST]: caddv_zmm16r4_unroll_10x_u -- TSC=%llu,TSC_AUX=%d\n",elapsed_tsc,tsc_aux);
            fp = fopen(fout,"w+");
            if(!fp)
            {
                 std::perror("fopen failed to open a file -- TERMINATING!!");
            ...
            if(xim_neq_0) std::free(xim);
            fclose(fp);
            printf("[UNIT-TEST]: -- END: caddv_zmm16r4_unroll_10x_u\n");
        }
        break;
        case 1 :
        {
...
__attribute__((noinline))
__attribute__((aligned(32)))
void unit_test_caddv_zmm16r4_unroll_6x_u();
...
void unit_test_caddv_zmm16r4_unroll_6x_u()
{
    ...
    constexpr std::size_t nbytes{reinterpret_cast<std::size_t>(sizeof(float)*nelems)};
    constexpr unsigned long long RDTSCP_LAT{42ull};
    std::string fname1 = "UNIT-TEST_uni_distro_cadd_vec_zmm16r4_u6x_u_" + std::to_string(funi_distro_idx++)+".csv";
    std::string fname2 = "UNIT-TEST_norm_distro_cadd_vec_zmm16r4_u6x_u_" + std::to_string(fnorm_distr_idx++)+".csv";
    float * __restrict__ xre{NULL};
    float * __restrict__ xim{NULL};
    float * __restrict__ yre{NULL};
    float * __restrict__ yim{NULL};
    ...
    switch (which)
    {
        case 0 :
        {
            ...
            printf("[UNIT-TEST]: -- End of data initialization\n");
            printf("[UNIT-TEST]: -- START: caddv_zmm16r4_unroll_6x_u\n");
            start   = __rdtscp(&tsc_aux);
            gms::math::caddv_zmm16r4_unroll_6x_u(&xre[0],&xim[0],&yre[0],&yim[0],
                                                  &zre[0],&zim[0],nelems);
            end     = __rdtscp(&tsc_aux);
            start_c = start-RDTSCP_LAT;
            end_c   = end-RDTSCP_LAT;
            elapsed_tsc = end_c-start_c;
            printf("[UNIT-TEST]: caddv_zmm16r4_unroll_6x_u -- TSC=%llu,TSC_AUX=%d\n",elapsed_tsc,tsc_aux);
            fp = fopen(fout,"w+");
            if(!fp)
            {
                 std::perror("fopen failed to open a file -- TERMINATING!!");
            ...
            if(xim_neq_0) std::free(xim);
            fclose(fp);
            printf("[UNIT-TEST]: -- END: caddv_zmm16r4_unroll_6x_u\n");
        }
        break;
        case 1 :
        {
...
int main()
{  
   unit_test_caddv_zmm16r4_unroll_16x_u();
   unit_test_caddv_zmm16r4_unroll_10x_u();
   unit_test_caddv_zmm16r4_unroll_6x_u();
   return 0;
}