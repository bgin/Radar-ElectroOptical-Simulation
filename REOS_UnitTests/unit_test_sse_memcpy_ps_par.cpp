...
/*
   icpc -o unit_test_sse_memcpy_ps_par -fp-model fast=2 -fno-exceptions -ftz -ggdb -ipo -march=skylake-avx512 -mavx512f -falign-functions=32 -w1 -qopt-report=5  \
   GMS_config.h GMS_malloc.h GMS_sse_memset.h GMS_sse_memset.cpp GMS_sse_memcpy.h GMS_sse_memcpy.cpp unit_test_sse_memcpy_ps_par.cpp
   ASM: 
   icpc -S -fverbose-asm -masm=intel  -march=skylake-avx512 -mavx512f -fno-exceptions -falign-functions=32 GMS_config.h  GMS_sse_memcpy.h GMS_sse_memcpy.cpp unit_test_sse_memcpy_ps_par.cpp

*/

void unit_test_sse_memcpy_unroll8x_ps_par();
...
void unit_test_sse_memcpy_unroll8x_ps_par()
{
     ...
     p_src = reinterpret_cast<float*>(gms_mm_malloc((std::size_t)(buf_len*sizeof(float)),64ULL));
     p_dst = reinterpret_cast<float*>(gms_mm_malloc((std::size_t)(buf_len*sizeof(float)),64ULL));
     ...
     sse_memset_unroll8x_ps(p_src,fill,buf_len);
     ...
     for(std::size_t __i{0ULL}; __i != n_threads; ++__i)
     {
          ...
          threads.push_back(std::thread(sse_memcpy_unroll8x_ps,chunk_dst,chunk_src,chunk_size));
     ...
     sse_memcpy_unroll8x_ps(rem_dst,rem_src,remaining_chunks);
     for(std::size_t __i{0ULL}; __i != n_threads; ++__i)
     {
          threads[__i].join();
     }
...
void unit_test_sse_memcpy_unroll16x_ps_par()
{
     ...
     float * __restrict rem_dst{p_dst+rem_chunk};
     //threads.push_back(std::thread(sse_memcpy_unroll16x_ps,rem_dst,rem_src,remaining_chunks));
     sse_memcpy_unroll16x_ps(rem_dst,rem_src,remaining_chunks);
     for(std::size_t __i{0ULL}; __i != n_threads; ++__i)
     {
          threads[__i].join();
     }
...
int main()
{
     unit_test_sse_memcpy_unroll8x_ps_par();
