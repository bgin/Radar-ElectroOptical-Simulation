...
/*
   icpc -o unit_test_avx_memcpy_ps -fp-model fast=2 -fno-exceptions -ftz -ggdb -ipo -march=skylake-avx512 -mavx512f -falign-functions=32 -w1 -qopt-report=5  \
   GMS_config.h GMS_malloc.h GMS_sse_memset.h GMS_sse_memset.cpp GMS_avx_memcpy.h GMS_avx_memcpy.cpp unit_test_avx_memcpy_ps.cpp
   ASM: 
   icpc -S -fverbose-asm -masm=intel  -march=skylake-avx512 -mavx512f -fno-exceptions -falign-functions=32 \ 
   GMS_config.h GMS_malloc.h GMS_sse_memset.h GMS_sse_memset.cpp GMS_avx_memcpy.h GMS_avx_memcpy.cpp unit_test_avx_memcpy_ps.cpp

*/

void unit_test_avx_memcpy_unroll8x_ps();
...
void unit_test_avx_memcpy_unroll8x_ps()
{
    using namespace gms::common;
     ...
     avx_memcpy_unroll8x_ps(&dptrs.pdata4000_dst[0],&dptrs.pdata4000_src[0],size_4000);
     ...
     avx_memcpy_unroll8x_ps(&dptrs.pdata1024_dst[0],&dptrs.pdata1024_src[0],size_1024);
     ...
     avx_memcpy_unroll8x_ps(&dptrs.pdata489_dst[0],&dptrs.pdata489_src[0],size_489);
     ...
     avx_memcpy_unroll8x_ps(&dptrs.pdata145_dst[0],&dptrs.pdata145_src[0],size_145);
     ...
     avx_memcpy_unroll8x_ps(&dptrs.pdata16_dst[0],&dptrs.pdata16_src[0],size_16);
     ...
     avx_memcpy_unroll8x_ps(&dptrs.pdata5_dst[0],&dptrs.pdata5_src[0],size_5);
     ...
     avx_memcpy_unroll8x_ps(&dptrs.pdata1_dst[0],&dptrs.pdata1_src[0],size_1);
...
void unit_test_avx_memcpy_unroll16x_ps();
...
void unit_test_avx_memcpy_unroll16x_ps()
{
     ...
     avx_memcpy_unroll16x_ps(&dptrs.pdata4000_dst[0],&dptrs.pdata4000_src[0],size_4000);
     for(std::size_t __i{0ULL}; __i != size_4000; ++__i)
     {
          const float s{dptrs.pdata4000_src[__i]};
          const float d{dptrs.pdata4000_dst[__i]};
     ...
     avx_memcpy_unroll16x_ps(&dptrs.pdata1024_dst[0],&dptrs.pdata1024_src[0],size_1024);
     for(std::size_t __i{0ULL}; __i != size_1024; ++__i)
     {
          const float s{dptrs.pdata1024_src[__i]};
          const float d{dptrs.pdata1024_dst[__i]};
     ...
     avx_memcpy_unroll16x_ps(&dptrs.pdata489_dst[0],&dptrs.pdata489_src[0],size_489);
     for(std::size_t __i{0ULL}; __i != size_489; ++__i)
     {
          const float s{dptrs.pdata489_src[__i]};
          const float d{dptrs.pdata489_dst[__i]};
     ...
     avx_memcpy_unroll16x_ps(&dptrs.pdata145_dst[0],&dptrs.pdata145_src[0],size_145);
     for(std::size_t __i{0ULL}; __i != size_145; ++__i)
     {
          const float s{dptrs.pdata145_src[__i]};
          const float d{dptrs.pdata145_dst[__i]};
     ...
     avx_memcpy_unroll16x_ps(&dptrs.pdata16_dst[0],&dptrs.pdata16_src[0],size_16);
     for(std::size_t __i{0ULL}; __i != size_16; ++__i)
     {
          const float s{dptrs.pdata16_src[__i]};
          const float d{dptrs.pdata16_dst[__i]};
     ...
     avx_memcpy_unroll16x_ps(&dptrs.pdata5_dst[0],&dptrs.pdata5_src[0],size_5);
     for(std::size_t __i{0ULL}; __i != size_5; ++__i)
     {
          const float s{dptrs.pdata5_src[__i]};
          const float d{dptrs.pdata5_dst[__i]};
     ...
     avx_memcpy_unroll16x_ps(&dptrs.pdata1_dst[0],&dptrs.pdata1_src[0],size_1);
     for(std::size_t __i{0ULL}; __i != size_1; ++__i)
     {
          const float s{dptrs.pdata1_src[__i]};
          const float d{dptrs.pdata1_dst[__i]};
...
int main()
{
    ...
    unit_test_avx_memcpy_unroll8x_ps();
    unit_test_avx_memcpy_unroll16x_ps();
    return 0;
}