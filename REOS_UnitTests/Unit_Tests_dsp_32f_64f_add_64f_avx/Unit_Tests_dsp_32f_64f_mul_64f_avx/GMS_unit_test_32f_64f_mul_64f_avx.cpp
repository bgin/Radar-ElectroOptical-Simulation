...
static const char *funcs[] = {"dsp_32f_64f_mul_64f_u_avx", "dsp_32f_64f_mul_64f_a_avx"};

void unit_test_dsp_32f_64f_mul_64f_u_avx()
{
     ...
     const char * __restrict fname = "UNIT_TEST_OutputData_dsp_32f_64f_mul_64f_u_avx.csv";
     ...
     dsp_32f_64f_mul_64f_u_avx(&buf_c[0],&buf_a[0],&buf_b[0],npts);
     if(fopen_failed)
     {
...
void unit_test_dsp_32f_64f_mul_64f_a_avx()
{
     constexpr int32_t npts{1000};
     ...
     std::clock_t seed_a{0L};
     std::clock_t seed_b{0L};
     const char * __restrict fname = "UNIT_TEST_OutputData_dsp_32f_64f_mul_64f_a_avx.csv";
     bool  fopen_failed = false;
     FILE * fp{fopen(fname,"w+")};
     if(!fp)
     {
     ...
     printf("[Unit-Test]: %s() -- START\n",funcs[0]);
     dsp_32f_64f_mul_64f_a_avx(&buf_c[0],&buf_a[0],&buf_b[0],npts);
     if(fopen_failed)
     {
        printf("Dumping results to screen\n");
        for(int32_t i = 0; i != npts; ++i)
...
int main()
{
     unit_test_dsp_32f_64f_mul_64f_u_avx();
     unit_test_dsp_32f_64f_mul_64f_a_avx();
     return 0;
}