...
/*
   icpc -o unit_test_erfcinv_vec_zmm16r4_u -fp-model fast=2 -ftz -ggdb -ipo -qopt-zmm-usage=high -march=skylake-avx512 -mavx512f -falign-functions=32 -w1 -qopt-report=5  \
   GMS_config.h GMS_malloc.h GMS_erfcinv_vec_zmm16r4.h GMS_erfcinv_vec_zmm16r4.cpp unit_test_erfcinv_vec_zmm16r4_u.cpp
   ASM: 
   icpc -S -fverbose-asm -masm=intel -qopt-zmm-usage=high -march=skylake-avx512 -mavx512f -falign-functions=32 GMS_config.h GMS_malloc.h GMS_erfcinv_vec_zmm16r4.h GMS_erfcinv_vec_zmm16r4.cpp unit_test_erfcinv_vec_zmm16r4_u.cpp
*/
...
__attribute__((noinline))
__attribute__((aligned(32)))
void unit_test_erfcinvv_zmm16r4_unroll_10x_u();
...
void unit_test_erfcinvv_zmm16r4_unroll_10x_u()
{
    ...
    const char * __restrict__ fout{"UNIT-TEST_Output_erfcinvv_zmm16r4_unroll_10x_u.csv"};
    ...
    switch (which)
    {
        case 0 : 
        {
            ...
            p_x = reinterpret_cast<float * __restrict__>(std::malloc(nbytes));
            p_y = reinterpret_cast<float * __restrict__>(std::malloc(nbytes));
            seed_x = std::clock();
            ...
            printf("[UNIT-TEST]: -- START: erfcinvv_zmm16r4_unroll_10x_u\n");
            gms::math::erfcinvv_zmm16r4_unroll_10x_u(&p_x[0],&p_y[0],a,b,c,static_cast<int32_t>(nelems));
            fp = fopen(fout,"w+");
            if(!fp)
            {
            ...
            if(p_y) std::free(p_y);
            fclose(fp);
            printf("[UNIT-TEST]: -- END: erfcinvv_zmm16r4_unroll_10x_u\n");
        }
        break;
        case 1 :
        {
...
__attribute__((noinline))
__attribute__((aligned(32)))
void unit_test_erfcinvv_zmm16r4_unroll_6x_u();
...
void unit_test_erfcinvv_zmm16r4_unroll_6x_u()
{
    ...
    float * __restrict__ p_y{NULL};
    FILE  *  __restrict__ fp{NULL};
    const char * __restrict__ fout{"UNIT-TEST_Output_erfcinvv_zmm16r4_unroll_6x_u.csv"};
    constexpr std::size_t nelems{67456};
    constexpr std::size_t nbytes{4ull*nelems};
    std::clock_t seed_d{0ull};
    std::clock_t seed_x{0ull};
    ...
    switch (which)
    {
        case 0 : 
        {
            ...
            check_input_range(&p_x[0],static_cast<int32_t>(nelems));
            printf("[UNIT-TEST]: -- End of data initialization\n");
            printf("[UNIT-TEST]: -- START: erfcinvv_zmm16r4_unroll_6x_u\n");
            gms::math::erfcinvv_zmm16r4_unroll_6x_u(&p_x[0],&p_y[0],a,b,c,static_cast<int32_t>(nelems));
            fp = fopen(fout,"w+");
            if(!fp)
            {
                 std::perror("fopen failed to open a file -- TERMINATING!!");
            ...
            if(p_y) std::free(p_y);
            fclose(fp);
            printf("[UNIT-TEST]: -- END: erfcinvv_zmm16r4_unroll_6x_u\n");
        }
        break;
        case 1 :
        {
...
int main()
{
    unit_test_erfcinvv_zmm16r4_unroll_10x_u();
    unit_test_erfcinvv_zmm16r4_unroll_6x_u();
    return 0;
}