...
!dir$ code_align : 32 :: DAXPY
!dir$ attributes optimization_parameters: TARGET_ARCH=skylake_avx512 :: DAXPY
...
C
   20 M = MOD(N,4)
      IF (M .EQ. 0) GO TO 40
      !$omp simd reduction(+:DY) linear(I:1) unroll partial(8)
      DO 30 I = 1,M
        DY(I) = DY(I) + DA*DX(I)
   30 CONTINUE
      IF (N .LT. 4) RETURN
...
!dir$ attributes optimize : 3
!dir$ code_align : 32 :: DCOPY
!dir$ attributes optimization_parameters: TARGET_ARCH=skylake_avx512 :: DCOPY
...
C
    5 IX = 1
      ...
      IF (INCX .LT. 0) IX = (-N+1)*INCX + 1
      IF (INCY .LT. 0) IY = (-N+1)*INCY + 1
      !$omp simd linear(I:1) unroll partial(8)
      DO 10 I = 1,N
        DY(IY) = DX(IX)
        IX = IX + INCX
        IY = IY + INCY
...
C
   20 M = MOD(N,7)
      IF (M .EQ. 0) GO TO 40
      !$omp simd linear(I:1) unroll partial(16)
      DO 30 I = 1,M
        DY(I) = DX(I)
   30 CONTINUE
      IF (N .LT. 7) RETURN
...
!dir$ attributes optimize : 3
!dir$ code_align : 32 :: DDOT
!dir$ attributes optimization_parameters: TARGET_ARCH=skylake_avx512 :: DDOT  
...
C
    5 IX = 1
      ...
      IF (INCX .LT. 0) IX = (-N+1)*INCX + 1
      IF (INCY .LT. 0) IY = (-N+1)*INCY + 1
      !$omp simd reduction(+:DDOT) linear(I:1) 
      DO 10 I = 1,N
...
C
   ...
   30 CONTINUE
      IF (N .LT. 5) RETURN
 40   MP1 = M + 1
        !$omp simd reduction(+:DDOT) linear(I:5) 
      DO 50 I = MP1,N,5
...
!dir$ attributes optimize : 3
!dir$ code_align : 32 :: DSCAL
!dir$ attributes optimization_parameters: TARGET_ARCH=skylake_avx512 :: DSCAL
...
C
      IX = 1
      IF (INCX .LT. 0) IX = (-N+1)*INCX + 1
      !$omp simd reduction(+:DX) linear(I:1)
      DO 10 I = 1,N
        DX(IX) = DA*DX(IX)
        IX = IX + INCX
   10 CONTINUE
...
C
   ...
   30 CONTINUE
      IF (N .LT. 5) RETURN
 40   MP1 = M + 1
       !$omp simd reduction(+:DX) linear(I:5)
      DO 50 I = MP1,N,5
        DX(I) = DA*DX(I)
        DX(I+1) = DA*DX(I+1)
        DX(I+2) = DA*DX(I+2)
